...
**In-flight prerelease candidate:** **v3.9.1-local-efficiency-benchmark-alpha** (Phase 17B, candidate; tag created only after this PR merges and post-merge benchmark reproduces). Adds a reproducible benchmark harness that aggregates the existing Phase 11–17A canonical proofs into a single artifact with the master-prompt-mandated metric set; provides documented NOT_RUN slots for external competitors; introduces an optional Ollama latency probe (default skipped for `--network none` safety). v3.8.0 must remain Latest. v3.9.0-producer-fabric-alpha must remain the previous Pre-release.
