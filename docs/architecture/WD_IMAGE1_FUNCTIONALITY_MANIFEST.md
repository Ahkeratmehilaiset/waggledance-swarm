...
| Hexagonal upgrades | Partial, with in-memory proof, runtime-boundary smoke, and read-only topology boundary `/metrics` gauges | `waggledance/core/hex_topology/subdivision_operator.py`, `ring_messaging.py`, `parent_child_relations.py`, `waggledance/bootstrap/container.py`, `hex_topology_registry.py`, `hex_neighbor_assist.py`, `waggledance/adapters/http/routes/metrics.py` | Add a shadow subdivision replay artifact that binds the parent/child plan to topology boundary metrics without runtime mutation. |
...
agent payloads, and they do not enable runtime subdivision authority.
