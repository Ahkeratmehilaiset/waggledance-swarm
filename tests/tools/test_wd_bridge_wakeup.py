...
@pytest.mark.parametrize("scenario,agent,backend,model,effort", [
    ...
    for scenario in ["complete", "ack", "stale", "wake_during", "timeout", "wrong_identity",
                     "child_survives", "child_drains", "wrong_task", "future", "huge_receipt", "output_budget", "two_turns", "idle", "blocked"]
] + [
...
scenario = sys.argv[2]
owner = json.loads((pathlib.Path(spec["wake_path"]).parent / (".wd-turn-" + spec["agent"] + ".owner.json")).read_text(encoding="utf-8-sig"))
assert owner["status"] == "running" and owner["turn_id"] == spec["turn_id"]
assert pathlib.Path(owner["pending_path"]).exists()
...
if scenario == "wake_during" or (scenario == "two_turns" and len(calls.read_text().splitlines()) == 1):
...
if scenario != "stale":
    checkpoint.write_text(json.dumps({"schema":"wd.lane-current.v1", "agent":spec["agent"],
        "worktree":spec["worktree"], "task_id":"test", "status":scenario if scenario in ["idle","blocked"] else "completed",
...
if scenario != "ack":
    ...
    receipt["disposition"] = scenario if scenario in ["idle","blocked"] else "completed"
...
''')
    script = f"""
. {quote(RUNNER)}
function Get-WdTurnArguments {{ param($Backend, $Model, $Effort, $ImagePath)
...
Invoke-WdLaneTurnLoop -Agent {agent} -Backend {backend} -CliPath {quote(PYTHON)} `
  ...
  -ImagePath 'initial-image.png' -MaxTurns {2 if scenario == 'two_turns' else 1} `
  -TurnTimeoutSeconds 2 -MaxOutputBytes 4096
"""
    ...
    expected = {"complete":"completed", "wake_during":"completed", "ack":"blocked_missing_receipt",
                ...
                "two_turns":"completed", "idle":"idle", "blocked":"blocked"}[scenario]
    ...
    checkpointed = expected in {"completed", "idle"}
    ...
    assert len(calls) == (2 if scenario == "two_turns" else 1)
...
[pscustomobject]@{{
 ...
 claude=@(Get-WdTurnArguments claude sonnet max '' 'C:\\runtime bridge')
...
''')
    ...
    script = f"""
. {quote(RUNNER)}
function Get-WdTurnArguments {{ return @({quote(fake)}, $script:WdTurnSpecPath, {quote('blocked' if forever else 'completed')}) }}
Invoke-WdLaneTurnLoop -Agent codex-lead-1 -Backend codex -CliPath {quote(PYTHON)} `
  -Model gpt-5.6-sol -Effort ultra -Worktree {quote(tmp_path)} -RuntimeRoot {quote(runtime)} `
...
"""
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            try:
                status = json.loads(owner_path.read_text(encoding="utf-8-sig"))["status"]
        ...
        if forever:
            ...
            while time.monotonic() < deadline:
                try:
                    deferred = json.loads((journal / "owner.json").read_text(encoding="utf-8-sig"))
                except FileNotFoundError:
                    continue
    ...
    assert json.loads(stdout)["last_disposition"] == ("blocked" if forever else "completed")
