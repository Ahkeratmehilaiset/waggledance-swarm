...
| `ARCH-006` | Phase 2BR2 | Dual-ledger contract (phase_fix_ledger vs regression_ledger) is implicit | fixed | Phase 2BR3 | `Test-LedgerContract.ps1` | `docs/design/ledger_contract.md :: Dual-ledger contract`, `orchestrator/Test-LedgerContract.ps1 :: PM-CL-002` |
...
| `REL-020`  | Phase 2BR2 | Cockpit fetch URL not updated after orchestrator/cockpit/ relocation | fixed | Phase 2BR3 | `Test-CockpitData.ps1` | `orchestrator/cockpit/review_cockpit.html :: COCKPIT_DATA_URL`, `orchestrator/Test-CockpitData.ps1 :: COCKPIT_DATA_URL resolves to repo-root` |
| `REL-021`  | Phase 2BR3 | P5c regression-ledger hook silently downcasts critical/high review findings to severity=info | fixed | Phase 2BR3 | `Test-RegressionLedger.ps1` | `orchestrator/lib/RegressionLedger.ps1 :: Phase 2B-R3 P10 (Codex REL-001 fix)`, `orchestrator/lib/RegressionLedger.ps1 :: _Rl-MaxSeverity` |
| `REL-022`  | Phase 2BR3 | Save-WaggleRegressionLedger deletes old file before moving temp; crash window can lose ledger entirely | fixed | Phase 2BR3 | `Test-RegressionLedger.ps1` | `orchestrator/lib/RegressionLedger.ps1 :: Phase 2B-R3 P10 (Codex REL-002 fix)` |
| `SEC-010`  | Phase 2BR3 | Redactor missing rule for URL-userinfo credentials and fine-grained GitHub PAT prefix | fixed | Phase 2BR3 | `Test-Redactor.ps1`, `Test-Redaction.ps1` | `orchestrator/lib/Redactor.ps1 :: GITHUB_FINE_GRAINED_PAT`, `orchestrator/lib/Redactor.ps1 :: URL_USERINFO_CREDENTIAL` |
| `ARCH-008` | Phase 2BR3 | Cockpit external-origin allowlist disagreed across header comment, footer text, and PROVIDER_URLS | fixed | Phase 2BR3 | `Test-CockpitData.ps1` | `orchestrator/cockpit/review_cockpit.html :: PROVIDER_URLS`, `orchestrator/cockpit/review_cockpit.html :: ALLOWED EXTERNAL ORIGINS`, `orchestrator/Test-CockpitData.ps1 :: allowlist consistency` |
| `ARCH-009` | Phase 2BR3 | ledger_contract.md described stale runtime-ledger schema and helper names | fixed | Phase 2BR3 | `Test-LedgerContract.ps1` | `docs/design/ledger_contract.md :: Authoritative writer`, `orchestrator/Test-LedgerContract.ps1 :: RequiredContractRuntimeHelpers`, `orchestrator/Test-LedgerContract.ps1 :: ForbiddenContractStaleNames` |
| `SEC-011`  | Phase 2BR3 | Source-supplement redaction omitted new URL-userinfo and fine-grained PAT rules | fixed | Phase 2BR3 | `Test-Redactor.ps1` | `orchestrator/lib/Redactor.ps1 :: SourceSupplementRuleNames`, `orchestrator/lib/Redactor.ps1 :: Phase 2B-R3 P10c (Codex post-fix SEC-001 fix)` |
| `REL-023`  | Phase 2BR3 | Severity floor was applied on insert but downcast on score_delta updates | fixed | Phase 2BR3 | `Test-RegressionLedger.ps1` | `orchestrator/lib/RegressionLedger.ps1 :: Phase 2B-R3 P10c (Codex post-fix REL-001 fix)`, `orchestrator/lib/RegressionLedger.ps1 :: Update-WaggleRegressionEntry` |
| `ARCH-010` | Phase 2BR3 | Schema rejected runtime score_categories field but contract documented it as preserved | fixed | Phase 2BR3 | `Test-LedgerContract.ps1` | `schemas/regression_ledger.schema.json :: score_categories`, `docs/design/ledger_contract.md :: Authoritative writer` |
| `ARCH-011` | Phase 2BR3 | ledger_contract.md dedup section described non-existent updated_at_utc and seen_count fields | fixed | Phase 2BR3 | — | `docs/design/ledger_contract.md :: Dedup contract` |
