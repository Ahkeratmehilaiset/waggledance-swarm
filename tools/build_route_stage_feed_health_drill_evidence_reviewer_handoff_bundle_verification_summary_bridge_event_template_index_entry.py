...
"""Build a local index entry for a route-stage handoff-bundle summary template."""
...
from tools.build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template import (  # noqa: E402
...
PROOF_ID = (
    ...
    "handoff_bundle_verification_summary_bridge_event_template_index_entry_v1"
)
SUMMARY_ARTIFACT_ID = (
    "route_stage_feed_health_drill_evidence_reviewer_"
    "handoff_bundle_verification_summary"
)
TEMPLATE_ARTIFACT_ID = (
    "route_stage_feed_health_drill_evidence_reviewer_"
    "handoff_bundle_verification_summary_bridge_event_template"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-19T07:05:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry(
        ...
        report = _failure_report(
            "route_stage_feed_health_drill_evidence_reviewer_"
            "handoff_bundle_verification_summary_bridge_event_template_"
            "index_entry_invalid"
        )
    ...
    if args.json or report["ok"]:
        ...
        print(
            "route-stage handoff-bundle summary bridge-event-template index "
            "entry FAILED: "
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    _assert_mapping(SUMMARY_ARTIFACT_ID, bundle_verification_summary)
    _assert_mapping(TEMPLATE_ARTIFACT_ID, summary_bridge_event_template_report)
    _assert_no_forbidden_input(SUMMARY_ARTIFACT_ID, bundle_verification_summary)
    ...
    summary_verification = _mapping(
        bundle_verification_summary.get(ROUTE_STAGE_BUNDLE_VERIFICATION_KEY)
    )
    reviewer = _mapping(bundle_verification_summary.get("reviewer_ownership"))
    artifacts = [
        _artifact_record(
            artifact_id=SUMMARY_ARTIFACT_ID,
            role="verified_handoff_bundle_summary_context",
    ...
    entry = {
        ...
        "verification_summary": {
            "summary_proof_id": _safe_ref_or_invalid(
                bundle_verification_summary.get("proof_id")
            ),
            "verification_ok": True,
            "verification_version": _safe_ref_or_invalid(
                summary_verification.get("verification_version")
            ),
            "artifact_count_checked": _as_nonnegative_int(
                summary_verification.get("artifact_count_checked")
            ),
        ...
        "reviewer_next_actions": [
            "review_route_stage_handoff_bundle_summary_bridge_event_template_index_entry",
...
def _rebuilt_template(
    ...
    rebuilt = build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template(
        summary=summary,
...
def _assert_template_report_contract(
    ...
    *,
    ...
    if payload.get("summary_proof_id") != bundle_verification_summary.get(
        "proof_id"
    ):
        raise TemplateIndexEntryError(
    ...
    summary_reviewer = _mapping(bundle_verification_summary.get("reviewer_ownership"))
    for field in (
        "reviewer_agent_id",
        "handoff_ref",
    ...
    payload_verification = _mapping(payload.get(ROUTE_STAGE_BUNDLE_VERIFICATION_KEY))
    summary_verification = _mapping(
        bundle_verification_summary.get(ROUTE_STAGE_BUNDLE_VERIFICATION_KEY)
    )
    for field in (
...
def _summary_contract_blockers(summary: Mapping[str, Any]) -> list[str]:
    ...
    _expect_empty_items_or_collect(
        ...
        "bundle_verification_summary_blockers_present",
        blockers,
    )
    ...
    if verification.get("artifact_count_checked") != 2:
    ...
    for field in (
        ...
        if verification.get(field) != "match":
            blockers.append(f"bundle_verification_{field}_not_match")
    ...
    _expect_empty_items_or_collect(
        ...
        "bundle_verification_blockers_present",
        blockers,
    )
    for check_name in ("digest_checks", "size_checks", "schema_version_checks"):
        checks = _mapping(verification.get(check_name))
        if not checks or set(checks.values()) != {"match"}:
            blockers.append(f"bundle_verification_{check_name}_not_all_match")
...
def _failure_report(reason: str) -> dict[str, Any]:
    return {
        ...
        "blockers": [
            ...
            f"entry_failed:{_safe_reason(reason)}"
        ],
        "warnings": [],
    }
