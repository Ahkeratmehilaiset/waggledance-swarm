...
"""Render a sanitized MAGMA summary bridge-template index-entry verifier summary."""
...
SUMMARY_VERSION = "magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary.v1"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--summary-bridge-template-index-entry-verification-json",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            ...
            summary = _failure_summary(
                "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_invalid"
            )
    ...
    if args.json:
        ...
        print(
            ...
            "bridge-event template index-entry verification summary bridge-event "
            "template index-entry verification summary FAILED: "
...
def build_magma_decision_review_verification_template_index_entry_summary_bridge_event_template_index_entry_verification_summary(
    *,
    ...
    """Return path-free reviewer context for a local summary index-entry verification."""
    ...
    summary = {
        ...
        "reviewer_next_actions": [
            "review_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification",
            "compare_summary_bridge_template_index_entry_verification_to_local_artifacts",
...
def render_magma_decision_review_verification_template_index_entry_summary_bridge_event_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# MAGMA Summary Bridge-Template Index-Entry Verification Summary",
        "",
        ...
        "",
        "## Summary Bridge-Template Index-Entry Verification",
        "",
...
def _failure_summary(reason: str) -> dict[str, Any]:
    return {
        ...
        "local_paths_recorded": False,
        "blockers": [
            "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_failed:"
        ...
        for raw_key, child in value.items():
            ...
            blockers.extend(
                _recursive_boundary_blockers(
                    child, parent_path=parent_path + (key,)
                )
            )
...
def _input_key_boundary_blockers(
    key: str, value: Any, parent_path: tuple[str, ...]
) -> list[str]:
    blockers: list[str] = []
    ...
    if parent_path and key in _AUTHORITY_FALSE_FIELDS and value is not False:
        blockers.append(
            f"verification_report_nested_authority_field_not_false:{key}"
        )
    if parent_path and key in _REFERENCE_FALSE_FIELDS and value is not False:
        blockers.append(
            "verification_report_nested_reference_authority_field_not_false:"
            f"{key}"
        )
    return blockers
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            ...
            "bridge-event template index-entry verification summary bridge-event "
            "template index-entry verification summary contains forbidden markers"
        )
