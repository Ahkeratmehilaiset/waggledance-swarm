...
"""Build a hex cross-consistency verifier-summary bridge-event template."""
...
TEMPLATE_VERSION = (
    ...
    "verification_summary_bridge_event_template.v1"
)
PROOF_ID = (
...
EVENT_STATUS = (
    "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
    "verification_summary_bridge_event_template_ready"
)
HEX_VERIFICATION_KEY = (
    "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
    "verification"
)
_ARTIFACT_IDS = (DIGEST_ARTIFACT_ID, TEMPLATE_ARTIFACT_ID)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T09:30:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = _failure_report(
            ...
            "index_entry_verification_summary_bridge_event_template_invalid"
        )
    ...
    if args.json:
        ...
        print(
            "hex verifier-summary bridge-event template FAILED: "
...
def build_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template(
    *,
    ...
    payload = {
        ...
        "reviewer_next_actions": [
            "review_hex_cross_consistency_index_entry_verification_summary_bridge_event_template",
    ...
    event = {
        ...
        "to": targets,
        "message": (
            "Hex cross-consistency index-entry verification summary bridge-event "
            "template ready; manual_review_required=true; approval_granted=false; "
            "release_decision_made=false; merge_decision_made=false; "
            "promotion_granted=false; claim_safe=false; template_only=true; no "
            "bridge write, transport, external fetch, payload inclusion, local "
            "path recording, runtime controls, runtime authority, runtime "
            "subdivision authority, fast-track priority, gate skip, claim "
            "upgrade, merge, promotion, or release decision."
        ),
        "paths": [],
        "write_scope": [],
        "run_id": run_id,
...
def _summary_contract_blockers(summary: Mapping[str, Any]) -> list[str]:
    ...
    for field in (
        ...
        if verification.get(field) != "match":
            blockers.append(f"index_entry_verification_{field}_not_match")
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "hex verifier-summary bridge-event template contains forbidden markers"
        )
