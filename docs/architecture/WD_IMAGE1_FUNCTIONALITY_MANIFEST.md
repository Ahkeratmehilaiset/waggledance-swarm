...
authority. The local digest-template index entry, verifier, path-free verifier
summary, verifier-summary bridge-event template, local index entry for that
template, local verifier for that index entry, and path-free reviewer summary
for that verifier result continue that same review chain without appending
bridge events, including payloads or local paths, upgrading claims, or granting
runtime subdivision authority. The next small step is a template-only
bridge-event renderer for that verifier-result summary.
