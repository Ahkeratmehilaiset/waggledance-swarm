...
**Freshness audit:** 2026-07-07 read-only public-doc refresh and staleness audit, following the 2026-06-20, 2026-06-15, 2026-06-11, 2026-06-06, and 2026-05-27 read-only audits, found the dated PROVEN/MEASURED evidence older than the dream-mode <=14-day freshness target. On 2026-07-07, the referenced global matrix evidence band is 62 days old, and the G/J/L priority rows are also 62 days old. Labels below remain historical evidence labels until refreshed; these audits do not upgrade or invalidate any row.
**Freshness metadata:** `snapshot_date=2026-05-06`; `freshness_audit_date=2026-07-07`; `prior_freshness_audit_date=2026-06-20`; `max_age_days=14`; `status=historical_stale`; `fresh_for_planning=false`; `priority_rows=G,J,L`; `priority_rows_snapshot_date=2026-05-06`; `priority_rows_freshness_audit_date=2026-07-07`; `priority_rows_fresh_for_planning=false`; `historical_labels_until_refreshed=true`.
...
The 2026-06-06 bridge audit rechecked the same snapshot and found those evidence bands 31-33 days old. The 2026-06-11 bridge audit rechecked the same snapshot and found those evidence bands 36-38 days old. The 2026-06-15 bridge audit rechecked the same matrix snapshot and found the matrix and G/J/L priority rows 40 days old. The 2026-06-20 bridge audit rechecked the same matrix snapshot and found the matrix and G/J/L priority rows 45 days old. The 2026-07-07 public-doc refresh rechecked the same matrix snapshot and found the matrix and G/J/L priority rows 62 days old. These are staleness markers only, not reruns, carry-forward proofs, label upgrades, or row invalidations.

The 2026-07-07 public-doc refresh found visible current movement in the rival governance/control-plane space: Microsoft Agent Governance Toolkit continues to present deterministic runtime policy enforcement, zero-trust identity, sandboxing, reliability, and 10/10 OWASP Agentic Top 10 coverage (`https://github.com/microsoft/agent-governance-toolkit`, `https://microsoft.github.io/agent-governance-toolkit/`); Asqav now has public material for quantum-safe signed, hash-chained audit trails and a Dify plugin (`https://www.helpnetsecurity.com/2026/04/09/asqav-ai-agent-audit-trail/`, `https://marketplace.dify.ai/plugin/asqav/asqav`); Preloop presents an open-source control plane combining MCP firewall, model gateway, policy-as-code, human approvals, runtime observability, and audit trails (`https://github.com/preloop/preloop`); JamJet presents a safety layer for tool-call blocking, approvals, budgets, audit, and replay (`https://github.com/jamjet-labs/jamjet`); and Cordum distinguishes out-of-process governance kernels from in-process policy engines (`https://cordum.io/blog/in-process-vs-out-of-process-ai-agent-governance`). This refresh is public-doc-only; it does not rerun WD axes G/J/L, does not compare benchmark outputs, and does not change any label below.
