...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_recomputes_digests_without_authority() -> None:
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_rejects_digest_mismatch_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert payload["source_contract_check"] == "match"
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_rejects_missing_record() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_rejects_nested_authority() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert report["ok"] is False
    assert "template_index_entry_runtime_authority_granted_not_false" in report[
        "blockers"
    ]
    assert report["runtime_authority_granted"] is False
    assert report["release_decision_made"] is False
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_rejects_deterministic_entry_drift() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_rejects_source_contract_forgery() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    tampered_summary = copy.deepcopy(artifacts["summary"])
    tampered_summary[VERIFICATION_KEY]["blocker_count"] = "1"
    ...
    assert report["ok"] is False
    assert (
        "source_contract_failed:summary_bridge_event_template_source_contract_"
        "failed:index_entry_verification_blocker_count_nonzero"
        in report["blockers"]
    )
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_missing_input_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert payload["blockers"] == [
        f"{INDEX_ENTRY_ARTIFACT_ID}_verification_failed:"
...
def test_route_stage_handoff_verifier_summary_bridge_template_index_entry_verifier_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
