...
SCRIPT = (
    ROOT
    / "tools"
    / "verify_v12_memory_palace_shortcut_runtime_promotion_design.py"
)
...
def test_verifies_valid_runtime_promotion_design() -> None:
    ...
    assert verification["runtime_promotion_design_count_checked"] == 2
...
def test_rejects_design_row_side_effects_without_echoing_values() -> None:
    report = _valid_report()
    report["runtime_promotion_designs"][0]["runtime_route_changed"] = True
    report["runtime_promotion_designs"][0]["target_node_id"] = (
    ...
    assert verification["ok"] is False
    assert verification["design_row_action_boundary_check"] == "mismatch"
    assert "runtime_design_0_runtime_route_changed_not_false" in (
...
def test_rejects_summary_drift() -> None:
    report = _valid_report()
    report["design_summary"]["runtime_promotion_design_count"] = 99
    ...
    assert verification["ok"] is False
    assert "runtime_promotion_design_count_mismatch" in (
        verification["blockers"]
    )


def test_cli_json_verifies_report_path_free(tmp_path: Path) -> None:
    report_path = tmp_path / "runtime_design_report.json"
...
def test_cli_rejects_duplicate_json_keys_path_free(tmp_path: Path) -> None:
    ...
    assert payload["blockers"] == [
        "memory_palace_shortcut_runtime_promotion_design_report_"
        "verification_failed:"
        "memory_palace_shortcut_runtime_promotion_design_report_json_error"
    ]
...
def test_cli_missing_input_is_path_free() -> None:
    ...
    assert payload["blockers"] == [
        "memory_palace_shortcut_runtime_promotion_design_report_"
        "verification_failed:"
        "memory_palace_shortcut_runtime_promotion_design_report_unreadable"
    ]
    combined = result.stdout + result.stderr
...
def _valid_report() -> dict[str, object]:
    return build_memory_palace_shortcut_runtime_promotion_design(
