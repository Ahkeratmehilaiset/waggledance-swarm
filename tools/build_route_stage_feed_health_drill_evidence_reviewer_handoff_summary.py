...
"""Render a path-free route-stage feed-health reviewer handoff summary."""
...
from tools.build_route_stage_feed_health_drill_evidence_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry import (  # noqa: E402
...
SUMMARY_VERSION = "waggledance.route_stage_feed_health_drill_evidence_reviewer_handoff_summary.v1"
PROOF_ID = "route_stage_feed_health_drill_evidence_reviewer_handoff_summary_v1"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-19T06:00:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            ...
            summary = _failure_summary(
                "route_stage_feed_health_drill_evidence_reviewer_handoff_summary_invalid"
            )
    ...
    if args.json:
        ...
        print(render_route_stage_feed_health_drill_evidence_reviewer_handoff_summary_markdown(summary))
    else:
        print(
            "route-stage feed-health reviewer handoff summary FAILED: "
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_summary(
    *,
    ...
    """Return path-free reviewer context for the full route-stage verifier chain."""
    ...
    summary = {
        ...
        "reviewer_next_actions": [
            "review_route_stage_feed_health_drill_evidence_chain",
            "confirm_verifier_summary_bridge_event_template_index_entry",
...
def render_route_stage_feed_health_drill_evidence_reviewer_handoff_summary_markdown(
    summary: Mapping[str, Any],
) -> str:
    verifier = _mapping(summary.get("validated_verifier_chain"))
    boundary = _mapping(summary.get("operator_boundary"))
    lines = [
        "# Route-Stage Feed-Health Reviewer Handoff Summary",
        "",
        ...
        "## Verifier Chain",
        "",
        f"- Verification OK: `{verifier.get('verification_ok')}`",
        f"- Verification version: `{verifier.get('verification_version')}`",
        f"- Index entry version: `{verifier.get('index_entry_version')}`",
        f"- Artifact count checked: `{verifier.get('artifact_count_checked')}`",
        f"- Source contract check: `{verifier.get('source_contract_check')}`",
        "- Rebuilt index-entry check: "
        f"`{verifier.get('rebuilt_index_entry_check')}`",
        "- Bridge event schema check: "
        f"`{verifier.get('bridge_event_schema_check')}`",
        f"- Template only: `{verifier.get('template_only')}`",
        "- Verification blockers:",
    ]
    lines.extend(_markdown_token_list(verifier.get("blockers")))
    lines.append("- Verification warnings:")
    lines.extend(_markdown_token_list(verifier.get("warnings")))
    lines.extend([
        "",
        "## Operator Boundary",
        "",
    ...
    lines.extend([
        "",
        "This summary is reviewer handoff context only. It does not approve, "
        "merge, promote, append bridge events, transport artifacts, fetch "
        "endpoints, include payloads, record local paths, control feeds, or "
...
def _markdown_token_list(value: Any) -> list[str]:
    tokens = value if isinstance(value, Sequence) and not isinstance(value, str) else []
    safe_tokens = [_safe_token(token) for token in tokens if isinstance(token, str)]
    return [f"  - `{token}`" for token in safe_tokens] or ["  - `none`"]


def _assert_no_forbidden_input(label: str, value: Any) -> None:
    blob = json.dumps(value, allow_nan=False, sort_keys=True)
    if _forbidden_output_markers(blob):
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError("forbidden_output_marker")


def _forbidden_output_markers(text: str) -> bool:
