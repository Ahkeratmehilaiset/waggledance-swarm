...
"""Verify a local runtime gap scheduler-candidate bridge-template index entry."""
...
INDEX_ENTRY_ARTIFACT_ID = (
    "runtime_gap_scheduler_candidate_bridge_event_template_index_entry"
)
_REQUIRED_ARTIFACTS = (ARTIFACT_ID, TEMPLATE_ARTIFACT_ID)
_INDEX_ENTRY_REQUIRED_FALSE_FIELDS = tuple(
    field
...
class RuntimeGapIndexEntryVerificationError(ValueError):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except RuntimeGapIndexEntryVerificationError as exc:
    ...
    if args.json or report["ok"]:
        ...
        print(
            "runtime gap scheduler-candidate bridge-template index entry "
            "verification FAILED: "
...
def verify_runtime_gap_scheduler_candidate_bridge_event_template_index_entry(
    *,
    ...
    """Recompute artifact checks for the scheduler-candidate index entry."""
    ...
    artifacts = {
        ARTIFACT_ID: (artifact, artifact_bytes),
        TEMPLATE_ARTIFACT_ID: (
...
def _collect_boundary_blockers(
    ...
    for field in _INDEX_ENTRY_REQUIRED_FALSE_FIELDS:
    ...
    for field in AUTHORITY_FALSE_FIELDS:
        if field in index_entry and index_entry.get(field) is not False:
            blockers.append(f"{field}_not_false")
        if field in operator_boundary and operator_boundary.get(field) is not False:
            blockers.append(f"operator_boundary_{field}_not_false")
...
def _collect_template_entry_blockers(
    ...
    *,
    ...
    if template_entry.get("source_artifact_id") != ARTIFACT_ID:
        blockers.append("template_index_entry_source_artifact_id_mismatch")
    if template_entry.get("source_artifact_sha256") != _sha256_hex(artifact_bytes):
        blockers.append("template_index_entry_source_artifact_sha256_mismatch")
    if template_entry.get("template_sha256") != _sha256_hex(
        bridge_event_template_bytes
    ):
        blockers.append("template_index_entry_template_sha256_mismatch")
    for field in ("source_contract_check", "artifact_digest_check", "rebuilt_template_check"):
...
def _schema_version(artifact: Mapping[str, Any]) -> str:
    for field in ("schema_version", "template_version", "index_entry_version"):
...
def _load_json_artifact(path: Path, artifact_id: str) -> tuple[bytes, Mapping[str, Any]]:
    try:
        ...
        raise RuntimeGapIndexEntryVerificationError(
    ...
    if not isinstance(parsed, Mapping):
        raise RuntimeGapIndexEntryVerificationError(f"{artifact_id}_not_object")
    return raw, parsed
...
def _assert_no_forbidden_input(artifact_id: str, value: Mapping[str, Any]) -> None:
    ...
    if _forbidden_output_markers(serialized):
        raise RuntimeGapIndexEntryVerificationError(f"{artifact_id}_forbidden_marker")
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "runtime gap scheduler-candidate bridge-event template index entry "
