# Weekly Rollup — 2026-W17

**Campaign:** `ui_gauntlet_400h_20260413_092800`
**Generated:** 2026-04-23T11:10:35+00:00

## Trend metrics this week

- Green hours: 17.19h
- Segments committed: 3
- HOT queries: 9699 (7388 responded)
- Avg latency (responded, >100ms): 4413.0 ms
- p95 latency: 14741 ms
- Incidents: 679
- COLD health: no checks
- Feeds monotonic: yes
- Hologram honest: yes

## Incidents by category

| category | count |
|---|---|
| chat_response_failure | 554 |
| health_failure | 49 |
| chat_failure | 40 |
| cycle_crash_recovery | 12 |
| tab_switch_failure | 6 |
| auth_recovery_failure | 5 |
| backend_unhealthy | 5 |
| chat_send_failure | 4 |
| context_recycle_failure | 2 |
| auth_bootstrap_failure | 1 |
| cookie_failure | 1 |

## Safety gates (zero tolerance)

- XSS: 0
- DOM breaks: 0

## Mode-level breakdown

- HOT: 3 segments, 17.19h
- WARM: 0 segments, 0.00h
- COLD: 0 segments, 0.00h
