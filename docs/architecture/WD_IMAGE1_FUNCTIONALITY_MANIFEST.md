...
runtime controls, claim upgrades, and runtime subdivision authority false. The
next small step is a local index entry for that digest bridge-event template
that binds digest/schema refs without payload inclusion, path recording,
transport, bridge writes, claim upgrades, or runtime subdivision authority.
