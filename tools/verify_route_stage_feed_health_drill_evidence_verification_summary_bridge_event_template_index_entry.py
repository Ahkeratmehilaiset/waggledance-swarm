...
"""Verify a local route-stage feed-health bridge-template index entry."""
...
def verify_route_stage_feed_health_drill_evidence_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    """Recompute artifact checks for the route-stage bridge-template index entry."""
...
def _failure_report(reason: str) -> dict[str, Any]:
    return {
        ...
        "blockers": [
            "route_stage_feed_health_drill_evidence_verification_summary_"
            f"bridge_event_template_index_entry_verification_failed:{_safe_reason(reason)}"
        ],
        "warnings": [],
    }
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "route-stage feed-health bridge-event template index entry "
