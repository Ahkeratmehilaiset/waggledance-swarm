...
""", shell)
    ...
    assert value["config"]["sandbox_workspace_write"]["writable_roots"] == ["C:\\bridge"]
...
def finish(status="completed"):
    ...
    if status == "completed" and scenario != "missing":
...
for line in sys.stdin:
    ...
    if method == "initialize": send({"id":message["id"],"result":{}})
        ...
        owner = json.loads((root / "bridge/.wd-turn-codex-lead-1.owner.json").read_text(encoding="utf-8-sig"))
        ...
        if scenario == "wake_during" and turns == 1:
            (root / "bridge/wake_codex-lead-1").write_text("first wake")
            (root / "bridge/wake_codex-lead-1").write_text("coalesced wake")
        ...
        elif scenario not in ["steer","late_steer","steer_rejected","interrupt","interrupt_lost"] or turns > 1: finish()
...
@pytest.mark.parametrize("shell", SHELLS)
@pytest.mark.parametrize("scenario", ["complete", "steer", "late_start", "late_steer", "steer_rejected", "tool_progress", "interrupt", "interrupt_lost", "missing", "disconnect", "question", "approval", "timeout", "output", "spontaneous", "wake_during", "resume", "paused_resume", "large_resume"])
def test_owned_fake_conversation(tmp_path, shell, scenario):
    ...
    if scenario in ["resume", "paused_resume", "large_resume"]:
        ...
        (journal / "conversation.json").write_text(json.dumps({
            "schema":"wd.codex-conversation.v1", "agent":"codex-lead-1", "worktree":str(tmp_path),
            "thread_id":"thread-owned", "model":"gpt-5.6-sol", "effort":"ultra",
...
function New-WdConversationNativeProcess {{ param($CliPath,$Worktree)
    ...
    New-Object WdConversationProcess({q(python)}, [string[]]@({q(fake)}, {q(tmp_path)}, {q(scenario)}), $Worktree)
}}
$script:status=@{{active=$false;send=$false;text='';epoch='';turn='';automatic=$true}}; $script:sent=0; $script:tick=0; $script:question=''
$script:messages=[Collections.Generic.List[object]]::new()
function New-WdOperatorConversationView {{ param([switch]$Headless) return @{{}} }}
...
function Set-WdOperatorConversationStatus {{ param($View,$Text,[bool]$CanSend,[bool]$CanInterrupt,[bool]$CanToggleAutomation,[bool]$AutomationEnabled,[bool]$TurnActive,$OwnerEpoch,$ObservedTurnId)
    $script:status=@{{text=$Text;active=$TurnActive;send=$CanSend;epoch=$OwnerEpoch;turn=$ObservedTurnId;automatic=$AutomationEnabled}}
}}
...
function Get-WdOperatorConversationActions {{ param($View)
    ...
    if ($script:tick -gt 12 -and (($script:status.send -and -not $script:status.active -and ({q(scenario)} -ne 'interrupt' -or $script:sent -eq 2)) -or $script:status.text -like 'blocked_*')) {{ return @{{kind='close'}} }}
    return @()
}}
$result=Invoke-WdCodexConversationLoop -CliPath {q(python)} -Worktree {q(tmp_path)} -RuntimeRoot {q(runtime)} -SessionId test-session -Generation {'a'*40} -CompactStatePath {q(tmp_path / '.codex-audit/wd-current-state.json')} -StartupPrompt 'Initial directive' -ContinuationPrompt 'Continue directive' -Headless -RpcTimeoutSeconds 3 -TurnTimeoutSeconds {1 if scenario == 'timeout' else 5} -MaxIterations 160
$again=$null
if ({q(scenario)} -in @('missing','interrupt_lost')) {{
    $again=Invoke-WdCodexConversationLoop -CliPath {q(python)} -Worktree {q(tmp_path)} -RuntimeRoot {q(runtime)} -SessionId restarted -Generation {'a'*40} -CompactStatePath {q(tmp_path / '.codex-audit/wd-current-state.json')} -StartupPrompt 'Must not replay' -Headless -MaxIterations 1
}}
@{{owner=$result;messages=$script:messages.ToArray();status=$script:status;restart=$again}} | ConvertTo-Json -Depth 20 -Compress
"""
    result = run(script, shell, timeout=25)
    assert result.returncode == 0, result.stdout + result.stderr
    value = json.loads(result.stdout)
    calls = [json.loads(line) for line in (tmp_path / "rpc.jsonl").read_text().splitlines()]
    thread_method = "thread/resume" if scenario in ["resume", "paused_resume", "large_resume"] else "thread/start"
    ...
    if scenario in ["missing", "disconnect", "timeout", "output", "spontaneous", "interrupt_lost"]:
    ...
    if scenario == "wake_during":
        assert len([x for x in calls if x.get("method") == "turn/start"]) == 2
        assert not (runtime / "wake_codex-lead-1").exists()
    if scenario in ["timeout", "output"]:
        import ctypes
...
@pytest.mark.parametrize("shell", SHELLS)
@pytest.mark.parametrize("scenario", ["lease", "old_pending", "wrong_pins", "interactive"])
