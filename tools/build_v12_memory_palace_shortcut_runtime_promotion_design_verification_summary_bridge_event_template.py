...
"""Build a bridge-event template for Memory Palace verification summaries.

The template is deliberately inert: it validates a path-free verification
summary and returns a schema-valid bridge event that another operator or agent
can review. It does not append bridge events, enqueue schedulers, change
routes, include artifact payloads, or grant runtime-promotion authority.
"""
...
from tools.build_v12_memory_palace_shortcut_runtime_promotion_design_verification_summary import (  # noqa: E402
...
EVENT_STATUS = (
    "memory_palace_runtime_promotion_design_verification_summary_ready"
)
PROOF_ID = (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--to",
        default="operator,claude-rco-1,claude-rco-2,codex-tools-1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "Memory Palace runtime-promotion design verification summary "
...
def build_memory_palace_shortcut_runtime_promotion_design_verification_summary_bridge_event_template(
    *,
    summary: Mapping[str, Any],
    agent_id: str,
    task_id: str,
    to: str = "operator,claude-rco-1,claude-rco-2,codex-tools-1",
    severity: str = "medium",
    role: str = "lead-impl",
    run_id: str = "",
    session_id: str = "",
    ...
    design_count = _as_nonnegative_int(
        summary.get("runtime_promotion_design_count_checked")
    )
    payload = {
    ...
    event = {
        ...
        "message": (
            ...
            "change, gate skip, payload inclusion, local path recording, or "
