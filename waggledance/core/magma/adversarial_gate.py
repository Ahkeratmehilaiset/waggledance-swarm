...
def verify_adversarial_corpus_gate(
    *,
    ...
    if not isinstance(coverage, Mapping):
        reasons.append("report coverage is missing or invalid (fail-closed)")
        defect_class_counts = {}
    else:
        raw_counts = coverage.get("defect_class_counts")
        if not isinstance(raw_counts, Mapping):
            reasons.append("report coverage.defect_class_counts is missing or invalid")
            defect_class_counts = {}
        else:
            defect_class_counts = raw_counts
    for class_name in sorted(REQUIRED_DEFECT_CLASSES):
        count = defect_class_counts.get(class_name, 0)
        if (
            ...
            reasons.append(
                f"missing required defect_class coverage '{class_name}' (count {count})"
            )
