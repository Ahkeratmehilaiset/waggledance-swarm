...
from test_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry import (  # noqa: E402
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_recomputes_without_authority() -> None:
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_digest_mismatch_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_missing_record() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_nested_authority() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_deterministic_entry_drift() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    index_entry["reviewer_next_actions"] = ["manual_review_marker"]
    raw = _artifact_bytes(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_source_contract_forgery() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_rejects_ignored_path_marker() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    index_entry["created_at_utc"] = "D:/wd/path-leak/index-entry.json"
    raw = _artifact_bytes(artifacts)
    ...
    try:
        ...
        assert exc.code == f"{INDEX_ENTRY_ARTIFACT_ID}_forbidden_marker"
        assert "D:/wd/path-leak/index-entry.json" not in str(exc)
    else:
        raise AssertionError("path marker in ignored created_at_utc was accepted")


def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_missing_input_is_path_free() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(SCRIPT),
...
def test_route_stage_handoff_bundle_summary_template_index_entry_verifier_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert payload["blockers"] == [
        "route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_"
        "verification_summary_bridge_event_template_index_entry_"
        f"verification_failed:{TEMPLATE_ARTIFACT_ID}_json_error"
    ]
    combined = result.stdout + result.stderr
...
def _write_bundle(
    ...
    paths = {
        ...
        "summary": tmp_path / "bundle_verification_summary.json",
