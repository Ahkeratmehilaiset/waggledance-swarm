...
"""Render a path-free Memory Palace shortcut verifier summary.

This summary is reviewer context for the existing V12 Memory Palace shortcut
promotion-candidate verifier. It intentionally grants no runtime, promotion,
scheduler, bridge, solver, storage, network, or gate-skip authority.
"""
...
SUMMARY_VERSION = (
    "wd.v12.memory_palace_shortcut_promotion_candidate_verification_"
    "summary.v0"
)
PROOF_ID = (
    "memory_palace_shortcut_promotion_candidate_verification_summary_v0"
)
_CHECK_STATUS = frozenset({"match", "mismatch", "unknown"})
...
_NESTED_AUTHORITY_FIELDS = frozenset(
    (*_VERIFICATION_FALSE_FIELDS, "runtime_authority", "promotion_authority")
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Path to JSON emitted by the shortcut promotion verifier.",
    )
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-11T05:45:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        verification_report = _load_json_report(args.verification_json)
    ...
    if args.json:
        ...
        print(
            "Memory Palace shortcut promotion verifier summary FAILED: "
...
def build_memory_palace_shortcut_promotion_candidate_verification_summary(
    *,
    ...
    """Return path-free reviewer context for a local verifier result."""
    ...
    verifier = {
        ...
        "source_report_version_check": _check_status(
            verification_report.get("source_report_version_check")
        ),
        "source_claim_label_check": _check_status(
            verification_report.get("source_claim_label_check")
        ),
        "candidate_count_checked": _as_nonnegative_int(
            verification_report.get("candidate_count_checked")
        ),
        "promotion_observable_count_checked": _as_nonnegative_int(
            verification_report.get("promotion_observable_count_checked")
        ),
        "authority_boundary_check": _check_status(
            verification_report.get("authority_boundary_check")
        ),
        "guardrail_check": _check_status(verification_report.get("guardrail_check")),
        "candidate_action_boundary_check": _check_status(
            verification_report.get("candidate_action_boundary_check")
        ),
        ...
        is True,
        "manual_review_required": verification_report.get("manual_review_required")
        is True,
    ...
    summary = {
        ...
        "memory_palace_shortcut_promotion_candidate_verification": verifier,
        "operator_boundary": {
        ...
        "reviewer_next_actions": [
            "review_memory_palace_shortcut_promotion_candidate_verification_summary",
            "compare_verification_summary_to_local_report",
            "request_runtime_promotion_design_separately_if_wanted",
        ],
        "template_only": True,
...
def render_memory_palace_shortcut_promotion_candidate_verification_summary_markdown(
    summary: Mapping[str, Any],
) -> str:
    verification = _mapping(
        summary.get("memory_palace_shortcut_promotion_candidate_verification")
    )
    boundary = _mapping(summary.get("operator_boundary"))
    lines = [
        "# Memory Palace Shortcut Promotion Verifier Summary",
        "",
        ...
        "- Source report version check: "
        f"`{verification.get('source_report_version_check')}`",
        "- Source claim label check: "
        f"`{verification.get('source_claim_label_check')}`",
        "- Candidate count checked: "
        f"`{verification.get('candidate_count_checked')}`",
        "- Promotion-observable count checked: "
        f"`{verification.get('promotion_observable_count_checked')}`",
        "- Authority boundary check: "
        f"`{verification.get('authority_boundary_check')}`",
        f"- Guardrail check: `{verification.get('guardrail_check')}`",
        "- Candidate action boundary check: "
        f"`{verification.get('candidate_action_boundary_check')}`",
    ...
    lines.extend(
        [
            ...
            "include payloads, record local paths, enqueue scheduler work, dispatch "
            "solvers, skip gates, change runtime routes, or grant runtime authority.",
            "",
        ]
    )
    markdown = "\n".join(lines)
...
def _boundary_blockers(report: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    for field in _VERIFICATION_TRUE_FIELDS:
        if report.get(field) is not True:
            blockers.append(f"verification_report_{field}_not_true")
...
def _input_key_boundary_blockers(
    ...
    if parent_path and key in _NESTED_AUTHORITY_FIELDS and value is not False:
...
def _verification_report_contract_blockers(report: Mapping[str, Any]) -> list[str]:
    ...
    for field in (
        ...
        "guardrail_check",
        "candidate_action_boundary_check",
    ):
        if report.get(field) != "match":
            blockers.append(f"verification_report_{field}_not_match")
    candidate_count = _as_nonnegative_int(report.get("candidate_count_checked"))
    observable_count = _as_nonnegative_int(
        report.get("promotion_observable_count_checked")
    )
    if candidate_count < 1:
        blockers.append("verification_report_candidate_count_checked_empty")
    if observable_count < 1:
        blockers.append(
            "verification_report_promotion_observable_count_checked_empty"
        )
    if observable_count > candidate_count:
        blockers.append(
            "verification_report_promotion_observable_count_exceeds_candidates"
        )
    return sorted(set(blockers))
...
def _safe_input_key(value: Any) -> str:
    if not isinstance(value, str):
        return "invalid_key"
    return value if _SAFE_TOKEN_RE.fullmatch(value) else "invalid_key"
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "memory palace shortcut promotion verification summary contains "
            "forbidden markers"
        )
