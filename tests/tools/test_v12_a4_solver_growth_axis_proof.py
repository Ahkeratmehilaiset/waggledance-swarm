...
def test_build_a4_solver_growth_axis_proof_writes_receipt_bundle(
    ...
    assert report["dispatch"]["dispatch_success_count"] == 18
    assert report["dispatch"]["dispatch_case_count"] == 18
...
def test_render_markdown_carries_scope_caveat(tmp_path: Path) -> None:
    ...
    assert "dispatch successes | 18/18" in markdown
...
def test_cli_json_reports_a4_axis(tmp_path: Path) -> None:
    ...
    assert payload["dispatch"]["dispatch_success_count"] == 18
