...
def classify_request_host(
    ...
    *,
    ...
    Returns a refusal code (HOST_REFUSED_LOCAL / HOST_REFUSED_PRIVATE) or None if
    the host is allowed. Refuses loopback / link-local / unspecified / private IPs
    and local-use DNS names (single-label or private/local suffix) unless the
    exact normalized host is in ``allowed_private_hosts``. A normal dotted public
    FQDN and public IPs are allowed.
    """
