...
Advances the WD Image #1 *hexagonal upgrades* pillar. PR #1278 added the
path-free, measurement-only ``run_hex_upgrade_cross_consistency_digest`` (and wired it
...
bridge, transports, writes files, calls providers, mutates runtime, grants runtime
subdivision authority, or upgrades any claim. The emitted template hardcodes every
approval/transport/authority axis False. Content-safe by construction: the
cross-consistency section emits ONLY derived booleans (plus fixed version/schema
strings + a sha256 digest ref) via a closed ``_TEMPLATE_SAFE_KEYS`` allowlist that
raises on any non-allowlisted key; ``path_free_verified`` is DERIVED by scanning the
rendered output (never hardcoded); ``claim_safe`` is hardcoded False and a digest that
self-declares ``claim_safe`` True is refused. The rendered event validates against the
bridge-event schema.

Exact validation commands::

    python tools/build_hex_upgrade_cross_consistency_digest_bridge_event_template.py \\
        --agent codex-lead-1 --task-id demo --json
    python -m pytest \\
        tests/test_build_hex_upgrade_cross_consistency_digest_bridge_event_template.py -q
...
# The digest's derived-boolean verdicts the template faithfully renders. These are
# already re-derived upstream from the reviewer summary, shadow-only invariant, and
# chain final summary; this tool NEVER recomputes a new "clean" that could exceed the
# digest.
...
def build_hex_upgrade_cross_consistency_digest_bridge_event_template(
    *,
    ...
    Fail-closed: a non-Mapping / wrong-version / path-tainted / self-claim_safe /
    unsafe digest or unsafe bridge inputs yield an ok=False error report (never a
    partial or a side effect).
    """
    ...
    payload = {
        ...
        "runtime_subdivision_authority_added": False,
...
def _load_digest(path: Path | None) -> Mapping[str, Any]:
    if path is None:
        ...
        raise SafeInputError("hex_upgrade_proof_not_found")
    try:
        raw = path.read_bytes()
