...
| Hexagonal upgrades | Partial, with in-memory proof, runtime-boundary smoke, read-only topology boundary `/metrics` gauges, and a shadow subdivision replay artifact | `waggledance/core/hex_topology/subdivision_operator.py`, `ring_messaging.py`, `parent_child_relations.py`, `waggledance/bootstrap/container.py`, `hex_topology_registry.py`, `hex_neighbor_assist.py`, `waggledance/adapters/http/routes/metrics.py`, `tools/hex_shadow_subdivision_replay.py` | Add a local verifier for the shadow subdivision replay artifact without activating runtime subdivision authority. |
...
contents or message payloads, and does not activate subdivision authority.
