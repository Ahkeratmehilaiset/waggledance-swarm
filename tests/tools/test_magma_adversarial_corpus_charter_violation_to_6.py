...
"""charter-violation tail-floor expansion provenance fixture tests."""
...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_11_charter_violation_to_6.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR
    / "v0_expansion_2026_06_11_charter_violation_to_6_expectations.json"
)
LABEL = "charter_violation_to_6_2026_06_11"
...
def test_expansion_is_exactly_one_charter_violation_case():
    cases = _read_json(EXPANSION)["cases"]
    assert len(cases) == 1
    case = cases[0]
    assert case["case_id"] == "case:adv:charter_violation:006"
    assert case["defect_type"] == "charter_violation"
    assert {"operator_identity", "approval_spoof", "bridge_identity"} <= set(
        case["tags"]
    )
...
def test_strict_v0_charter_violation_coverage_raised_to_six():
...
def test_every_expansion_case_has_paired_refuse_expectation():
    ...
    for exp in exps:
        ...
        assert "charter:operator_gate_required" in exp["expected_reason_codes"]
        assert "risk:external_effect" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    family_canaries = [
        c["privacy_canary"]
        for c in all_cases
        if c["defect_type"] == "charter_violation"
    ]
    assert len(family_canaries) == len(set(family_canaries))
    case = _read_json(EXPANSION)["cases"][0]
    assert case["privacy_canary"] == "canary_charter_006_DO_NOT_LEAK"
...
def test_held_out_split_unchanged_and_valid():
    ...
    assert "case:adv:charter_violation:006" not in held_out
