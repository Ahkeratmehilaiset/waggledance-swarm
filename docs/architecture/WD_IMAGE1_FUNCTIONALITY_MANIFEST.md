...
`local_paths_recorded=false`, `transport_added=false`,
`approval_granted=false`, and `release_decision_made=false`. The next small
step is adding a local operator decision-reference validator for reviewer
handoff bundles without granting approval.
