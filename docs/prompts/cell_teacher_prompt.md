...
7. **Declare uncertainty.** Every proposal carries `uncertainty_declaration`. Reviewers trust you more when you're honest about what you don't know.
...
11. No secrets / no local absolute paths in the proposal
12. No hidden LLM dependency (must be declared)

Verdicts:

- `REJECT_SCHEMA` — proposal doesn't parse against the schema
...
- `REJECT_LOW_VALUE` — passes all checks but coverage lift below threshold
- `ACCEPT_SHADOW_ONLY` — worth deploying in shadow mode, affects no user response
- `ACCEPT_CANDIDATE` — passes; proceeds to standard promotion flow (shadow → canary → promote)
