...
"""Build a local index entry for Memory Palace visualization bridge templates.

The index entry records digest, size, schema, and contract checks for a
visualization-export verification summary plus its inert bridge-event template.
It is local and path-free: it does not append bridge events, enqueue scheduler
work, dispatch solvers, change routes, include artifact payloads, or grant
runtime authority.
"""
...
from tools.build_memory_palace_visualization_export_verification_summary_bridge_event_template import (  # noqa: E402
...
SUMMARY_ARTIFACT_ID = "memory_palace_visualization_export_verification_summary"
TEMPLATE_ARTIFACT_ID = (
    "memory_palace_visualization_export_verification_summary_bridge_event_template"
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = build_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry(
    ...
    if args.json or report["ok"]:
        ...
        print(
            "Memory Palace visualization bridge-template index entry FAILED: "
...
def build_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    summary_payload = _mapping(
        payload.get("memory_palace_visualization_export_verification_summary")
    )
    artifacts = (
        _artifact_record(
            artifact_id=SUMMARY_ARTIFACT_ID,
            role="verified_memory_palace_visualization_export_context",
    ...
    entry = {
        ...
        "template_index_entry": {
            ...
            "node_count_checked": verification_summary.get("counts", {}).get(
                "node_count_checked",
            ),
            "edge_count_checked": verification_summary.get("counts", {}).get(
                "edge_count_checked",
            ),
            "shortcut_edge_count_checked": verification_summary.get("counts", {}).get(
        ...
        "reviewer_next_actions": [
            "review_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry",
...
def _rebuilt_template(
    ...
    return build_memory_palace_visualization_export_verification_summary_bridge_event_template(
        summary=summary,
...
def _assert_template_contract(
    ...
    _expect_false_fields(
        template_report,
        (*AUTHORITY_FALSE_FIELDS, *EXTRA_TEMPLATE_FALSE_FIELDS),
    ...
    if payload.get("manual_review_required") is not True:
        raise IndexEntryError("bridge_event_template_payload_manual_review_not_true")
    _expect_false_fields(
        payload,
        (*AUTHORITY_FALSE_FIELDS, *EXTRA_TEMPLATE_FALSE_FIELDS),
        "bridge_event_template_payload",
    )
    ...
    if verification.get("counts") != summary.get("counts"):
        raise IndexEntryError("bridge_event_template_summary_counts_mismatch")
...
def _expect_false_fields(payload: Mapping[str, Any], fields: Sequence[str], prefix: str) -> None:
    for field in fields:
