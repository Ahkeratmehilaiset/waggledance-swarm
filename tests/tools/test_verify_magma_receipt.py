...
@pytest.mark.parametrize(
    "extension",
    [
        {"digest_semantics": {"generic_contract": "v1"}},
        {"route_stage_trace": [{"stage": "generic_workflow"}]},
        {"route_stage_trace_digest": "sha256:" + "a" * 64},
        {
            "route_stage_trace": [{"stage": "generic_workflow"}],
...
def test_generic_payload_chat_named_extensions_remain_generic(
...
@pytest.mark.parametrize("survivor", ["digest_semantics", "bound_route_trace"])
def test_cli_rejects_chat_downgrade_with_only_one_value_bound_marker_cluster(
    tmp_path: Path, survivor: str
) -> None:
    marker = "SECRET_MULTI_MARKER_STRIP_RAW_PAYLOAD"
    manifest = _write_chat_chain(tmp_path / "chat-chain")
    manifest_json = json.loads(manifest.read_text(encoding="utf-8"))
    manifest_json["chain_id"] = "magma:fixture:attacker_reclassified"
    entry = manifest_json["entries"][0]
    ...
    payload["served_path"] = "Attacker.handle"
    payload["raw_private_extra"] = marker
    if survivor == "digest_semantics":
        payload.pop("route_stage_trace")
        payload.pop("route_stage_trace_count")
        payload.pop("route_stage_trace_digest")
    else:
        payload.pop("digest_semantics")
    payload_digest = sha256_digest(payload)
    evaluation["case_id"] = "case:attacker:fixture"
    evaluation["target_digest"] = payload_digest
    evaluation["verifier_path"] = ["attacker_verifier"]
...
@pytest.mark.parametrize("entries", [None, 7, True])
def test_non_array_manifest_entries_fail_closed_without_traceback(
    ...
    _write_json(
        manifest,
        {"chain_id": "magma:fixture:malformed_entries", "entries": entries},
    )
    ...
    assert "manifest: entries must be a non-empty array" in report["errors"]
    assert result.returncode == 1
    assert "Traceback" not in result.stdout + result.stderr
    cli_report = json.loads(result.stdout)
    assert cli_report["ok"] is False
    assert cli_report["receipt_count"] == 0
