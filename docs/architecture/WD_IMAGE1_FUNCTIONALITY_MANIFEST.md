...
summary, verifier-summary bridge-event template, and local index entry for that
template continue that same review chain without appending bridge events,
including payloads or local paths, upgrading claims, or granting runtime
subdivision authority. The next small step is a local verifier for that
verifier-summary bridge-event template index entry.
