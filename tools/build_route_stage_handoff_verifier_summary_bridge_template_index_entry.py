...
"""Build a route-stage handoff-bundle template index-entry verifier verification-summary bridge-event-template index entry."""
...
from tools.build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry import (  # noqa: E402
    ...
    SUMMARY_ARTIFACT_ID as SOURCE_VERIFICATION_SUMMARY_ARTIFACT_ID,
    TEMPLATE_ARTIFACT_ID as SOURCE_BRIDGE_EVENT_TEMPLATE_ARTIFACT_ID,
)
...
from tools.build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template import (  # noqa: E402
...
SUMMARY_ARTIFACT_ID = (
    "route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_"
    "bridge_event_template_index_entry_verification_summary_"
    "bridge_event_template_index_entry_verification_summary"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-06T06:05:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
        ...
        report = _failure_report(
            ...
            "bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_index_entry_invalid"
        )
    ...
    if args.json or report["ok"]:
        ...
        print(
            "route-stage handoff-bundle template index-entry verification-summary bridge-event-template index entry "
            "FAILED: "
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    entry = {
        ...
        "reviewer_next_actions": [
            "review_route_stage_verifier_summary_bridge_event_template_index_entry",
...
def _rebuilt_summary_bridge_template(
    ...
    rebuilt = build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template(
        summary=summary,
...
def _assert_template_report_contract(
    ...
    *,
    ...
    if payload.get("summary_proof_id") != _safe_ref_or_invalid(
        index_entry_verification_summary.get("proof_id")
    ):
        raise TemplateIndexEntryError(
    ...
    payload_verification = _mapping(payload.get(ROUTE_STAGE_BUNDLE_TEMPLATE_INDEX_ENTRY_VERIFICATION_KEY))
    summary_verification = _mapping(
        index_entry_verification_summary.get(ROUTE_STAGE_BUNDLE_TEMPLATE_INDEX_ENTRY_VERIFICATION_KEY)
    )
    for field in (
...
def _safe_reason(value: Any) -> str:
    return (
        value
        if isinstance(value, str)
        and _SAFE_REASON_RE.fullmatch(value)
        and not _forbidden_output_markers(value)
        else "unsafe_reason_redacted"
    )
