...
- **PR2** (Codex, M0 reference shims): `.orchestrator/bridge_classify.py` + `.orchestrator/eig2_bridge_projection.py` + `.orchestrator/no_human_prompt_lint.py` + `.orchestrator/contracts/eig2_bridge_event.schema.json` + `.orchestrator/contracts/eig2_config.schema.json` + `configs/explosive_intelligence_growth_v2.yaml` (`enabled:false`, `implemented:true` as alpha metadata per ADR-012 semantic note) + contract test skeletons. **MERGED** to main via PR #268 commit `ba27ae1` at 2026-05-11T20:25:15Z. (Note: `configs/eig2_self_modification_denylist.yaml` moved to Claude ownership per `docs/eig2/spikes/M0-reality-check.md` §12; created on prep branch in commit `31efeca`. ADR drafts R10/R11/R14/R18 deferred to joint PR3 per ownership-split §3.b.)
