...
def test_hex_mesh_route_stage_runtime_metrics_smoke_reports_counters() -> None:
    ...
    assert (
        smoke[
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_supported"
        ]
        is True
    )
    assert (
        smoke[
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verifier_supported"
        ]
        is True
    )
    assert (
        smoke[
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_supported"
        ]
        is True
    )
...
def test_manifest_embeds_hexagonal_upgrade_proof_without_upgrading_claim() -> None:
    ...
    assert "bind that bridge-event template digest" in capability["safe_statement"]
    # next_smallest_pr advanced beyond the now-wired index entry to the next honest
    # step: a local verifier for that index entry.
...
def test_manifest_embeds_hex_entry_proof_without_upgrading_claim() -> None:
    ...
    assert (
        capability["proof"]["route_stage_runtime_metrics_smoke"][
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_supported"
        ]
        is True
    )
    assert (
        capability["proof"]["route_stage_runtime_metrics_smoke"][
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verifier_supported"
        ]
        is True
    )
    assert (
    ...
    assert (
        capability["proof"]["route_stage_runtime_metrics_smoke"][
            "latency_feed_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_supported"
        ]
        is True
    )
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_smoke = (
        verifier_smoke[
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_verification_summary_template_smoke = (
        verifier_smoke[
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_verification_summary_template_index_entry_smoke = (
        verifier_smoke[
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_verification_summary_template_index_entry_verification_smoke = (
        verifier_smoke[
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_verification_summary_template_index_entry_verification_summary_smoke = (
        verifier_smoke[
    ...
    reviewer_handoff_bundle_verification_summary_template_index_entry_verification_summary_template_index_entry_verification_summary_template_smoke = (
        verifier_smoke[
...
def test_chat_served_claim_window_evidence_inputs_drive_safe_aggregate(tmp_path) -> None:
    ...
    aggregate = build_chat_served_claim_window_aggregate(
        ...
        window_id=window_id,
        enabled_samples=[
            new_enabled_state_sample(window_id=window_id, enabled=True, ts_utc=ts_utc),
        ],
        clean_shutdown_marker_path=str(marker_path),
        served_point_observations=[
            new_served_point_observation(point=point, wired=True, ts_utc=ts_utc)
...
def test_chat_served_claim_window_evidence_inputs_fail_closed(tmp_path) -> None:
    ...
    missing_inputs = build_chat_served_claim_window_aggregate(
        force=True,
        ledger_path=str(ledger_path),
        anchor_store_path=str(tmp_path / "anchors.jsonl"),
    )
    assert missing_inputs["claim_window_eligible"] is False
    assert missing_inputs["input_evidence_derived"] is False
    assert missing_inputs["reason"] == "missing_evidence_inputs"

    self_anchored = build_chat_served_claim_window_aggregate(
        force=True,
        ledger_path=str(ledger_path),
        ...
        clean_shutdown_marker_path=str(tmp_path / "clean.json"),
        served_point_observations=[
            new_served_point_observation(point=point, wired=True, ts_utc=ts_utc)
            for point in sorted(REQUIRED_CHAT_SERVED_POINTS)
        ],
    )
    assert self_anchored["claim_window_eligible"] is False
    assert self_anchored["input_evidence_derived"] is True
    assert self_anchored["reason"] == "head_anchor_not_independent"
...
def test_manifest_stores_hex_xcons_template_index_entry_summary_content_safe() -> None:
    ...
    assert summary["template_only"] is True
    assert summary["manual_review_required"] is True
    assert summary["artifact_count_one"] is True
    assert summary["bridge_event_schema_validated"] is True
    assert summary["template_report_validator_pass"] is True
    assert summary["all_artifact_digests_recorded"] is True
    assert summary["cross_consistency_digest_ref_recorded"] is True
    assert summary["no_runtime_authority_granted"] is True
    assert summary["no_runtime_subdivision_authority_granted"] is True
    assert summary["no_direct_bridge_write"] is True
    assert summary["no_bridge_event_written"] is True
...
def test_manifest_hex_xcons_template_index_entry_measurement_only() -> None:
    ...
    assert isinstance(
        proof.get("cross_consistency_digest_bridge_event_template_index_entry"), dict
    )
    assert proof.get("ok") is True
    import inspect
    from tools import wd_image1_capability_manifest as mod
    ...
    src = inspect.getsource(mod._capabilities)
    ok_assign = src.split('hex_upgrade_proof["ok"] = bool(', 1)[1].split(")", 1)[0]
    assert "cross_consistency_digest_bridge_event_template_index_entry" not in ok_assign
    tpl_idx = src.index(
        'hex_upgrade_proof["cross_consistency_digest_bridge_event_template"]'
    )
    idx_entry_idx = src.index(
        'hex_upgrade_proof[\n        "cross_consistency_digest_bridge_event_template_index_entry"'
    )
    assert idx_entry_idx > tpl_idx
...
def test_manifest_hex_xcons_template_index_entry_cannot_flip_ok(monkeypatch) -> None:
    ...
    forged = {
        ...
        "artifact_count_one": False,
        "bridge_event_schema_validated": False,
        "template_report_validator_pass": False,
        "all_artifact_digests_recorded": False,
        "cross_consistency_digest_ref_recorded": False,
        "no_runtime_authority_granted": False,
        "no_runtime_subdivision_authority_granted": False,
        "no_direct_bridge_write": False,
        "no_bridge_event_written": False,
