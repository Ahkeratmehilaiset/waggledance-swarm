...
"""Render a sanitized MAGMA operator decision-reference review bundle verification summary."""
...
SUMMARY_VERSION = "magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_summary.v1"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--verification-json",
        dest="review_bundle_verification_json",
        required=True,
        type=Path,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        verification_report = _load_json_report(
            args.review_bundle_verification_json
        )
        ...
        try:
            ...
            summary = _failure_summary(
                "operator_decision_reference_review_bundle_verification_summary_invalid"
            )

    if args.json:
        print(json.dumps(summary, indent=2, sort_keys=True))
    elif summary["ok"]:
        print(render_operator_decision_reference_review_bundle_verification_summary_markdown(summary))
    else:
        print(
...
def build_magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_summary(
    *,
    ...
    """Return path-free reviewer context for a local review-bundle verification."""
    ...
    reference = _mapping(verification_report.get("operator_decision_reference"))
    ...
    summary = {
        ...
        "operator_decision_reference_review_bundle_verification": {
            ...
            "rebuilt_index_check": _check_status(
                verification_report.get("rebuilt_index_check")
            ),
        ...
        "reviewer_next_actions": [
            "review_operator_decision_reference_review_bundle_verification",
            "compare_review_bundle_verification_to_local_artifacts",
...
def render_operator_decision_reference_review_bundle_verification_summary_markdown(
    ...
    lines = [
        "# MAGMA Operator Decision-Reference Review Bundle Verification Summary",
        "",
        ...
        f"- Rebuilt index check: `{verification.get('rebuilt_index_check')}`",
