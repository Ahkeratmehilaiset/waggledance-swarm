...
template, and local verifier for that index entry continue that same review
chain without appending bridge events, including payloads or local paths,
upgrading claims, or granting runtime subdivision authority. The next small
step is a path-free reviewer summary renderer for that verifier result.
