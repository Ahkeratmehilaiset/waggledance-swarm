...
try:
    ms_ff = MemoryStore(path=td_ff)
    eval_emb = EvalEmbeddingEngine()

    if not eval_emb.available:
        WARN("EvalEmbed (all-minilm) not available -- skipping FiFastStore tests")
    else:
        from core.fast_memory import FiFastStore

        ffs = FiFastStore(ms_ff, eval_emb)

        # 12a. fi_fast collection created
        if ffs.fi_fast_count == 0:
            OK("fi_fast collection created (empty)")
        else:
            WARN(f"fi_fast collection has {ffs.fi_fast_count} items (expected 0)")

        # 12b. store single item
        test_meta_ff = {
            "agent_id": "test", "source_type": "test",
            "confidence": 0.9, "validated": True,
            "text_fi": "Varroa on mehil\u00e4isen loinen",
            "text_en": "Varroa is a bee parasite",
        }
        ffs.store("ff_001", "Varroa on mehil\u00e4isen loinen", test_meta_ff)
        if ffs.fi_fast_count >= 1:
            OK(f"store: fi_fast count={ffs.fi_fast_count}")
        else:
            FAIL(f"store failed: fi_fast count={ffs.fi_fast_count}")

        # 12c. search returns results
        ff_results = ffs.search("varroa loinen", top_k=3)
        if ff_results and len(ff_results) > 0:
            best_ff = ff_results[0]
            OK(f"search found result: score={best_ff.score:.2f}")
        else:
            FAIL("search returned no results")

        # 12d. search returns MemoryMatch
        if ff_results and isinstance(ff_results[0], MemoryMatch):
            OK("search returns MemoryMatch instances")
        else:
            FAIL("search should return MemoryMatch")

        # 12e. store_batch
        batch_ids = ["ff_002", "ff_003"]
        batch_texts = [
            "Hunajan linkous kes\u00e4kuussa",
            "Oksaalihappo varroa-hoitoon",
        ]
        batch_metas = [
            {"agent_id": "test", "confidence": 0.8, "validated": True,
             "text_fi": batch_texts[0], "text_en": "Honey extraction in June"},
            {"agent_id": "test", "confidence": 0.85, "validated": True,
             "text_fi": batch_texts[1], "text_en": "Oxalic acid for varroa treatment"},
        ]
        n_stored = ffs.store_batch(batch_ids, batch_texts, batch_metas)
        if n_stored >= 2:
            OK(f"store_batch stored {n_stored} items, total={ffs.fi_fast_count}")
        elif n_stored >= 1:
            WARN(f"store_batch stored {n_stored}/2 items")
        else:
            FAIL(f"store_batch stored 0 items")

        # 12f. seasonal_prewarm loads config
        ffs.seasonal_prewarm()
        if ffs._seasonal_keywords:
            OK(f"seasonal_prewarm loaded {len(ffs._seasonal_keywords)} months")
        else:
            WARN("seasonal_prewarm: no seasonal_focus.yaml found (non-critical)")

        # 12g. seasonal boost in search
        # Store a seasonal item
        ffs.store("ff_season", "Varroa-hoito muurahaishapolla elo-syyskuussa",
                  {"agent_id": "test", "confidence": 0.9, "validated": True,
                   "text_fi": "Varroa-hoito muurahaishapolla elo-syyskuussa",
                   "text_en": "Varroa treatment with formic acid Aug-Sep"})
        boosted = ffs.search("varroa hoito", top_k=5,
                             seasonal_boost=["muurahaishappo", "varroa"])
        if boosted:
            OK(f"search with seasonal_boost: top score={boosted[0].score:.2f}")
        else:
            FAIL("search with seasonal_boost returned no results")

        # 12h. stats
        ff_stats = ffs.stats
        if "fi_fast_count" in ff_stats:
            OK(f"stats: fi_fast_count={ff_stats['fi_fast_count']}")
        else:
            FAIL(f"stats missing fi_fast_count: {ff_stats}")

        # 12i. populate_from_memory
        # Store a fact in EN memory first
        en_emb = EmbeddingEngine()
        if en_emb.available:
            en_vec = en_emb.embed_document("Oxalic acid kills varroa mites")
            if en_vec:
                ms_ff.store("pop_001", "Oksaalihappo tappaa varroapunkkeja | Oxalic acid kills varroa mites",
                            en_vec, {"text_fi": "Oksaalihappo tappaa varroapunkkeja",
                                     "text_en": "Oxalic acid kills varroa mites",
                                     "confidence": 0.9, "validated": True})
                count_before = ffs.fi_fast_count
                added = ffs.populate_from_memory(ms_ff)
                if added >= 1:
                    OK(f"populate_from_memory added {added} items (total={ffs.fi_fast_count})")
                else:
                    WARN(f"populate_from_memory added {added} (may already exist)")
            else:
                WARN("Could not embed EN text for populate test")
        else:
            WARN("EmbeddingEngine not available for populate test")
...
try:
    c_ff = Consciousness(db_path=td_ffc)

    # 13a. fi_fast attribute exists
    if hasattr(c_ff, 'fi_fast'):
        if c_ff.fi_fast is not None:
            OK(f"Consciousness.fi_fast initialized (count={c_ff.fi_fast.fi_fast_count})")
        else:
            WARN("Consciousness.fi_fast is None (eval_embed may be unavailable)")
    else:
        FAIL("Consciousness missing fi_fast attribute")

    # 13b. fi_fast in stats
    st = c_ff.stats
    if "fi_fast" in st:
        OK(f"fi_fast in consciousness.stats: {st['fi_fast']}")
    else:
        FAIL("fi_fast missing from consciousness.stats")

    # 13c. before_llm routes through fi_fast when data present
    if c_ff.fi_fast and c_ff.eval_embed.available and c_ff.embed.available:
        # Store a validated fact
        c_ff.learn("Varroa-kynnys on 3 punkkia sadassa mehil\u00e4isess\u00e4",
                   agent_id="test", source_type="test",
                   confidence=0.95, validated=True, immediate=True)
        # Check fi_fast got populated
        if c_ff.fi_fast.fi_fast_count >= 1:
            OK(f"learn() populates fi_fast: count={c_ff.fi_fast.fi_fast_count}")
        else:
            WARN("learn() did not populate fi_fast (embedding may have failed)")
    else:
        WARN("Skipping fi_fast learn test (embedding unavailable)")
...
try:
    c_cfg = Consciousness(db_path=td_cfg)
    al_cfg2 = c_cfg._load_advanced_learning_config()
    if al_cfg2.get("fi_fast_enabled") is True:
        OK("Config loaded fi_fast_enabled=true")
    elif "fi_fast_enabled" in al_cfg2:
        WARN(f"fi_fast_enabled={al_cfg2.get('fi_fast_enabled')}")
    else:
        WARN("fi_fast_enabled not in settings.yaml (uses default=true)")
finally:
...
if sf_path.exists():
    try:
        ...
        with open(sf_path, encoding="utf-8") as _f_sf:
            sf_data = _yaml_sf.safe_load(_f_sf) or {}
        sf_focus = sf_data.get("seasonal_focus", {})
        if len(sf_focus) >= 12:
            OK(f"seasonal_focus.yaml: {len(sf_focus)} months defined")
        else:
            WARN(f"seasonal_focus.yaml: only {len(sf_focus)} months (expected 12)")
    except Exception as e_sf:
        FAIL(f"seasonal_focus.yaml parse error: {e_sf}")
else:
...
# FiFastStore reuses existing all-minilm (already loaded for eval)
...
OK("FiFastStore: reuses all-minilm (already loaded for eval, no new GPU)")
...
if results["fail"] == 0:
    ...
    print(f"  Bilingual Index + Hot Cache + fi_fast + Fact Enrichment")
    print(f"  {'='*50}{W}")
else:
    print(f"\n  {R}{'='*50}")
