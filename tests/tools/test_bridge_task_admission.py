...
def test_cached_keep_cannot_override_a_current_quota_failure():
    request = base_request(failure={"kind": "quota"})
    result = admit(request, judgments={decision_key(request): KEEP})
    assert result["verdict"] == PARK
    assert result["judgment_requested"] is False
...
def observed_from(request):
    return {"hold": request["hold"], "cancelled": request["cancelled"],
            ...
            "epochs": request["epochs"], "current_profile": request["current_profile"],
...
@pytest.mark.parametrize('change', [
    {'failure': {'kind': 'quota'}},
    {'failure': {'kind': 'auth'}},
    {'current_profile': {'profile_id': 'other', 'authorized': False}},
    {'ttl_expired': True},
    {'current_profile': {}},
])
def test_new_failure_or_authorization_loss_blocks_revalidation(change):
    request = base_request()
    observed = deepcopy(observed_from(request))
    observed.update(change)
    assert revalidate(admit(request), observed)['revalidated'] is False
