...
import tools.verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry as source_verifier
...
SCRIPT = (
    ...
    / "tools"
    / (
        "verify_hex_upgrade_cross_consistency_digest_bridge_event_template_"
...
FORBIDDEN_INDEX_ENTRY_PATH = "".join(
    (FORBIDDEN_PATH_PREFIX, "/", "summary-template-index-entry.json")
)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_recomputes_without_authority() -> None:
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_digest_mismatch_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_missing_record() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_nested_authority() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_deterministic_entry_drift() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_source_contract_forgery() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    tampered_summary = deepcopy(artifacts["summary"])
    tampered_summary[HEX_XCONS_VERIFICATION_KEY][
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_missing_input_is_path_free() -> None:
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_hex_xcons_verifier_summary_template_index_entry_verifier_rejects_path_markers_without_leak(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def _source_verification_report() -> dict:
    template_report = _source_template_report()
    template_bytes = _json_bytes(template_report)
    return source_verifier.verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry(
        index_entry=_source_index_entry(template_report),
...
def _source_template_report() -> dict:
...
def _source_index_entry(template_report: dict) -> dict:
...
def _assert_path_free_output(
    ...
    assert "summary-template-index-entry.json" not in combined
