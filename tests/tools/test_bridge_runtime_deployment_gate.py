...
def _assert_no_validated_inventory(report: dict[str, object]) -> None:
    observations = report["observations"]
    assert "validated_inventory" not in observations
    live = observations.get("live_evidence")
    if isinstance(live, dict):
        assert "validated_inventory" not in live


def _validated_inventories(
    fixture: GateFixture,
) -> tuple[dict[str, set[tuple[object, ...]]], dict[str, tuple[object, ...]]]:
    blockers: list[dict[str, str]] = []
    sample = fixture.evidence["samples"][1]
    processes = gate._audit_process_inventory(
        sample,
        manifest=fixture.manifest,
        label="B",
        blockers=blockers,
    )
    tasks = gate._audit_task_inventory(
        sample,
        manifest=fixture.manifest,
        label="B",
        blockers=blockers,
...
def test_exact_fixture_match_is_never_authoritative(tmp_path: Path) -> None:
    ...
    projection = report["observations"]["validated_inventory"]
    assert projection == live["validated_inventory"]
    ...
    assert projection["schema"] == gate.VALIDATED_INVENTORY_SCHEMA
    assert projection["exact_source_head"] == fixture.commit
    assert projection["scope_proof"] == {
        "complete": False,
    ...
    assert projection["inventory_sha256"] == gate.canonical_digest(digest_input)

    serialized = json.dumps(projection, sort_keys=True)
    for forbidden in (
        str(fixture.repo.resolve()),
        str(fixture.runtime_root.resolve()),
        str(fixture.interpreter.resolve()),
        COLLECTOR_SID,
        fixture.manifest["actions"]["scheduled_tasks"][0]["task_name"],
    ):
        assert forbidden not in serialized
    for blob in projection["runtime_blobs"]:
        assert not Path(blob["source_path"]).is_absolute()


def test_validated_inventory_projection_is_deterministic_sorted_and_hash_only(
    tmp_path: Path,
) -> None:
    fixture = _fixture(tmp_path)
    manifest_a = deepcopy(fixture.manifest)
    manifest_a["actions"]["processes"][0]["required_count"] = 2
    manifest_b = deepcopy(manifest_a)
    manifest_b["toolchain"].reverse()
    manifest_b["runtime_blobs"].reverse()

    process = manifest_a["actions"]["processes"][0]
    executable_sha = next(
        item["sha256"]
        for item in manifest_a["toolchain"]
        if item["id"] == process["executable_toolchain_id"]
    )
    hidden_process_marker = r"C:\secret\event-wal-payload-command-line"
    processes = {
        process["id"]: {
            (
                9002,
    ...
    task = manifest_a["actions"]["scheduled_tasks"][0]
    hidden_task_marker = "<Task>event WAL query payload</Task>"
    tasks = {
        task["id"]: (
            (r"\secret-task-path", hidden_task_marker),
            True,
            COLLECTOR_SID,
            hidden_task_marker,
            r"C:\secret\working-directory",
            task["action_sha256"],
            task["definition_sha256"],
        )
    }
    kwargs = {
        "exact_source_head": fixture.commit,
        "provenance_head": fixture.commit,
        "sample_a": fixture.evidence["samples"][0],
        "sample_b": fixture.evidence["samples"][1],
        "process_inventory": processes,
        "task_inventory": tasks,
    }

    projection_a = gate._validated_inventory_projection(
        manifest=manifest_a, **kwargs
    )
    projection_b = gate._validated_inventory_projection(
        manifest=manifest_b, **kwargs
    )

    assert projection_a == projection_b
    assert [item["pid"] for item in projection_a["processes"]] == [9001, 9002]
    assert [item["id"] for item in projection_a["runtime_blobs"]] == [
        "entry",
        "writer",
    ]
    assert [item["id"] for item in projection_a["toolchain"]] == [
        "git",
        "powershell",
        "python-gate",
    ]
    serialized = json.dumps(projection_a, sort_keys=True)
    for forbidden in (
        hidden_process_marker,
        hidden_task_marker,
        COLLECTOR_SID,
    ...
    digest_input = {
        key: value
        for key, value in projection_a.items()
        if key != "inventory_sha256"
    }
    assert projection_a["inventory_sha256"] == gate.canonical_digest(
        digest_input
    )


def test_validated_inventory_refuses_source_provenance_mismatch(
    tmp_path: Path,
) -> None:
    fixture = _fixture(tmp_path)
    processes, tasks = _validated_inventories(fixture)

    with pytest.raises(RefusalError, match="provenance head"):
        gate._validated_inventory_projection(
            exact_source_head=fixture.commit,
            provenance_head="f" * 40,
            manifest=fixture.manifest,
            sample_a=fixture.evidence["samples"][0],
            sample_b=fixture.evidence["samples"][1],
            process_inventory=processes,
...
def test_production_wrapper_seals_injected_authority_and_scope_fixture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fixture = _fixture(tmp_path)
    projection = deepcopy(
        fixture.audit()["observations"]["validated_inventory"]
    )
    projection["scope_proof"] = {
        "complete": True,
        "reason": "caller_claimed_complete",
    }
    injected = {
        "schema": "wd.bridge_runtime_deployment_gate_report.v2",
        "ok": True,
        ...
        "authority": {
            key: True for key in gate.authority_flags()
        },
    }
    monkeypatch.setattr(
        gate,
        "_evaluate_bridge_runtime_deployment",
        lambda **_: deepcopy(injected),
    )

    report = gate.audit_bridge_runtime_deployment(
    ...
    assert report["matches_expected"] is False
    assert all(value is False for value in report["authority"].values())
    sealed = report["observations"]["validated_inventory"]
    assert sealed["scope_proof"] == {
        "complete": False,
        "reason": "non_heuristic_process_task_scope_not_implemented",
    }
    assert sealed["inventory_sha256"] == gate.canonical_digest(
        {
            key: value
...
def test_ready_collector_config_comment_mutation_is_refused_after_reduction(
    ...
    assert "deployment_definition_refused" in _codes(report)
    assert "config changed" in report["blockers"][0]["detail"]
    _assert_no_validated_inventory(report)
...
def test_duplicate_process_and_task_inventory_cannot_be_projected(
    tmp_path: Path,
) -> None:
    fixture = _fixture(tmp_path)
    for sample in fixture.evidence["samples"]:
        sample["processes"].append(deepcopy(sample["processes"][0]))
        sample["scheduled_tasks"].append(
            deepcopy(sample["scheduled_tasks"][0])
        )

    report = fixture.audit()

    assert "duplicate_observed_process_pid" in _codes(report)
    assert "duplicate_scheduled_task" in _codes(report)
    assert report["matches_expected"] is False
    _assert_no_validated_inventory(report)
