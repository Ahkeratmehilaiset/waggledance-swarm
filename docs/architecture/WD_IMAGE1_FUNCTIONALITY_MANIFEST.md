...
| Hexagonal upgrades | Partial, with in-memory proof, runtime-boundary smoke, read-only topology boundary `/metrics` gauges, a shadow subdivision replay artifact, and a local offline replay verifier | `waggledance/core/hex_topology/subdivision_operator.py`, `ring_messaging.py`, `parent_child_relations.py`, `waggledance/bootstrap/container.py`, `hex_topology_registry.py`, `hex_neighbor_assist.py`, `waggledance/adapters/http/routes/metrics.py`, `tools/hex_shadow_subdivision_replay.py` | Add a path-free reviewer summary for the shadow subdivision replay verifier without activating runtime subdivision authority. |
...
invalid or unreadable inputs fail closed without recording the input path.
