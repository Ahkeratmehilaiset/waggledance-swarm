...
| Hexagonal upgrades | Partial, with in-memory proof, runtime-boundary smoke, read-only topology boundary `/metrics` gauges, a shadow subdivision replay artifact, a local offline replay verifier, a path-free reviewer summary for that verifier, a template-only bridge-event renderer for that summary, a local template index entry for that renderer, a local verifier for that index entry, a path-free reviewer summary for that verifier result, a template-only bridge-event renderer for that verification summary, a local index entry for that verification-summary template, a local verifier for that index entry, a measurement-only cross-consistency digest, and a template-only bridge-event renderer for that digest | `waggledance/core/hex_topology/subdivision_operator.py`, `ring_messaging.py`, `parent_child_relations.py`, `waggledance/bootstrap/container.py`, `hex_topology_registry.py`, `hex_neighbor_assist.py`, `waggledance/adapters/http/routes/metrics.py`, `tools/hex_shadow_subdivision_replay.py`, `tools/build_hex_upgrade_cross_consistency_digest_bridge_event_template.py` | Add a local index entry for the hex-upgrade cross-consistency digest bridge-event template without including payloads, recording paths, appending it, upgrading any claim, or activating runtime subdivision authority. |
