...
**CI status:** 🟢 PR-level CI green on Phase 16F (PR #68: 5/5 checks). Push-event CI on `main` may continue to fail `tests/phase10/test_truth_regression.py::test_phase10_branch_history_is_linear_descended_from_main` until [PR #67](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/67) (`fix/ci-fetch-depth-truth-regression` — adds `fetch-depth: 0` to the GitHub Actions checkout) merges. PR #67 is orthogonal to v3.8.0 (does not change autonomy code; does not affect Docker proofs); it merely unblocks the main push CI lane. v3.8.0 release is unaffected.
