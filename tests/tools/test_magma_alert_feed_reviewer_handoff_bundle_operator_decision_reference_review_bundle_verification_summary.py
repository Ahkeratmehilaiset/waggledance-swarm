...
from tools.build_magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_summary import (
    ...
    render_operator_decision_reference_review_bundle_verification_summary_markdown,
)
...
SCRIPT = (
    ...
    / (
        ...
        "reference_review_bundle_verification_summary.py"
    )
)
COMMIT_SHA = "9" * 40
...
def test_operator_decision_reference_review_bundle_verification_summary_renders_verifier_result_without_authority() -> None:
    summary = build_magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_summary(
    ...
    assert summary["created_at_utc"] == "2026-05-29T03:00:00Z"
    assert summary["release_ref"] == "pr:765"
    assert summary["commit_sha"] == COMMIT_SHA
    assert summary["ci_run_ref"] == "gh:run:decision-reference-review-bundle-verification"
    ...
    markdown = render_operator_decision_reference_review_bundle_verification_summary_markdown(
        summary
    )
...
def test_operator_decision_reference_review_bundle_verification_summary_propagates_verifier_blockers_without_approval() -> None:
    report = _verification_report()
    report["ok"] = False
    report["rebuilt_index_check"] = "mismatch"
    report["blockers"] = ["rebuilt_index_mismatch"]
    ...
    assert verification["rebuilt_index_check"] == "mismatch"
    assert "rebuilt_index_mismatch" in summary["blockers"]
    assert (
...
def test_operator_decision_reference_review_bundle_verification_summary_blocks_authority_flags() -> None:
    report = _verification_report()
...
def test_operator_decision_reference_review_bundle_verification_summary_blocks_invalid_contract_fields() -> None:
    cases = (
        (
            "missing_release_ref",
...
def test_operator_decision_reference_review_bundle_verification_summary_blocks_nonmatching_checks() -> None:
    cases = (
        (
            "digest_checks",
            "operator_decision_reference_review_summary",
            "mismatch",
            "verification_check_not_match:digest_checks:operator_decision_reference_review_summary",
        ),
        (
            "size_checks",
            "operator_decision_reference_validation",
            "missing_index_record",
            "verification_check_not_match:size_checks:operator_decision_reference_validation",
        ),
        (
            "schema_version_checks",
            "operator_decision_reference_review_summary",
            "unknown",
            "verification_check_not_match:schema_version_checks:operator_decision_reference_review_summary",
        ),
    )
...
def test_operator_decision_reference_review_bundle_verification_summary_blocks_invalid_decision_reference() -> None:
    cases = (
        (
            "missing_reference",
            lambda report: report.pop("operator_decision_reference", None),
...
def test_operator_decision_reference_review_bundle_verification_summary_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    verification_path = tmp_path / "review_bundle_verification_report.json"
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(SCRIPT),
            "--review-bundle-verification-json",
            str(verification_path),
            "--reviewer-agent",
            "claude-rco-1",
            "--handoff-ref",
            "bridge:handoff:decision-reference-review-bundle-verification",
            "--now",
...
def test_operator_decision_reference_review_bundle_verification_summary_missing_input_is_path_free() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(SCRIPT),
            "--review-bundle-verification-json",
            "C:/private/review_bundle_verification_report.json",
            "--reviewer-agent",
            "claude-rco-1",
            "--handoff-ref",
            "bridge:handoff:decision-reference-review-bundle-verification",
    ...
    assert payload["blockers"] == [
        "operator_decision_reference_review_bundle_verification_summary_failed:"
        "verification_report_unreadable"
    ]
    assert payload["approval_granted"] is False
    assert payload["local_paths_recorded"] is False
    assert "review_bundle_verification_report" not in result.stdout
    assert not any(marker in result.stdout for marker in PRIVATE_MARKERS)


def test_operator_decision_reference_review_bundle_verification_summary_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    verification_path = tmp_path / "review_bundle_verification_report.json"
    report = _verification_report()
