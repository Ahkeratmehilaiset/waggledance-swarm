...
"""Build a MAGMA template-index verification summary bridge-event template."""
...
TEMPLATE_VERSION = "magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template.v1"
    ...
    try:
        ...
        try:
            ...
            report = _failure_report(
                "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_invalid"
            )
...
def build_magma_decision_review_verification_template_index_entry_summary_bridge_event_template(
    *,
    ...
    if not isinstance(summary, Mapping):
        raise ContractError("index_entry_verification_summary_not_mapping")
...
def _failure_report(reason: str) -> dict[str, Any]:
    return {
        ...
        "blockers": [
            ...
            f"verification_summary_bridge_event_template_failed:{_safe_token(reason)}"
        ],
        "warnings": [],
    }
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            ...
            "template contains forbidden markers"
        )
