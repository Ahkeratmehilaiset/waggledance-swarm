...
**Known gap:** `bridge_policy_epochs._policy_material` currently derives the
policy epoch from the document refs and digests only, so the domain label does
*not* yet participate in the epoch value. Two different domains do produce
different epochs today, but only incidentally, because their file paths differ.
Binding the domain into the epoch material needs a change to
`bridge_policy_epochs.py`, which was outside the allowed scope of this slice.
