...
MAGMA_PROVENANCE_FILE_DENYLIST_ENTRIES = {
    ...
    "waggledance/core/magma/receipt.py",
...
def test_charter_denylist_contains_magma_provenance_primitives() -> None:
    charter = load_charter()
    assert MAGMA_PROVENANCE_FILE_DENYLIST_ENTRIES <= set(charter.file_denylist)
...
def test_evaluate_paths_blocks_magma_provenance_primitives() -> None:
    charter = load_charter()
    for path in sorted(MAGMA_PROVENANCE_FILE_DENYLIST_ENTRIES):
        decision = evaluate_paths(charter, [path])
        assert decision.allowed is False
        assert decision.blocked_paths == (path,)
        assert decision.reason == "denylist hit"
...
def test_magma_allowlist_still_allows_non_denied_magma_subpaths() -> None:
    charter = load_charter()
    decision = evaluate_paths(
        charter,
