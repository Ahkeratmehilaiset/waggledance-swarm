...
**Derived from declared material, not authenticated.** This builder does not
accept an epoch directly, but a caller can still assert the underlying digests
and native identity. Hashing those declarations does not verify their truth.
A trusted evidence source must verify file content and observed identity before
any runtime consumer relies on this snapshot. Reading and hashing real files
is compatible with observation-only operation; this pure builder does not do it.
