...
"""Verify a local MAGMA summary bridge-template index entry.

This is a local verifier for the operator decision-reference review bundle verification bridge-event template index-entry verification summary bridge-event template index entry.
"""
...
VERIFICATION_VERSION = "magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification.v1"
...
class SummaryBridgeTemplateIndexEntryVerificationError(ValueError):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        _, index_entry = _load_json_artifact(
            ...
            "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry",
        )
        summary_bytes, summary = _load_json_artifact(
            args.index_entry_verification_summary_json,
            _REQUIRED_ARTIFACTS[0],
        )
    ...
    except SummaryBridgeTemplateIndexEntryVerificationError as exc:
        report = _failure_report(exc.code)
    except ValueError:
        report = _failure_report(
            "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_invalid"
        )
...
def verify_magma_decision_review_verification_template_index_entry_summary_bridge_event_template_index_entry(
    *,
    ...
    """Recompute artifact checks for a summary bridge-template index entry."""

    _assert_mapping(
        "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry",
        index_entry,
    )
    _assert_mapping(_REQUIRED_ARTIFACTS[0], index_entry_verification_summary)
    _assert_mapping(_REQUIRED_ARTIFACTS[1], summary_bridge_event_template_report)
    for artifact_id, artifact in (
        (
            "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry",
            index_entry,
        ),
        (_REQUIRED_ARTIFACTS[0], index_entry_verification_summary),
        (_REQUIRED_ARTIFACTS[1], summary_bridge_event_template_report),
    ):
    ...
    rebuilt_entry = _rebuilt_source_index_entry(
        ...
        index_entry_verification_summary_bytes=index_entry_verification_summary_bytes,
    ...
    identity = _verified_identity(
        (
            index_entry,
            index_entry_verification_summary,
            _template_payload_identity_source(summary_bridge_event_template_report),
        ),
        blockers,
    )
...
def _collect_template_entry_blockers(
    ...
    *,
    ...
    if template_entry.get("template_version") != SUMMARY_BRIDGE_TEMPLATE_VERSION:
        blockers.append("template_index_entry_template_version_mismatch")
    if template_entry.get("source_summary_artifact_id") != _REQUIRED_ARTIFACTS[0]:
        blockers.append("template_index_entry_source_summary_artifact_id_mismatch")
    if template_entry.get("source_summary_sha256") != summary_digest:
        blockers.append("template_index_entry_source_summary_sha256_mismatch")
    if template_entry.get("template_sha256") != template_digest:
    ...
    if template_entry.get("event_status") != _BRIDGE_TEMPLATE_EVENT_STATUS:
...
def _load_json_artifact(path: Path, artifact_id: str) -> tuple[bytes, Mapping[str, Any]]:
    try:
        ...
        raise SummaryBridgeTemplateIndexEntryVerificationError(
