# Release Readiness — WaggleDance (v3.8.0 stable + v3.9.0/v3.9.1/v3.9.2/v3.9.3 + v3.10.0 + v3.10.1 + v3.10.2 + v3.10.3 PRERELEASE + v3.10.4-incremental-gap-replay-alpha CANDIDATE)
...
**Phase 18D landed:** docs-only PR #85 (squash-merge `7d1dedef`, 2026-05-05T21:55:09Z). No tag created.

**Released prerelease (Phase 18E):** **v3.10.3-runtime-gap-replay-alpha** (released 2026-05-06T05:42:51Z from PR #86 squash-merge `6c6ca859`). Phase 18E proves the **durable** autonomous-learning loop end-to-end: persisted runtime gap events (in the existing `runtime_gap_signals` table with `kind = phase18e.runtime_gap_event.v1`) → loaded → mined by Phase 18B `mine_runtime_gaps` → registered by Phase 18C `register_mined_solver_specs` → dispatched through the **real** `LowRiskSolverDispatcher.dispatch_by_features`. New mainline module `waggledance/core/autonomy_growth/runtime_gap_replay.py`; new proof harness `tools/run_phase18e_runtime_gap_replay_proof.py`; 48 unit tests. **ControlPlaneDB schema v3 unchanged** — no `ALTER TABLE`, no new column; one read-only helper (`list_runtime_gap_signals`) added. Host run: 32 events persisted (3 malformed + 1 forbidden-field rejected), 32 loaded, 6 registered auto-promoted solvers, 18/18 dispatch cases hit, 6/6 families covered, replay idempotent (second persist inserted 0, second replay added 0 extra rows). `release_gate_pass = true`. Docker `--network none` PASS for Phase 18E proof + Phase 18C / 18B / 18A carry-forward (4/4 exit 0). Honesty: no model pull/download, no cloud API call, no live builder execution, no allowlist widening, no Stage-2 flip, no HUMAN_APPROVAL collected, no consciousness claim, no cross-vendor ranking claim, no raw-intelligence superiority claim, no new pip dependency, no DB/SQLite files committed, no token/secret exposure.

**Candidate prerelease (Phase 18F):** **v3.10.4-incremental-gap-replay-alpha** — to be tagged from the Phase 18F PR squash-merge SHA after PR-level CI green and `--match-head-commit`-protected merge. Phase 18F upgrades Phase 18E's whole-corpus replay into a cursor-based incremental learning loop. New mainline module `waggledance/core/autonomy_growth/incremental_gap_replay.py` (~580 LOC, stdlib + WaggleDance only) ships cursor + lock state in `schema_meta` (no new table; key/value reuse), strict + counted-skip loader, RuntimeGapDetector adapter, and `run_incremental_gap_replay_once` orchestrator. Phase 18C compile table extended with 6 new strict per-family rules — no allowlist widening, no new family_kind. **ControlPlaneDB schema unchanged** (schema_version still 4, no `ALTER TABLE`, no new column, no new table). Host run: seed 32 events; first replay registers 6 solvers / 6 families / 18 dispatch hits / cursor advances; no-op processes 0 rows; 12 post-cursor events appended; third replay registers 6 new solvers / 18 new dispatch hits; total 12 registered solvers; type-confused / forbidden / non-allowlisted all rejected; detector bridge accepts 1 + rejects 2 malformed; held-lock test returns `LOCKED_NOT_RUN` with 0 duplicates. `release_gate_pass = true`. Docker `--network none` PASS for Phase 18F + 18E + 18C + 18B + 18A validator (5/5 exit 0). Honesty: no model pull/download, no cloud API call, no live builder execution, no allowlist widening, no Stage-2 flip, no HUMAN_APPROVAL collected, no consciousness claim, no cross-vendor ranking claim, no raw-intelligence superiority claim, no new pip dependency, no DB/SQLite files committed, no token/secret exposure.
