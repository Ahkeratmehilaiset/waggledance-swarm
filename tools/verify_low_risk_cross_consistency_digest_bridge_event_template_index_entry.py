...
"""Verify a local low-risk cross-consistency bridge-template index entry.

This is an offline verifier for the WD Image #1 low-risk autonomy loop chain:
...
network I/O, writes files, enqueues scheduler work, or grants runtime authority.
"""
...
from tools.build_low_risk_cross_consistency_digest_bridge_event_template_index_entry import (  # noqa: E402
    AUTHORITY_FALSE_FIELDS,
    FORBIDDEN_OUTPUT_MARKERS,
    INDEX_ENTRY_VERSION,
    TEMPLATE_ARTIFACT_ID,
    build_low_risk_cross_consistency_digest_bridge_event_template_index_entry,
    validate_low_risk_cross_consistency_digest_bridge_event_template_index_entry,
)
from waggledance.core.magma.canonical import sha256_digest  # noqa: E402


VERIFICATION_VERSION = (
    "wd.low_risk_cross_consistency_digest_bridge_event_template_"
    "index_entry_verification.v1"
)
INDEX_ENTRY_ARTIFACT_ID = (
    "low_risk_cross_consistency_digest_bridge_event_template_index_entry"
)


class LowRiskIndexEntryVerificationError(ValueError):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = verify_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
    ...
    except LowRiskIndexEntryVerificationError as exc:
        report = _failure_report(exc.code)
    except ValueError:
        report = _failure_report(
            "low_risk_cross_consistency_digest_bridge_event_template_"
    ...
    if args.json or report["ok"]:
        ...
        print(
            "low-risk cross-consistency bridge-template index entry "
            "verification FAILED: "
...
def verify_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
    *,
    ...
    """Recompute and verify the local low-risk bridge-template index entry."""
...
def _load_json_artifact(path: Path, label: str) -> tuple[bytes, Mapping[str, Any]]:
    try:
        ...
        raise LowRiskIndexEntryVerificationError(f"{label}_unreadable") from exc
...
def _decode_json_artifact_bytes(raw: bytes, label: str) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise LowRiskIndexEntryVerificationError(f"{label}_decode_error") from exc
    except (json.JSONDecodeError, ValueError) as exc:
        raise LowRiskIndexEntryVerificationError(f"{label}_json_error") from exc
    if not isinstance(parsed, Mapping):
        raise LowRiskIndexEntryVerificationError(f"{label}_not_object")
    return parsed
...
def _assert_no_forbidden_input(label: str, value: Any) -> None:
    if _contains_forbidden_marker(value):
        raise LowRiskIndexEntryVerificationError(f"{label}_contains_forbidden_marker")


def _assert_no_forbidden_output(encoded: str) -> None:
    if _has_forbidden_marker(encoded):
        raise LowRiskIndexEntryVerificationError(
...
def _plain_json_object(value: Any) -> dict[str, Any]:
    try:
        ...
        raise LowRiskIndexEntryVerificationError("json_object_required") from exc
    if not isinstance(plain, dict):
        raise LowRiskIndexEntryVerificationError("json_object_required")
    return plain
