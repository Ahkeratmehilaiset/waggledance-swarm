...
"""hallucinated-success tail-floor expansion provenance fixture tests."""
...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_11_hallucinated_success_to_7.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR / "v0_expansion_2026_06_11_hallucinated_success_to_7_expectations.json"
)
LABEL = "hallucinated_success_to_7_2026_06_11"
...
def test_expansion_is_exactly_one_hallucinated_success_case():
    cases = _read_json(EXPANSION)["cases"]
    assert len(cases) == 1
    case = cases[0]
    assert case["case_id"] == "case:adv:hallucinated_success:007"
    assert case["defect_type"] == "hallucinated-success"
    assert {"stale_artifact", "run_identity"} <= set(case["tags"])
...
def test_strict_v0_hallucinated_success_coverage_raised_to_seven():
    cases = _read_json(CORPUS_DIR / "v0.json")["cases"]
    count = sum(1 for c in cases if c["defect_type"] == "hallucinated-success")
    assert count == 7


def test_every_expansion_case_has_paired_review_expectation():
    ...
    for exp in exps:
        ...
        assert "verifier:insufficient_signal" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    assert case["privacy_canary"] == "canary_hallucinate_007_DO_NOT_LEAK"
...
def test_held_out_split_unchanged_and_valid():
    ...
    assert "case:adv:hallucinated_success:007" not in held_out
