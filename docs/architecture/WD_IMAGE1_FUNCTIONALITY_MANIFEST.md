...
| Low-risk autonomy loop | Partial, with temp-DB proof, runtime-boundary smoke, operator metrics, a read-only dashboard ops overlay with local alert state, operator alert thresholds, and a read-only runtime gap detector report | `waggledance/core/autonomy_growth/low_risk_policy.py`, `runtime_query_router.py`, `autogrowth_scheduler.py`, `waggledance/bootstrap/container.py`, `waggledance/adapters/http/api.py`, `waggledance/adapters/http/routes/metrics.py`, `waggledance/adapters/http/routes/compat_dashboard.py`, `tools/run_runtime_gap_detector_report.py`, `web/hologram-brain-v6.html`, `docs/operations/LOW_RISK_AUTOGROWTH_RUNBOOK.md` | Feed an operator-owned runtime gap signal export into a scheduler-candidate artifact without enqueueing or running the scheduler. |
...
`tools/run_runtime_gap_detector_report.py` can also summarize deterministic or
operator-owned `GapSignal`-shaped exports into scheduler-candidate readiness
without opening the control-plane database, calling `RuntimeGapDetector.record`,
digesting signals into intents, enqueueing work, or running a scheduler tick.
The report records only allowlist-safe aggregates and spec-seed digests, not raw
payloads or input paths.
