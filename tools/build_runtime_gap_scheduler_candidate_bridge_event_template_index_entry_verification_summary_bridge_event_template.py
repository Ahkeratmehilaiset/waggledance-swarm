...
"""Build a runtime-gap verifier-summary bridge-event template.

The template is reviewer context for an already-local verification summary. It
never appends a bridge event, transports payloads, enqueues scheduler work,
runs a scheduler tick, fast-tracks priority, skips gates, or grants runtime
authority.
"""
...
_ARTIFACT_IDS = (ARTIFACT_ID, TEMPLATE_ARTIFACT_ID)
...
_NESTED_FALSE_FIELDS = frozenset((*AUTHORITY_FALSE_FIELDS, *_SUMMARY_FALSE_FIELDS))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-07T15:00:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "runtime-gap scheduler-candidate verifier-summary bridge-event "
            "template FAILED: "
...
def build_runtime_gap_scheduler_candidate_bridge_event_template_index_entry_verification_summary_bridge_event_template(
    *,
    ...
    payload = {
        ...
        "reviewer_next_actions": [
            "review_runtime_gap_scheduler_candidate_index_entry_verification_summary_bridge_event_template",
    ...
    event = {
        ...
        "to": targets,
        "message": (
            "Runtime-gap scheduler-candidate index-entry verification summary "
            ...
            "recording, runtime controls, runtime authority, scheduler enqueue, "
            "scheduler tick, queue/control-plane write, fast-track priority, "
            "gate skip, merge, promotion, or release decision."
        ),
        "paths": [],
        "write_scope": [],
        "run_id": run_id,
...
def _recursive_contract_blockers(value: Any) -> list[str]:
    ...
    if isinstance(value, Mapping):
        for raw_key, child in value.items():
            ...
            if key in _NESTED_FALSE_FIELDS and child is not False:
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "runtime-gap verifier-summary bridge-event template contains "
            "forbidden markers"
        )
