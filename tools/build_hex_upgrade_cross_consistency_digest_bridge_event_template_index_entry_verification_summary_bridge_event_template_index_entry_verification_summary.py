...
"""Render a hex xcons verifier-summary index verifier summary."""
...
_AUTHORITY_FALSE_FIELDS = _OUTPUT_FALSE_FIELDS
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T10:30:00Z.",
    )
...
def build_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary(
    *,
    ...
    contract_blockers.extend(
        _token_list_schema_blockers(verification_report, "blockers")
    )
    contract_blockers.extend(
    ...
    summary: dict[str, Any] = {
        ...
        "reviewer_next_actions": [
            "review_hex_xcons_verifier_summary_bridge_template_index_entry_verification_summary",
...
def render_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# Hex Cross-Consistency Verifier-Summary Index Verification Summary",
        "",
...
def _assert_no_forbidden_output(text: str) -> None:
    if _contains_path_marker(text) or _forbidden_output_markers(text):
        raise ValueError(
            "hex-upgrade xcons verifier-summary bridge-template index-entry "
            "verification summary contains forbidden markers"
        )
