...
VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_BRIDGE_EVENT_TEMPLATE_PROOF_ID = (
    "hex_shadow_subdivision_replay_verifier_summary_bridge_event_template_"
    "index_entry_verification_summary_bridge_event_template_v1"
)
...
VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_BRIDGE_EVENT_STATUS = (
    "hex_shadow_subdivision_replay_verifier_summary_bridge_event_template_"
    "index_entry_verification_summary_bridge_event_template_ready"
)
VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_PROOF_ID = (
    "hex_shadow_subdivision_replay_verifier_summary_bridge_event_template_"
    "index_entry_verification_summary_bridge_event_template_index_entry_v1"
)
...
def _parse_json_bytes_without_duplicate_keys_for_reasons(
    raw: bytes,
    *,
    duplicate_key_reason: str,
    json_error_reason: str,
    raw_path_marker_reason: str,
    ...
    try:
        ...
        return None, json_error_reason
    if _contains_path_marker(decoded):
        return None, raw_path_marker_reason
    return parsed, None
...
def _parse_json_bytes_without_duplicate_keys(
    ...
    return _parse_json_bytes_without_duplicate_keys_for_reasons(
        raw,
        duplicate_key_reason="summary_bridge_event_template_duplicate_key",
        json_error_reason="summary_bridge_event_template_json_error",
        raw_path_marker_reason="summary_bridge_event_template_raw_path_marker",
    )
...
def _bridge_template_index_entry_verification_error_report(reason: str) -> dict:
    return {
        "proof_id": (
            VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_PROOF_ID
        ),
        ...
        "blockers": [
            "shadow_subdivision_replay_verifier_summary_bridge_event_template_"
            f"index_entry_verification_failed:{_safe_token(reason)}"
        ],
        "warnings": [],
        "safe_conclusion": (
            "The bridge-event template index-entry verifier failed closed. "
            "It does not append to the bridge, transport artifacts, include "
            "payloads, record local paths, or grant runtime subdivision "
            "authority."
        ),
    }
...
def _index_entry_verification_summary_bridge_template_index_entry_error_report(
    reason: str,
) -> dict:
    blocker = (
        "shadow_subdivision_replay_verifier_summary_bridge_event_template_"
    ...
    return {
        ...
        "safe_conclusion": (
            ...
            "index entry failed closed before rendering. It does not append "
            "to the bridge, transport artifacts, include payloads, record "
            "local paths, or grant runtime subdivision authority."
        ),
    }
...
def _deterministic_index_entry_for_verification(
    ...
    try:
        normalized = json.loads(
            json.dumps(index_entry, allow_nan=False, sort_keys=True)
        )
    except (TypeError, ValueError):
        blockers.append("rebuilt_index_entry_not_deterministic")
        return {}
    if not isinstance(normalized, dict):
        blockers.append("rebuilt_index_entry_not_object")
        return {}
    normalized.pop("created_at_utc", None)
    return normalized


def _compare_rebuilt_bridge_template_index_entry(
    *,
    observed: Mapping[str, Any],
    rebuilt: Mapping[str, Any],
    blockers: list[str],
) -> str:
    observed_index = _deterministic_index_entry_for_verification(observed, blockers)
    rebuilt_index = _deterministic_index_entry_for_verification(rebuilt, blockers)
    if observed_index and observed_index == rebuilt_index:
        return "match"
    blockers.append("rebuilt_index_entry_mismatch")
    return "mismatch"


def _bridge_template_index_entry_artifact_records(
    index_entry: Mapping[str, Any],
    blockers: list[str],
...
def verify_shadow_subdivision_replay_verifier_summary_bridge_event_template_index_entry(
    index_entry: Mapping[str, Any],
    template_report: Mapping[str, Any],
    *,
    template_report_bytes: bytes | None = None,
) -> dict:
    """Verify a local template index entry without recording paths or payloads."""

    if not isinstance(index_entry, Mapping):
        return _bridge_template_index_entry_verification_error_report(
            "summary_bridge_event_template_index_entry_not_object"
        )
    if not isinstance(template_report, Mapping):
        return _bridge_template_index_entry_verification_error_report(
            "summary_bridge_event_template_not_object"
    ...
    if _contains_path_marker(index_entry):
        return _bridge_template_index_entry_verification_error_report(
            "summary_bridge_event_template_index_entry_path_free"
        )
    if _contains_path_marker(template_report):
        return _bridge_template_index_entry_verification_error_report(
            "summary_bridge_event_template_path_free"
    ...
    blockers: list[str] = []
    warnings: list[str] = []
    rebuilt_entry = build_shadow_subdivision_replay_verifier_summary_bridge_event_template_index_entry(
        template_report,
    ...
    records = _bridge_template_index_entry_artifact_records(index_entry, blockers)
    ...
    if record is None:
        ...
        blockers.append(f"missing_artifact_record:{artifact_id}")
    else:
        if record.get("sha256") == digest:
            digest_checks[artifact_id] = "match"
        else:
            digest_checks[artifact_id] = "mismatch"
            blockers.append(f"digest_mismatch:{artifact_id}")
        if record.get("size_bytes") == len(raw):
            size_checks[artifact_id] = "match"
        else:
            size_checks[artifact_id] = "mismatch"
            blockers.append(f"size_mismatch:{artifact_id}")
        if (
            record.get("json_schema_version")
            == VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_VERSION
        ):
            schema_version_checks[artifact_id] = "match"
        else:
            schema_version_checks[artifact_id] = "mismatch"
            blockers.append(f"schema_version_mismatch:{artifact_id}")
    ...
    if not template_index_entry:
        ...
        if template_index_entry.get("template_sha256") != digest:
            blockers.append(f"template_index_entry_digest_mismatch:{artifact_id}")
        ...
        if (
            template_index_entry.get("event_status")
            != VERIFIER_SUMMARY_BRIDGE_EVENT_STATUS
        ):
    ...
    if not consistency:
        ...
        if consistency.get("required_artifacts_present") != [artifact_id]:
    ...
    report = {
        ...
        "safe_conclusion": (
            ...
            "verifier summary bridge-event template index-entry digest, size, "
            "schema, source-contract, rebuilt-entry, and bridge-event schema "
            "checks. It does not include payloads, append bridge events, "
            "transport artifacts, record local paths, or grant runtime "
            "subdivision authority."
        ),
    }
    ...
    if _contains_path_marker(serialized):
        return _bridge_template_index_entry_verification_error_report(
            "summary_bridge_event_template_index_entry_verification_output_path_marker"
        )
    return report
...
def _index_entry_verification_summary_bridge_template_error_report(
    reason: str,
) -> dict:
    return {
        "proof_id": (
        ...
        "safe_conclusion": (
            "The index-entry verification summary bridge-event template "
            "failed closed before rendering. It does not append to the "
...
def _bridge_template_index_entry_verification_summary_contract_blockers(
    summary: Mapping[str, Any],
) -> list[str]:
    blockers: list[str] = []
    artifact_id = VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_ARTIFACT_ID
    ...
    if (
        ...
        blockers.append("index_entry_verification_summary_proof_id_mismatch")
    if (
    ...
    if (
        summary.get("verified_verifier_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_PROOF_ID
    ):
        blockers.append("index_entry_verification_summary_verified_verifier_mismatch")
    if summary.get("template_only") is not True:
        blockers.append("index_entry_verification_summary_template_only_not_true")
    if summary.get("manual_review_required") is not True:
        blockers.append(
            "index_entry_verification_summary_manual_review_required_not_true"
        )
    error = _empty_blocker_list_contract_error(
        summary.get("blockers"),
        malformed="index_entry_verification_summary_blockers_malformed",
        present="index_entry_verification_summary_blockers_present",
    )
    if error is not None:
        blockers.append(error)
    for field in SUMMARY_AUTHORITY_FALSE_FIELDS:
    ...
    reviewer = _mapping_or_empty(summary.get("reviewer_ownership"))
    if _safe_ref_or_invalid(reviewer.get("reviewer_agent_id")) == "invalid_ref":
        blockers.append("index_entry_verification_summary_reviewer_agent_invalid")
    if _safe_ref_or_invalid(reviewer.get("handoff_ref")) == "invalid_ref":
        blockers.append("index_entry_verification_summary_handoff_ref_invalid")
    if reviewer.get("manual_review_required") is not True:
        blockers.append(
            "index_entry_verification_summary_reviewer_manual_review_required_not_true"
        )
    if reviewer.get("approval_granted") is not False:
        blockers.append(
            "index_entry_verification_summary_reviewer_approval_granted_not_false"
        )
    if reviewer.get("runtime_subdivision_authority_granted") is not False:
        blockers.append(
            "index_entry_verification_summary_reviewer_runtime_subdivision_"
            "authority_granted_not_false"
        )
    ...
    if verification.get("verification_ok") is not True:
        blockers.append("index_entry_verification_summary_verification_not_ok")
    if (
        verification.get("verifier_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_PROOF_ID
    ):
        blockers.append("index_entry_verification_summary_verifier_proof_id_mismatch")
    if (
        verification.get("verified_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_PROOF_ID
    ):
        blockers.append("index_entry_verification_summary_verified_proof_id_mismatch")
    if (
        verification.get("verification_version")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_VERSION
    ):
        blockers.append(
            "index_entry_verification_summary_verification_version_mismatch"
        )
    if (
        verification.get("index_entry_version")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERSION
    ):
        blockers.append("index_entry_verification_summary_index_entry_version_mismatch")
    artifact_count = verification.get("artifact_count_checked")
    if (
        isinstance(artifact_count, bool)
        or not isinstance(artifact_count, int)
        or artifact_count != 1
    ):
        blockers.append("index_entry_verification_summary_artifact_count_mismatch")
    if verification.get("blocker_count") != 0:
        blockers.append("index_entry_verification_summary_blocker_count_nonzero")
    error = _empty_blocker_list_contract_error(
        verification.get("blockers"),
        malformed="index_entry_verification_summary_verification_blockers_malformed",
        present="index_entry_verification_summary_verification_blockers_present",
    )
    if error is not None:
        blockers.append(error)
    for name, value in (
        ("digest", verification.get("digest_checks")),
        ("size", verification.get("size_checks")),
        ("schema_version", verification.get("schema_version_checks")),
    ):
        checks = _mapping_or_empty(value)
        if checks.get(artifact_id) != "match":
            blockers.append(f"index_entry_verification_summary_{name}_check_not_match")
    for name in (
        "source_contract_check",
        "rebuilt_index_entry_check",
        "bridge_event_schema_check",
    ):
        if verification.get(name) != "match":
            blockers.append(f"index_entry_verification_summary_{name}_not_match")

    boundary = _mapping_or_empty(summary.get("operator_boundary"))
    if boundary.get("index_entry_verification_report_boundary_ok") is not True:
        blockers.append(
            "index_entry_verification_summary_boundary_verification_report_not_ok"
        )
    error = _empty_blocker_list_contract_error(
        boundary.get("boundary_blockers"),
        malformed="index_entry_verification_summary_boundary_blockers_malformed",
        present="index_entry_verification_summary_boundary_blockers_present",
    )
    if error is not None:
        blockers.append(error)
    if boundary.get("manual_review_required") is not True:
        blockers.append(
            "index_entry_verification_summary_boundary_manual_review_required_not_true"
        )
    for field in SUMMARY_AUTHORITY_FALSE_FIELDS:
        if boundary.get(field) is not False:
            blockers.append(
                f"index_entry_verification_summary_boundary_{field}_not_false"
            )
    return sorted(set(blockers))


def build_shadow_subdivision_replay_verifier_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template(
    summary: Mapping[str, Any],
    *,
    agent_id: str,
    task_id: str,
    ...
    """Return a bridge-event template for the index-entry verification summary."""

    if not isinstance(summary, Mapping):
        return _index_entry_verification_summary_bridge_template_error_report(
            "index_entry_verification_summary_not_object"
        )
    if _contains_path_marker(summary):
        return _index_entry_verification_summary_bridge_template_error_report(
            "index_entry_verification_summary_path_free"
        )
    input_error = _bridge_template_input_error(
        agent_id=agent_id,
        task_id=task_id,
    ...
    contract_blockers = (
        _bridge_template_index_entry_verification_summary_contract_blockers(summary)
    )
    if contract_blockers:
        return _index_entry_verification_summary_bridge_template_error_report(
            contract_blockers[0]
    ...
    payload = {
        ...
        "operator_boundary": {
            "index_entry_verification_report_boundary_ok": True,
            "manual_review_required": True,
            "approval_granted": False,
            "release_decision_made": False,
            "automatic_release_decision": False,
    ...
    event = {
        ...
        "message": (
            ...
            "ready; manual_review_required=true; approval_granted=false; "
            "release_decision_made=false; "
            "runtime_subdivision_authority_granted=false; template_only=true; "
            "no bridge write, transport, external fetch, payload inclusion, "
            "local path recording, runtime controls, or topology mutation."
    ...
    if _contains_path_marker(event):
        return _index_entry_verification_summary_bridge_template_error_report(
            "bridge_event_template_path_marker"
        )
    json.dumps(event, allow_nan=False, sort_keys=True)
    return {
        "proof_id": (
...
def _parse_index_entry_verification_summary_bridge_template_bytes(
    raw: bytes,
) -> tuple[Any | None, str | None]:
    return _parse_json_bytes_without_duplicate_keys_for_reasons(
        raw,
        duplicate_key_reason=(
            ...
            "bridge_event_template_duplicate_key"
        ),
        json_error_reason=(
            "summary_bridge_event_template_index_entry_verification_summary_"
        ...
        raw_path_marker_reason=(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_raw_path_marker"
        ),
    )


def _index_entry_verification_summary_bridge_template_bytes(
    template_report: Mapping[str, Any],
    template_report_bytes: bytes | None,
) -> tuple[bytes | None, str | None]:
    if template_report_bytes is None:
    ...
    raw = bytes(template_report_bytes)
    parsed, parse_error = _parse_index_entry_verification_summary_bridge_template_bytes(
        raw
    )
    if parse_error is not None:
        return None, parse_error
    ...
    if dict(parsed) != dict(template_report):
        return (
            None,
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_bytes_mismatch",
        )
    return raw, None


def _index_entry_verification_summary_bridge_template_contract_blockers(
    template_report: Mapping[str, Any],
) -> list[str]:
    blockers: list[str] = []
    if template_report.get("ok") is not True:
    ...
    if (
        ...
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_proof_id_mismatch"
        )
    if (
        template_report.get("template_version")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_BRIDGE_EVENT_TEMPLATE_VERSION
    ...
    error = _empty_blocker_list_contract_error(
        ...
        malformed=(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_blockers_malformed"
        ),
        present=(
            "summary_bridge_event_template_index_entry_verification_summary_"
    ...
    for field in SUMMARY_AUTHORITY_FALSE_FIELDS:
        if template_report.get(field) is not False:
            blockers.append(
                "summary_bridge_event_template_index_entry_verification_summary_"
                f"bridge_event_template_{field}_not_false"
            )
    ...
    if event.get("type") != "handoff":
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_type_mismatch"
        )
    if (
        event.get("status")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_BRIDGE_EVENT_STATUS
        ...
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_status_mismatch"
        )
    if event.get("paths") != []:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_paths_not_empty"
        )
    if event.get("write_scope") != []:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
    ...
    if event.get("pid") != 0:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_pid_not_zero"
        )
    if event.get("cwd") != "template_not_emitted":
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_cwd_mismatch"
        )
    ...
    if (
        payload.get("summary_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_SUMMARY_PROOF_ID
    ):
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_payload_summary_proof_id_mismatch"
        )
    if payload.get("template_only") is not True:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
    ...
    if payload.get("manual_review_required") is not True:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_payload_manual_review_required_not_true"
        )
    for field in SUMMARY_AUTHORITY_FALSE_FIELDS:
        if payload.get(field) is not False:
            blockers.append(
                "summary_bridge_event_template_index_entry_verification_summary_"
                f"bridge_event_template_payload_{field}_not_false"
            )
    ...
    if nested.get("verification_ok") is not True:
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_verification_not_ok"
        )
    if (
        nested.get("verifier_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_VERIFICATION_PROOF_ID
    ):
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_verifier_proof_id_mismatch"
        )
    if (
        nested.get("verified_proof_id")
        != VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_PROOF_ID
    ):
        blockers.append(
            "summary_bridge_event_template_index_entry_verification_summary_"
            "bridge_event_template_verified_proof_id_mismatch"
        )
    artifact_id = VERIFIER_SUMMARY_BRIDGE_EVENT_TEMPLATE_INDEX_ENTRY_ARTIFACT_ID
    for name, value in (
        ("digest", nested.get("digest_checks")),
        ("size", nested.get("size_checks")),
        ("schema_version", nested.get("schema_version_checks")),
    ):
        checks = _mapping_or_empty(value)
        if checks.get(artifact_id) != "match":
            blockers.append(
                "summary_bridge_event_template_index_entry_verification_summary_"
                f"bridge_event_template_{name}_check_not_match"
            )
    for name in (
        "source_contract_check",
        "rebuilt_index_entry_check",
        ...
        if nested.get(name) != "match":
            blockers.append(
                "summary_bridge_event_template_index_entry_verification_summary_"
                f"bridge_event_template_{name}_not_match"
            )
    return sorted(set(blockers))


def build_shadow_subdivision_replay_verifier_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
    template_report: Mapping[str, Any],
    *,
    template_report_bytes: bytes | None = None,
    now_utc: datetime | None = None,
    ...
    entry = {
        ...
        "consistency": {
            "required_artifacts_present": [artifact_id],
            "all_artifact_digests_recorded": True,
            "bridge_event_schema_validated": True,
            "source_contract_check": "match",
            "template_only": True,
        ...
        "reviewer_next_actions": [
            "review_shadow_subdivision_replay_verifier_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template",
            "verify_index_entry_in_separate_local_verifier",
            "record_operator_decision_separately",
        ],
        "template_only": True,
        ...
        "safe_conclusion": (
            ...
            "for the shadow subdivision replay verifier summary bridge-event "
            "template index-entry verification summary bridge-event template. "
            "It does not include the template payload, append a bridge event, "
            "transport artifacts, record local paths, or grant runtime "
            "subdivision authority."
        ),
    }
...
def _load_bridge_template_report_for_index_entry_verification(
    path: Path | str,
) -> tuple[dict | None, bytes | None, str | None]:
    try:
        raw = Path(path).read_bytes()
    except OSError:
        return None, None, "summary_bridge_event_template_unreadable"
    report, parse_error = _parse_json_bytes_without_duplicate_keys(raw)
    if parse_error is not None:
        return None, None, parse_error
    if not isinstance(report, Mapping):
        return None, None, "summary_bridge_event_template_not_object"
    return dict(report), raw, None


def _load_bridge_template_index_entry_report(
    path: Path | str,
) -> tuple[dict | None, str | None]:
    try:
        raw = Path(path).read_bytes()
    except OSError:
        return None, "summary_bridge_event_template_index_entry_unreadable"
    report, parse_error = _parse_json_bytes_without_duplicate_keys_for_reasons(
        raw,
        duplicate_key_reason="summary_bridge_event_template_index_entry_duplicate_key",
        json_error_reason="summary_bridge_event_template_index_entry_json_error",
        raw_path_marker_reason=(
            "summary_bridge_event_template_index_entry_raw_path_marker"
        ),
    )
    if parse_error is not None:
        return None, parse_error
...
def _load_bridge_template_index_entry_verification_summary_report(
    path: Path | str,
) -> tuple[dict | None, str | None]:
    try:
        raw = Path(path).read_bytes()
        ...
        return (
            None,
            "summary_bridge_event_template_index_entry_verification_summary_unreadable",
        )
    report, parse_error = _parse_json_bytes_without_duplicate_keys_for_reasons(
        raw,
        duplicate_key_reason=(
        ...
        json_error_reason=(
            "summary_bridge_event_template_index_entry_verification_summary_json_error"
        ),
        raw_path_marker_reason=(
            "summary_bridge_event_template_index_entry_verification_summary_raw_path_marker"
        ),
    )
    if parse_error is not None:
        return None, parse_error
    if not isinstance(report, Mapping):
        return (
            None,
            "summary_bridge_event_template_index_entry_verification_summary_not_object",
        )
    return dict(report), None


def _load_bridge_template_index_entry_verification_summary_bridge_template_report(
    path: Path | str,
) -> tuple[dict | None, bytes | None, dict | None]:
    try:
        raw = Path(path).read_bytes()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Verify an existing local verifier summary bridge-event template "
            "index-entry JSON file. The verification is printed but not "
            "appended."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Render a template-only bridge event from an existing local "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        template_report, template_report_bytes, failure = (
            _load_bridge_template_index_entry_verification_summary_bridge_template_report(
                args.summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_json
            )
        )
        if failure is not None:
            ...
            try:
                ...
                report = _index_entry_verification_summary_bridge_template_index_entry_error_report(
                    "now_utc_invalid"
                )
            else:
    ...
    if (
        ...
        summary_report, failure = (
            _load_bridge_template_index_entry_verification_summary_report(
                args.summary_bridge_event_template_index_entry_verification_summary_json
            )
        )
        if failure is not None:
            report = _index_entry_verification_summary_bridge_template_error_report(
                failure
            )
        else:
            try:
    ...
    if args.verify_summary_bridge_event_template_index_entry_json is not None:
        if args.summary_bridge_event_template_source_json is None:
            report = _bridge_template_index_entry_verification_error_report(
                "summary_bridge_event_template_source_json_required"
            )
        else:
            index_entry, index_failure = _load_bridge_template_index_entry_report(
                args.verify_summary_bridge_event_template_index_entry_json
            )
            (
                template_report,
                template_report_bytes,
                template_failure,
            ) = _load_bridge_template_report_for_index_entry_verification(
                args.summary_bridge_event_template_source_json
            )
