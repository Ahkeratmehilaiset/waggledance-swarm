...
SCRIPT = (
    ROOT
    / "tools"
    / "verify_magma_decision_review_verification_template_index_entry.py"
)
COMMIT_SHA = "b" * 40
DECISION_REF = "bridge:operator-decision:pending-review"
FIXED_NOW = datetime(2026, 5, 29, 5, 5, tzinfo=timezone.utc)
PRIVATE_MARKERS = ("C:/private", "PRIVATE_", "http://", "https://")


def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_recomputes_digests_without_authority() -> None:
    ...
    assert report["release_ref"] == "pr:767"
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_rejects_digest_mismatch_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_rejects_missing_record() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_rejects_nested_authority_in_index() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_rejects_deterministic_entry_drift() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_rejects_source_contract_forgery() -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert (
        "source_contract_failed:bridge_event_template_source_contract_failed:"
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_missing_input_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert payload["blockers"] == [
        ...
        "index_entry_verification_failed:operator_decision_reference_review_bundle_"
        "verification_bridge_event_template_index_entry_unreadable"
    ]
    assert payload["direct_bridge_write_performed"] is False
    assert payload["artifact_payloads_included"] is False
    combined = result.stdout + result.stderr
...
def test_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verifier_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    artifacts = _artifact_set()
    index_entry = _index_entry(artifacts)
    ...
    assert payload["blockers"] == [
        "operator_decision_reference_review_bundle_verification_bridge_event_template_"
        "index_entry_verification_failed:operator_decision_reference_review_bundle_"
        "verification_bridge_event_template_index_entry_json_error"
    ]
    combined = result.stdout + result.stderr
...
def _artifact_set() -> dict[str, dict]:
    ...
    template = build_magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_bridge_event_template(
        ...
        task_id="wd-image1-decision-review-bundle-verification-template-index-verifier",
        to="operator,claude-rco-1",
        run_id="codex-lead-1-20260529T050000Z",
        session_id="codex-lead-1-20260529T050000Z",
        now_utc=datetime(2026, 5, 29, 5, 0, tzinfo=timezone.utc),
    )
    return {
        "summary": summary,
        "template": template,
...
def _summary() -> dict:
    return build_magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_summary(
        ...
        now_utc=datetime(2026, 5, 29, 4, 55, tzinfo=timezone.utc),
    )
...
def _write_bundle(
    ...
    paths = {
        "index_entry": tmp_path / "template_index_entry.json",
