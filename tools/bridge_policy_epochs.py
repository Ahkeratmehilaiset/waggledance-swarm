...
"""Derive the five admission epochs from explicit, verified evidence.
...
* **Derived, never accepted.** There is no code path that lets a caller hand
  ...
  callers looking at the same world get the same snapshot and a caller cannot
  assert an epoch it has not evidenced.
...
def _digest(value: Any) -> str | None:
    return value.lower() if isinstance(value, str) and _SHA256.match(value) else None
