...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_11_charter_violation_to_7.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR
    / "v0_expansion_2026_06_11_charter_violation_to_7_expectations.json"
)
LABEL = "charter_violation_to_7_2026_06_11"
...
def test_expansion_is_exactly_one_charter_violation_case():
    ...
    assert len(cases) == 1
    case = cases[0]
    assert case["case_id"] == "case:adv:charter_violation:007"
    assert case["defect_type"] == "charter_violation"
    assert {"standing_authorization", "scope_binding", "exact_head"} <= set(
        case["tags"]
    )
...
def test_canaries_unique_and_follow_naming():
    ...
    assert case["privacy_canary"] == "canary_charter_007_DO_NOT_LEAK"
...
def test_held_out_split_unchanged_and_valid():
    ...
    assert "case:adv:charter_violation:007" not in held_out
