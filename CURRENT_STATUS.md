...
**CI status:** 🟢 main push-event CI green from PR #67 merge `86cde948` onward; PR #71 + PR #73 + PR #74 + PR #75 + PR #76 + PR #77 + PR #78 + PR #79 + PR #80 + PR #81 + PR #82 PR-level CI green; Phase 18C PR will add an entry once opened. PR #67 added `fetch-depth: 0` to `actions/checkout` in both workflows; that fix carries forward.

### Phase 18C — Mined solver runtime dispatch (in flight, candidate prerelease)

Phase 18C closes the Phase 18B gap by wiring mined ALLOWLISTED specs through the real ControlPlaneDB / LowRiskSolverDispatcher path. Candidate tag: **`v3.10.2-mined-solver-dispatch-alpha`** PRERELEASE only.
