...
class MetaLearningEngine:
    ...
    async def weekly_analysis(self) -> dict:
        ...
        report = {
            ...
            "chat_metrics": self._analyze_metrics_log(days=7),  # D3
            "suggestions": [],
        }
    ...
    def _analyze_metrics_log(self, days: int = 7) -> dict:
        """Read data/learning_metrics.jsonl and compute weekly stats (D3).

        Returns dict with: total_queries, cache_hit_rate, avg_response_ms,
        hallucination_rate, route_breakdown, learning_events_count,
        top_methods, model_usage.

        Memory-safe: uses deque(maxlen=10000) to cap RAM usage on large files.
        """
        from collections import deque

        metrics_path = Path("data/learning_metrics.jsonl")
        if not metrics_path.exists():
            return {}

        cutoff = time.time() - days * 86400
        chat_records: deque = deque(maxlen=10000)
        learning_records: deque = deque(maxlen=5000)
        ...
        try:
            ...
            log.warning(f"Metrics log read error: {e}")
            return {}
        ...
        hallucinations = sum(1 for r in chat_records
                             if r.get("was_hallucination"))
        ...
        # Method breakdown (top 5)
        method_counts: dict = {}
        for r in chat_records:
            m = r.get("method", "unknown")
            method_counts[m] = method_counts.get(m, 0) + 1
        top_methods = sorted(method_counts.items(),
                             key=lambda x: x[1], reverse=True)[:5]

        # Model usage
        model_counts: dict = {}
        for r in chat_records:
            mdl = r.get("model_used") or "unknown"
            if mdl:
        ...
        return {
            ...
            "route_breakdown": dict(
                sorted(route_counts.items(), key=lambda x: x[1],
                       reverse=True)),
            "top_methods": dict(top_methods),
            "model_usage": model_counts,
            "learning_events_count": len(learning_records),
            "learning_events_breakdown": event_counts,
        }

    def generate_weekly_report(self) -> dict:
        """Generate standalone weekly report from learning_metrics.jsonl.

        Writes to data/weekly_report.json for dashboard consumption.
        Returns report dict.
        """
        metrics = self._analyze_metrics_log(days=7)
        report = {
            "timestamp": datetime.now().isoformat() + "Z",
            "type": "weekly_report",
            "chat_metrics": metrics,
            "memory": self._analyze_memory(),
            "hallucinations": self._analyze_hallucinations(),
            "learning_efficiency": self._analyze_learning_efficiency(),
        }

        # Write to data/weekly_report.json (overwrite, latest only)
        try:
            out = Path("data/weekly_report.json")
            out.parent.mkdir(parents=True, exist_ok=True)
            with open(out, "w", encoding="utf-8") as f:
                json.dump(report, f, ensure_ascii=False, indent=2, default=str)
            log.info(f"📊 Weekly report written: {out} "
                     f"({metrics.get('total_queries', 0)} queries, "
                     f"{metrics.get('avg_response_ms', 0)}ms avg)")
        except Exception as e:
            log.warning(f"Weekly report write failed: {e}")

        # Also append to metrics JSONL as a summary event
        try:
            from hivemind import StructuredLogger
            sl = StructuredLogger()
            sl.log_learning(
                event="weekly_report",
                count=metrics.get("total_queries", 0),
                extra={
