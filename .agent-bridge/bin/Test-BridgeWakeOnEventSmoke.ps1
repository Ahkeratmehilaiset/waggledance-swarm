...
try {
    ...
    Write-Host '7. replacement fails closed without waking:'
    Reset-State
    & $writeEvent -Agent codex -To operator -Type message -Status open `
    ...
    try {
        ...
        [System.IO.File]::WriteAllText(
            $replacement,
            (
                '{"agent":"codex","to":"operator","type":"message",' +
                '"task_id":"r23-smoke-7-replacement","value":"' +
                $padding + '"}' + [char]10
            ),
            $encoding
        )
        ...
        Add-Check 'replacement terminates watcher without wake' (
