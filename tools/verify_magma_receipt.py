...
_CHAT_ROUTE_STAGE_INDEX = {
    stage: index for index, stage in enumerate(_CHAT_ROUTE_STAGE_ORDER)
}
_CHAT_ROUTE_STAGES = frozenset(_CHAT_ROUTE_STAGE_ORDER)
_CHAT_ROUTE_TRACE_KEYS = frozenset(
    {
        "route_stage_trace",
...
def verify_manifest(
    ...
    *,
    ...
    manifest_entry_count = (
        len(raw_manifest_entries) if isinstance(raw_manifest_entries, list) else 0
    )
...
def _looks_like_chat_served(
    *,
    ...
    if isinstance(payload, dict):
        # These field names are valid generic MAGMA extensions, so presence
        # alone cannot classify a payload as chat. Match chat-specific values:
        # the exact fixed semantics declaration or a canonical digest binding
        # over a non-empty, ordered trace made only of known chat route stages.
        if payload.get("digest_semantics") == _CHAT_DIGEST_SEMANTICS:
            return True
        if _looks_like_bound_chat_route_trace(payload):
            return True
...
def _looks_like_bound_chat_route_trace(payload: dict[str, Any]) -> bool:
    if not _CHAT_ROUTE_TRACE_KEYS.issubset(payload):
        return False
    ...
    for entry in trace:
        ...
        if not isinstance(stage, str) or stage not in _CHAT_ROUTE_STAGES:
            return False
        stages.append(stage)
    if len(stages) != len(set(stages)):
        return False
    if stages != sorted(stages, key=_CHAT_ROUTE_STAGE_INDEX.__getitem__):
        return False
    try:
        expected_digest = sha256_digest({"route_stage_trace": trace})
    except (TypeError, ValueError, OverflowError):
        return False
    return payload.get("route_stage_trace_digest") == expected_digest
