...
@pytest.mark.parametrize("state", ["ok", "degraded"])
def test_composed_producer_attester_and_prefix_contract(subject_repo, state) -> None:
    """Synthetic fixture interoperability, not observed runtime duration."""
    ...
    repo, commit = subject_repo
    records = [json.loads(line) for line in _stream(commit).split("\n") if line]
    records[100].update(state=state, error_count=0, connection_errors=0)
    journal = "\n".join(json.dumps(record) for record in records) + "\n"
    _write_lf(repo / COVERAGE_REL, journal)

    report = _bound(repo, commit)
    report_path = repo.parent / "bound-audit.json"
    _write_lf(report_path, json.dumps(report) + "\n")
    attestation = evaluate_soak_log_source_attestation(
        report_path, repo, commit, require_fresh_contract=True
    )
    # Prefix integrity alone says nothing about record health or elapsed time.
    assert evaluate_soak_append_only(b"", journal.encode("utf-8")) == []
    if state == "ok":
        assert report["blockers"] == []
        assert attestation == []
    else:
...
def test_fresh_gap_parameter_cannot_exceed_policy_ceiling(subject_repo) -> None:
    from tools.release_soak_log_attestation import (
        evaluate_soak_log_source_attestation,
    )
    ...
    repo, commit = subject_repo
    _write_lf(repo / COVERAGE_REL, _stream(commit))
    assert "max_gap_hours_invalid" in _bound(repo, commit, max_gap_hours=25)["blockers"]
    report_path = repo.parent / "bound-audit.json"
    _write_lf(report_path, json.dumps(_bound(repo, commit)) + "\n")
    assert "soak_log_gap_policy_invalid" in evaluate_soak_log_source_attestation(
        report_path, repo, commit, max_gap_hours=25, require_fresh_contract=True
    )


def test_bound_current_directory_is_rejected_before_scan(subject_repo) -> None:
    repo, commit = subject_repo
    coverage = repo / COVERAGE_REL
    coverage.unlink()
    coverage.mkdir()
    report = _bound(repo, commit)
    assert f"current_source_not_regular:{COVERAGE_REL}" in report["blockers"]
    assert report["audit_result"] == "blocked"


@pytest.mark.parametrize(
    "invalid_kind", ["link", "directory", "hardlink", "reparse", "parent_reparse"]
)
def test_current_source_metadata_validation(tmp_path, monkeypatch, invalid_kind) -> None:
    import stat
    ...
    _write_lf(source, "{}\n")
    assert audit._regular_current_source(root, "journal.jsonl") is True
    original_lstat = Path.lstat

    def observed_stat(path):
        info = original_lstat(path)
        selected = root if invalid_kind == "parent_reparse" else source
        if path != selected:
            return info
        return SimpleNamespace(
            st_mode=(
                stat.S_IFLNK if invalid_kind == "link" else
                stat.S_IFDIR if invalid_kind == "directory" else info.st_mode
            ),
            st_nlink=2 if invalid_kind == "hardlink" else info.st_nlink,
            st_file_attributes=0x400 if "reparse" in invalid_kind else 0,
        )

    monkeypatch.setattr(Path, "lstat", observed_stat)
    assert audit._regular_current_source(root, "journal.jsonl") is False
