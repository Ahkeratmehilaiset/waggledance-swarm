"""Offline verifier tests for the hex cross-consistency index entry."""
...
import tools.verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry as mod


_FIXED = datetime(2026, 6, 21, 7, 45, 0, tzinfo=timezone.utc)
...
def _verify(entry: dict, report: dict) -> dict:
    return mod.verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry(
        index_entry=entry,
        bridge_event_template_report=report,
...
def test_verifier_rejects_authority_tamper() -> None:
    ...
    entry = _index_entry(report)
    tampered = copy.deepcopy(entry)
    tampered["runtime_subdivision_authority_granted"] = True

    verification = _verify(tampered, report)

    assert verification["ok"] is False
    assert (
        "index_entry_runtime_subdivision_authority_granted_not_false"
...
def test_verifier_rejects_source_template_contract_tamper() -> None:
    ...
    tampered_report["runtime_subdivision_authority_granted"] = True
...
def test_verifier_rejects_non_mapping_index_entry(bad_value) -> None:
    ...
    with pytest.raises(mod.HexIndexEntryVerificationError):
        mod.verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry(
