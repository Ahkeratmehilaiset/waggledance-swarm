...
"""governance_bypass expansion provenance fixture: schema + fold-in tests.

2026-06-10 expansion: governance_bypass was a joint-lowest critical
family at 3; now 7. The new cases encode real WD consensus-gate failure
modes (author-is-reviewer, veto override, head-drift carry-forward,
duplicate-identity quorum). Same discipline as the prior expansions: the
expansion fixture pair is provenance only; every case and paired
expectation must also exist in the strict ``v0`` fixture pair.
"""
...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_10_governance_bypass.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR / "v0_expansion_2026_06_10_governance_bypass_expectations.json"
)
LABEL = "governance_bypass_expansion_2026_06_10"
...
def test_expansion_is_exactly_four_governance_bypass_cases():
    cases = _read_json(EXPANSION)["cases"]
    assert len(cases) == 4
    assert all(c["defect_type"] == "governance_bypass" for c in cases)
    ids = [c["case_id"] for c in cases]
    assert ids == sorted(ids) and len(ids) == len(set(ids))
    assert all(":governance_bypass:" in case_id for case_id in ids)


def test_expansion_covers_distinct_consensus_gate_mechanisms():
    """Each case probes a distinct consensus-gate failure mode, distinct
    from the existing single-vote / stale-approval / spoofed-peer trio."""
    cases = _read_json(EXPANSION)["cases"]
...
def test_strict_v0_governance_bypass_coverage_raised():
    cases = _read_json(CORPUS_DIR / "v0.json")["cases"]
    count = sum(1 for c in cases if c["defect_type"] == "governance_bypass")
    assert count == 7
...
def test_every_expansion_case_has_paired_refuse_expectation():
    ...
    for exp in exps:
        ...
        assert "governance:blocking_bypass" in exp["expected_reason_codes"]
        assert "policy:single_vote_violation" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    family_canaries = [
        c["privacy_canary"]
        for c in all_cases
        if c["defect_type"] == "governance_bypass"
    ]
    ...
    for case in _read_json(EXPANSION)["cases"]:
        assert case["privacy_canary"].startswith("canary_governance_bypass_")
