...
def test_current_repo_matrix_is_valid_but_not_fresh_for_planning() -> None:
    ...
    report = mod.validate_matrix_freshness(text, now=date(2026, 7, 3))

    assert report["ok"] is True
    assert report["fresh_for_planning"] is False
    assert report["historical_stale_allowed"] is True
    assert report["freshness_audit_date"] == "2026-07-03"
    assert report["snapshot_age_days"] == 58
    assert report["priority_rows"] == ["G", "J", "L"]
    assert report["priority_rows_snapshot_date"] == "2026-05-06"
    assert report["priority_rows_snapshot_age_days"] == 58
    assert report["priority_rows_freshness_audit_date"] == "2026-07-03"
