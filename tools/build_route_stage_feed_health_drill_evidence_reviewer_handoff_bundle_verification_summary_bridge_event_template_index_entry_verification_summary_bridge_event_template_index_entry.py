...
"""Build a reviewer handoff verifier-summary bridge-template index entry."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T00:00:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json or report["ok"]:
        ...
        print(
            ...
            "bridge-template index entry FAILED: "
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    """Return a path-free local index entry for a verifier-summary template."""
    ...
    verification = _mapping(index_entry_verification_summary.get(VERIFICATION_KEY))
    ...
    entry = {
        ...
        "reviewer_next_actions": [
            "review_route_stage_reviewer_handoff_bundle_verifier_summary_bridge_event_template_index_entry",
...
def _assert_template_report_contract(
    ...
    *,
    ...
    if template_report.get("manual_review_required") is not True:
        raise TemplateIndexEntryError(
            "summary_bridge_event_template_manual_review_required_not_true"
        )
    _expect_empty_items(
    ...
    payload_verification = _mapping(payload.get(VERIFICATION_KEY))
    summary_verification = _mapping(index_entry_verification_summary.get(VERIFICATION_KEY))
    for field in (
    ...
    payload_boundary = _mapping(payload.get("operator_boundary"))
    summary_boundary = _mapping(index_entry_verification_summary.get("operator_boundary"))
    for field in (
        "verification_report_boundary_ok",
        "manual_review_required",
        *AUTHORITY_FALSE_FIELDS,
    ):
        if payload_boundary.get(field) != summary_boundary.get(field):
            raise TemplateIndexEntryError(
                f"summary_bridge_event_template_boundary_{field}_mismatch"
            )
    _expect_empty_items(
        payload_boundary.get("boundary_blockers"),
        "summary_bridge_event_template_boundary_blockers_present",
    )
