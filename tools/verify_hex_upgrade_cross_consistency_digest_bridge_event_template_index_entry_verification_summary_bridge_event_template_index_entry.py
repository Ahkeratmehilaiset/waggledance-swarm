...
"""Verify a hex xcons verifier-summary bridge-template index entry.

The verifier is local and inert. It recomputes the index entry from the
path-free verifier summary plus bridge-event template report, checks recorded
digest/size/schema metadata, and confirms the no-authority boundary. It does
not append bridge events, transport artifacts, include payloads, record local
paths, upgrade claims, skip gates, or grant runtime subdivision authority.
"""
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "hex-upgrade xcons verifier-summary bridge-event template index "
