...
"""Emit a local proof for fail-closed subdivision executor admission."""
...
REPORT_VERSION = "wd.hex_subdivision_runtime_executor_admission_proof.v0"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "hex subdivision runtime executor admission proof FAILED: "
            f"{exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if args.json:
        ...
        print(
            "hex subdivision runtime executor admission proof OK: "
            f"{report['proof_path']}"
        )
    else:
        print(
...
def build_hex_subdivision_runtime_executor_admission_proof(
    *,
    out_dir: Path,
    ...
    request = _execution_request(generated_at=generated_at)
...
def _execution_request(*, generated_at: datetime) -> dict[str, Any]:
    topology = _topology()
    preflight = build_subdivision_activation_preflight(
        ...
        plan=plan_subdivision(
            ...
            rationale="proof: dry-run executor admission for shadow children",
        ),
