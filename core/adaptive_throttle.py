...
class AdaptiveThrottle:
    ...
    async def benchmark(self, llm_provider, test_prompt="Sano 'OK'."):
        ...
        # Aseta alkuarvot koneen mukaan — aggressiiviset profiilit
        # GPU:ta EI saa jättää idle-tilaan: jokainen sekunti on oppimisaikaa
        profiles = {
            "fast": {
                "heartbeat_interval": 10,
                "max_concurrent": 6,
                "idle_every_n_heartbeat": 1,
                "idle_batch_size": 5,
            },
            "medium": {
                "heartbeat_interval": 15,
                "max_concurrent": 4,
                "idle_every_n_heartbeat": 2,
                "idle_batch_size": 4,
            },
            "slow": {
                "heartbeat_interval": 45,
                "max_concurrent": 2,
                "idle_every_n_heartbeat": 4,
                "idle_batch_size": 2,
            },
            "very_slow": {
                "heartbeat_interval": 120,
                "max_concurrent": 1,
                "idle_every_n_heartbeat": 8,
                "idle_batch_size": 1,
            },
        }
    ...
    async def benchmark_batch(self, consciousness=None, translation_proxy=None):
        ...
        # Only runs if Opus-MT is already loaded (avoids startup crash)
        if (translation_proxy and hasattr(translation_proxy, 'opus')
                and getattr(translation_proxy.opus, 'available', False)
                and getattr(translation_proxy.opus, '_model', None) is not None):
            test_texts_fi = [
    ...
    def _maybe_adjust(self):
        ...
        now = time.monotonic()
        if now - self._last_adjust < 10:  # Säädä max kerran per 10s
            return
        ...
        # Virheaste nolla + hyvä latenssi → nopeuta AGGRESSIIVISESTI
        # GPU idle = haaskattua oppimisaikaa
        if self.state.error_rate == 0 and len(self._latencies) >= 3:
            avg = self.state.avg_latency_ms
            if avg < 1000 and self.state.heartbeat_interval > 8:
                self._scale_up(f"Nopea vastaus ({avg:.0f}ms)")
            elif avg < 3000 and self.state.heartbeat_interval > 15:
                self._scale_up(f"Kohtuullinen vastaus ({avg:.0f}ms)")
            elif avg < 8000 and self.state.heartbeat_interval > 30:
                self._scale_up(f"OK vastaus ({avg:.0f}ms)")
