...
can verify that local operator decision-reference review bundle verification
bridge-event template index entry by recomputing `digest_checks`, size checks,
`schema_version_checks`, `source_contract_check`,
`rebuilt_index_entry_check`, and `bridge_event_schema_check` from explicit
local artifacts. The local operator decision-reference review bundle
verification bridge-event template index-entry verifier keeps
`artifact_payloads_included=false`, `local_paths_recorded=false`,
`transport_added=false`, `direct_bridge_write_performed=false`,
`approval_granted=false`, `release_decision_made=false`, and
`runtime_controls_added=false`.
...
can render that local index-entry verification summary as schema-valid bridge
handoff JSON. It carries the same `source_contract_check`,
`rebuilt_index_entry_check`, `bridge_event_schema_check`,
`decision_reference_verified`, and `template_only` context while keeping
`artifact_payloads_included=false`, `local_paths_recorded=false`,
`transport_added=false`, `direct_bridge_write_performed=false`,
`approval_granted=false`, `release_decision_made=false`, and
...
can build a local index entry for that index-entry verification summary
bridge-event template. The index entry records `template_index_entry`,
`bridge_event_schema_validated`, `source_contract_check`, and
`rebuilt_template_check` while keeping `artifact_payloads_included=false`,
`local_paths_recorded=false`, `transport_added=false`,
`direct_bridge_write_performed=false`, `approval_granted=false`,
...
is a local verifier for the operator decision-reference review bundle
verification bridge-event template index-entry verification summary
bridge-event template index entry. It recomputes digest, size, schema,
source-contract, rebuilt-index-entry, and bridge-event-schema checks from the
explicit local summary and bridge-event template artifacts while keeping
`artifact_payloads_included=false`, `local_paths_recorded=false`,
`transport_added=false`, `direct_bridge_write_performed=false`,
`approval_granted=false`, `release_decision_made=false`, and
`runtime_controls_added=false`; it does not append bridge events or make a
...
can build a local index entry for the operator decision-reference review bundle
verification bridge-event template index-entry verifier-summary bridge-event
template. The index entry records `template_index_entry`,
`bridge_event_schema_validated`, `source_contract_check`, and
`rebuilt_template_check` while keeping `artifact_payloads_included=false`,
`local_paths_recorded=false`, `transport_added=false`,
`direct_bridge_write_performed=false`, `approval_granted=false`,
