...
# ── Constants ────────────────────────────────────────────────────
# Support running from tools/ or from project root
...
TESTS = [
    ...
    # Phase 7 — Hologram / News / Wire regression (v3.5.7)
    {"file": "tests/test_phase7_hologram_news_wire.py", "name": "Hologram/News/Wire", "phase": "7-hnw", "args": [], "timeout": 60, "runner": "pytest"},
    ...
    # E2E Playwright gauntlet (400h campaign harness)
    {"file": "tests/e2e/",             "name": "E2E Gauntlet Harness",   "phase": "e2e",   "args": [], "timeout": 300, "runner": "pytest"},
]
...
def generate_ai_brief(
    ...
    lines.append("- v3.5.7 Phase 7: COMPLETE (HOLO-001, NEWS-001/002/003, WIRE-001 — Honest Hologram Release)")
    lines.append("- 400h Gauntlet: PHASE 1 COMPLETE (harness hardened, resumable, readiness-based)")
    lines.append("")

    lines.append("---")
    lines.append(f"*Generated by waggle_backup.py v9.1 | {now.strftime('%Y-%m-%d %H:%M')}*")
...
def load_manifest(backup_dir: Path) -> dict:
    ...
    if p.exists():
        try:
            ...
            pass
    # Fallback to persistent drives
    for loc in EXTRA_BACKUP_LOCATIONS:
        fp = loc / MANIFEST_FILE
        if fp.exists():
            try:
                data = json.loads(fp.read_text(encoding="utf-8"))
                # Restore to local backup dir for this session
                try:
...
def copy_to_extra_locations(zip_path: Path, meta_path: Path):
    ...
    for loc in EXTRA_BACKUP_LOCATIONS:
        ...
        try:
            ...
            # Copy manifest to extra backup location
            if manifest_src.exists():
                shutil.copy2(manifest_src, loc / MANIFEST_FILE)
            # Copy restore.bat to extra backup location
            restore_bat = PROJECT_ROOT / "restore.bat"
            if restore_bat.exists():
...
def print_header():
    print(f"\n{B}{'='*55}")
    print(f"  WAGGLEDANCE BACKUP & TEST v9.0")
...
# ── Main ─────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="WaggleDance Backup & Test Tool v9.0")
