# Release Readiness — WaggleDance (v3.8.0 stable + v3.9.0-producer-fabric-alpha PRERELEASE + v3.9.1-local-efficiency-benchmark-alpha PRERELEASE + v3.9.2-local-ollama-baseline-alpha PRERELEASE + v3.9.3-local-model-sweep-alpha CANDIDATE)
...
**Candidate prerelease (Phase 17D):** **v3.9.3-local-model-sweep-alpha** — to be tagged from the Phase 17D PR squash-merge SHA after PR-level CI green and `--match-head-commit`-protected merge. Phase 17D extends the Phase 17C single-model probe to a panel of 4 already-installed local models with 3 repeats per model. Adds `tools/run_phase17d_local_model_sweep.py` (~470 LOC), 13 targeted tests, `docs/benchmarks/LOCAL_OLLAMA_MODEL_SWEEP_2026.md`, and `docs/runs/phase17d_local_model_sweep_2026_05_05/` session folder. Host run: `release_gate_pass = true`, 360/360 prompts succeed (4 models × 3 repeats × 30 prompts), CoV across per-repeat medians 0.002–0.029, p50 panel spread 526.6–784.7 ms. Docker `--network none` PASS via `--skip-ollama --allow-no-ollama-track`. Honesty: no model pull, no cloud API call, no allowlist widening, no autonomy code change, no Stage-2 flip, no HUMAN_APPROVAL collected, no consciousness claim, no "beats all competitors" claim, no cross-vendor ranking implied.
