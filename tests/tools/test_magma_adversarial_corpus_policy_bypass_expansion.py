...
"""policy_bypass expansion provenance fixture: schema + fold-in tests.

2026-06-10 rollup-recommended expansion (stacked on the
correlated_review_trap expansion): policy_bypass was the lowest-coverage
family at 3; now 7. Same discipline as the prior expansions: the
...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_10_policy_bypass.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR / "v0_expansion_2026_06_10_policy_bypass_expectations.json"
)
LABEL = "policy_bypass_expansion_2026_06_10"
...
def test_expansion_is_exactly_four_policy_bypass_cases():
    cases = _read_json(EXPANSION)["cases"]
    assert len(cases) == 4
    assert all(c["defect_type"] == "policy_bypass" for c in cases)
    ids = [c["case_id"] for c in cases]
    assert ids == sorted(ids) and len(ids) == len(set(ids))
    assert all(":policy_bypass:" in case_id for case_id in ids)


def test_expansion_covers_distinct_bypass_mechanisms():
    """Each case probes a distinct policy-bypass mechanism, distinct from the
    existing replay-binding / version-downgrade / digest-branch trio."""
    cases = _read_json(EXPANSION)["cases"]
...
def test_strict_v0_policy_bypass_coverage_raised():
    cases = _read_json(CORPUS_DIR / "v0.json")["cases"]
    count = sum(1 for c in cases if c["defect_type"] == "policy_bypass")
    assert count == 7
...
def test_every_expansion_case_has_paired_refuse_expectation():
    ...
    for exp in exps:
        ...
        assert "policy:bypass_attempt_detected" in exp["expected_reason_codes"]
        assert "audit:integrity_required" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    family_canaries = [
        c["privacy_canary"]
        for c in all_cases
        if c["defect_type"] == "policy_bypass" and c.get("privacy_canary")
    ]
    ...
    for case in _read_json(EXPANSION)["cases"]:
        assert case["privacy_canary"].startswith("canary_policy_bypass_")
