...
"""Build a local index entry for the hex verifier-summary bridge template."""
...
from tools.build_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry import (  # noqa: E402
    DIGEST_ARTIFACT_ID as SOURCE_DIGEST_ARTIFACT_ID,
    INDEX_ENTRY_VERSION as SOURCE_INDEX_ENTRY_VERSION,
    IndexEntryError as TemplateIndexEntryError,
...
INDEX_ENTRY_VERSION = (
    ...
    "verification_summary_bridge_event_template_index_entry.v1"
)
PROOF_ID = (
...
_SOURCE_VERIFICATION_ARTIFACT_IDS = (
    SOURCE_DIGEST_ARTIFACT_ID,
    SOURCE_TEMPLATE_ARTIFACT_ID,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T10:00:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = _failure_report(
            "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
            "verification_summary_bridge_event_template_index_entry_invalid"
        )
    ...
    if args.json or report["ok"]:
        ...
        print(
            "hex verifier-summary bridge-event-template index entry FAILED: "
...
def build_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    verification = _mapping(
        index_entry_verification_summary.get(HEX_VERIFICATION_KEY)
    )
    ...
    artifacts = [
        _artifact_record(
            ...
            schema_version=SOURCE_SUMMARY_VERSION,
        ),
        _artifact_record(
            ...
            schema_version=SOURCE_TEMPLATE_VERSION,
        ),
    ]
    entry = {
        "proof_id": PROOF_ID,
        ...
        "template_index_entry": {
            ...
            "source_summary_proof_id": SOURCE_SUMMARY_PROOF_ID,
        ...
        "verification_summary": {
            ...
            "source_index_entry_version": _safe_ref_or_invalid(
                verification.get("index_entry_version")
            ),
            "source_index_entry_version_match": (
                verification.get("index_entry_version") == SOURCE_INDEX_ENTRY_VERSION
            ),
        ...
        "reviewer_next_actions": [
            "review_hex_cross_consistency_verifier_summary_bridge_event_template_index_entry",
...
def _assert_summary_contract(summary: Mapping[str, Any]) -> None:
    if summary.get("ok") is not True:
        raise TemplateIndexEntryError("index_entry_verification_summary_not_ok")
    if summary.get("proof_id") != SOURCE_SUMMARY_PROOF_ID:
        raise TemplateIndexEntryError("index_entry_verification_summary_proof_mismatch")
    if summary.get("summary_version") != SOURCE_SUMMARY_VERSION:
        raise TemplateIndexEntryError(
            "index_entry_verification_summary_version_mismatch"
        )
    if summary.get("template_only") is not True:
        raise TemplateIndexEntryError(
            "index_entry_verification_summary_template_only_not_true"
        )
    if summary.get("manual_review_required") is not True:
        raise TemplateIndexEntryError(
            "index_entry_verification_summary_manual_review_not_true"
        )
    if summary.get("blockers") != []:
        raise TemplateIndexEntryError("index_entry_verification_summary_blockers")
    _expect_false_fields(summary, "index_entry_verification_summary")

    verification = _mapping(summary.get(HEX_VERIFICATION_KEY))
    if verification.get("verification_ok") is not True:
        raise TemplateIndexEntryError("index_entry_verification_not_ok")
    if verification.get("verification_version") != SOURCE_VERIFICATION_VERSION:
        raise TemplateIndexEntryError("index_entry_verification_version_mismatch")
    ...
    if verification.get("artifact_count_checked") != len(
        ...
        raise TemplateIndexEntryError("index_entry_verification_artifact_count_mismatch")
    for field in (
        ...
        if verification.get(field) != "match":
            raise TemplateIndexEntryError(f"index_entry_verification_{field}_not_match")
    if verification.get("template_only") is not True:
        raise TemplateIndexEntryError("index_entry_verification_template_only_not_true")
    if verification.get("blocker_count") != 0 or verification.get("blockers") != []:
        raise TemplateIndexEntryError("index_entry_verification_blockers_present")
    _expect_false_fields(verification, "index_entry_verification")
    ...
    if boundary.get("verification_report_boundary_ok") is not True:
        raise TemplateIndexEntryError("operator_boundary_verification_report_not_ok")
    if boundary.get("boundary_blockers") != []:
        raise TemplateIndexEntryError("operator_boundary_blockers_present")
    if boundary.get("manual_review_required") is not True:
        raise TemplateIndexEntryError("operator_boundary_manual_review_not_true")
    _expect_false_fields(boundary, "operator_boundary")
...
def _assert_template_report_contract(
    ...
    *,
    ...
    if template_report.get("proof_id") != SOURCE_TEMPLATE_PROOF_ID:
        raise TemplateIndexEntryError("summary_bridge_event_template_proof_mismatch")
    ...
    if template_report.get("blockers") != []:
        raise TemplateIndexEntryError("summary_bridge_event_template_blockers")
    _expect_false_fields(template_report, "summary_bridge_event_template")
    ...
    _expect_false_fields(payload, "summary_bridge_event_template_payload")
    _expect_false_fields(
        _mapping(payload.get("operator_boundary")),
        "summary_bridge_event_template_boundary",
    )

    verification = _mapping(payload.get(HEX_VERIFICATION_KEY))
    if verification.get("verification_ok") is not True:
        raise TemplateIndexEntryError(
    ...
    if verification.get("verification_version") != SOURCE_VERIFICATION_VERSION:
        raise TemplateIndexEntryError(
            "summary_bridge_event_template_verification_version_mismatch"
        )
    if verification.get("index_entry_version") != SOURCE_INDEX_ENTRY_VERSION:
        raise TemplateIndexEntryError(
            "summary_bridge_event_template_source_index_entry_version_mismatch"
        )
    if verification.get("artifact_count_checked") != len(
        _SOURCE_VERIFICATION_ARTIFACT_IDS
    ):
        raise TemplateIndexEntryError(
            "summary_bridge_event_template_artifact_count_mismatch"
        )
    for field in (
        "source_contract_check",
        "rebuilt_index_entry_check",
        ...
        if verification.get(field) != "match":
            raise TemplateIndexEntryError(
                f"summary_bridge_event_template_{field}_not_match"
            )
    _expect_false_fields(verification, "summary_bridge_event_template_verification")
    for check_name in ("digest_checks", "size_checks", "schema_version_checks"):
        checks = _mapping(verification.get(check_name))
        for artifact_id in _SOURCE_VERIFICATION_ARTIFACT_IDS:
            if checks.get(artifact_id) != "match":
                raise TemplateIndexEntryError(
                    f"summary_bridge_event_template_{check_name}_{artifact_id}_not_match"
                )
...
def _artifact_record(
    *,
    ...
    return {
        ...
        "source_proof_id": _safe_ref_or_invalid(artifact.get("proof_id")),
...
def _false_output_axes() -> dict[str, bool]:
    return {field: False for field in _OUTPUT_FALSE_FIELDS}


def _expect_false_fields(payload: Mapping[str, Any], prefix: str) -> None:
...
def _safe_ref_or_invalid(value: Any) -> str:
    return _safe_token(value, fallback="invalid_ref")
...
def _assert_no_forbidden_output(text: str) -> None:
    ...
    if found:
        raise TemplateIndexEntryError(
            "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
            "verification_summary_bridge_event_template_index_entry_contains_forbidden_markers:"
