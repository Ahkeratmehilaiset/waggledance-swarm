...
$savedOwnerSessionId = $env:AGENT_BRIDGE_OWNER_SESSION_ID
$savedOwnerToken = $env:AGENT_BRIDGE_OWNER_TOKEN
$savedOwnerPid = $env:AGENT_BRIDGE_OWNER_PID
$savedOwnerProcessStartUtc = $env:AGENT_BRIDGE_OWNER_PROCESS_START_UTC
...
try {
    ...
    if ($null -ne $savedOwnerSessionId) { $env:AGENT_BRIDGE_OWNER_SESSION_ID = $savedOwnerSessionId } else { Remove-Item Env:AGENT_BRIDGE_OWNER_SESSION_ID -ErrorAction SilentlyContinue }
    if ($null -ne $savedOwnerToken) { $env:AGENT_BRIDGE_OWNER_TOKEN = $savedOwnerToken } else { Remove-Item Env:AGENT_BRIDGE_OWNER_TOKEN -ErrorAction SilentlyContinue }
    if ($null -ne $savedOwnerPid) { $env:AGENT_BRIDGE_OWNER_PID = $savedOwnerPid } else { Remove-Item Env:AGENT_BRIDGE_OWNER_PID -ErrorAction SilentlyContinue }
    if ($null -ne $savedOwnerProcessStartUtc) { $env:AGENT_BRIDGE_OWNER_PROCESS_START_UTC = $savedOwnerProcessStartUtc } else { Remove-Item Env:AGENT_BRIDGE_OWNER_PROCESS_START_UTC -ErrorAction SilentlyContinue }
