...
"""Verify a Memory Palace visualization bridge-template index entry.

The verifier is local and inert. It recomputes the index entry from the
source verification summary and bridge-event template, checks recorded
digests/sizes/schema versions, and confirms the review-only authority
boundary. It does not append bridge events, enqueue scheduler work, dispatch
...
VERIFICATION_VERSION = (
    ...
    "verification_summary_bridge_event_template_index_entry_verification.v0"
)
...
_REQUIRED_FALSE_FIELDS = (*AUTHORITY_FALSE_FIELDS, *EXTRA_TEMPLATE_FALSE_FIELDS)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = _failure_report(
            "memory_palace_visualization_export_verification_summary_"
            "bridge_event_template_index_entry_verification_invalid"
        )
    ...
    if args.json or report["ok"]:
        ...
        print(
            "Memory Palace visualization bridge-template index entry "
            "verification FAILED: "
...
def verify_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    """Recompute artifact checks for the visualization bridge-template index."""
    ...
    for artifact_id, artifact in (
        ...
        (SUMMARY_ARTIFACT_ID, verification_summary),
    ...
    artifacts = {
        SUMMARY_ARTIFACT_ID: (verification_summary, verification_summary_bytes),
        TEMPLATE_ARTIFACT_ID: (
...
def _collect_boundary_blockers(
    ...
    if index_entry.get("read_side_report_only") is not True:
        blockers.append("read_side_report_only_not_true")
    ...
    if index_entry.get("controls_present") is not False:
        blockers.append("controls_present_not_false")
...
def _collect_template_entry_blockers(
    ...
    *,
    ...
    counts = _mapping(verification_summary.get("counts"))
    for field in (
        "node_count_checked",
        "edge_count_checked",
        "shortcut_edge_count_checked",
    ):
        if template_entry.get(field) != counts.get(field):
            blockers.append(f"template_index_entry_{field}_mismatch")
    summary_checks = _mapping(verification_summary.get("checks"))
    if template_entry.get("summary_payload_check_count") != len(summary_checks):
        blockers.append("template_index_entry_summary_payload_check_count_mismatch")
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "Memory Palace visualization bridge-event template index entry "
