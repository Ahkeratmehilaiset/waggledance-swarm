...
def test_build_demo_pack_writes_verified_evidence(tmp_path: Path) -> None:
    ...
    assert result["a4_dispatch_success_count"] == 21
    assert result["a4_dispatch_case_count"] == 21
    ...
    assert "A4 dispatch success: `21/21`" in summary
...
def test_cli_json_reports_demo_pack(tmp_path: Path) -> None:
    ...
    assert payload["a4_dispatch_success_count"] == 21
