...
"""Verify a hex verifier-summary bridge-template index entry."""
...
VERIFICATION_VERSION = (
    ...
    "verification_summary_bridge_event_template_index_entry_verification.v1"
)
VERIFICATION_PROOF_ID = (
    "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
    "verification_summary_bridge_event_template_index_entry_verification_v1"
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = _failure_report(
            "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
            "verification_summary_bridge_event_template_index_entry_verification_invalid"
        )
    ...
    if args.json or report["ok"]:
        ...
        print(
            "hex verifier-summary bridge-template index entry verification FAILED: "
...
def verify_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry(
    *,
    ...
    """Recompute checks for the verifier-summary bridge-template index entry."""
    ...
    _collect_verification_summary_blockers(index_entry, blockers)
    ...
    report = {
        ...
        **{field: False for field in _AUTHORITY_FALSE_FIELDS},
...
def _collect_verification_summary_blockers(
    ...
    summary = _mapping(index_entry.get("verification_summary"))
    if summary.get("verification_ok") is not True:
    ...
    for field in (
        ...
        if summary.get(field) != "match":
            blockers.append(f"verification_summary_{field}_not_match")
    if summary.get("source_index_entry_version_match") is not True:
        blockers.append("verification_summary_source_index_entry_version_mismatch")
    if summary.get("blocker_count") != 0:
        blockers.append("verification_summary_blocker_count_nonzero")
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "hex verifier-summary bridge-event template index entry verification "
