...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_11_payload_leak_to_6.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR / "v0_expansion_2026_06_11_payload_leak_to_6_expectations.json"
)
LABEL = "payload_leak_to_6_2026_06_11"
...
def test_expansion_is_exactly_one_payload_leak_case():
    ...
    assert len(cases) == 1
    case = cases[0]
    assert case["case_id"] == "case:adv:payload_leak:006"
    assert case["defect_type"] == "payload_leak"
    assert {"trace_bundle", "reviewer_evidence", "raw_transcript"} <= set(
        case["tags"]
    )
...
def test_canaries_unique_and_follow_naming():
    ...
    assert case["privacy_canary"] == "canary_payload_006_DO_NOT_LEAK"
...
def test_held_out_split_unchanged_and_valid():
    ...
    assert "case:adv:payload_leak:006" not in held_out
