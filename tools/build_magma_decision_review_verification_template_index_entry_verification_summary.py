...
_SAFE_SCHEMA_VERSION_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9:._-]{0,255}$")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--template-index-entry-verification-json",
        "--verification-json",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(render_template_index_entry_verification_summary_markdown(summary))
    else:
        print(
...
def build_magma_decision_review_verification_template_index_entry_verification_summary(
    *,
    ...
    """Return path-free reviewer context for a template index-entry verifier."""
    ...
    summary = {
        ...
        "operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification": {
            ...
            "verification_version": _safe_schema_version_or_invalid(
                verification_report.get("verification_version")
            ),
            "index_entry_version": _safe_schema_version_or_invalid(
        ...
        "reviewer_next_actions": [
            "review_template_index_entry_verification_blockers",
            "compare_template_index_entry_verification_to_local_artifacts",
...
def render_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# MAGMA Template Index-Entry Verification Summary",
        "",
        ...
        "",
        "## Template Index-Entry Verification",
        "",
...
def _safe_schema_version_or_invalid(value: Any) -> str:
    return (
        value
        if isinstance(value, str)
        and _SAFE_SCHEMA_VERSION_RE.match(value)
        and not _forbidden_output_markers(value)
        else "invalid_ref"
    )
