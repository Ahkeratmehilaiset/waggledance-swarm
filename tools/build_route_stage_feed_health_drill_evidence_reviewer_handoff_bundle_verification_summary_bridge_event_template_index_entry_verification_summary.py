...
"""Render a path-free route-stage handoff-bundle template index verifier summary."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-19T07:20:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            summary = build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary(
    ...
    if args.json:
        ...
        print(
            "route-stage handoff-bundle template index-entry verification "
            "summary FAILED: "
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary(
    *,
    ...
    """Return path-free reviewer context for a local index-entry verifier."""
    ...
    report_warnings = _safe_warning_token_list(verification_report.get("warnings"))
    ...
    contract_blockers.extend(
        _token_list_schema_blockers(verification_report, "warnings")
    )
    ...
    summary = {
        ...
        "reviewer_next_actions": [
            "review_route_stage_handoff_bundle_template_index_entry_verification_summary",
            "compare_index_entry_verification_summary_to_local_artifacts",
...
def render_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# Route-Stage Handoff Bundle Template Index-Entry Verification Summary",
        "",
...
def _failure_summary(reason: str) -> dict[str, Any]:
    return {
        ...
        "blockers": [
            "route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_"
            "verification_summary_bridge_event_template_index_entry_"
            f"verification_summary_failed:{_safe_token(reason)}"
        ],
        "warnings": [],
    }
...
def _token_list_schema_blockers(report: Mapping[str, Any], field: str) -> list[str]:
    ...
    for item in value:
        if not isinstance(item, str):
            ...
            field == "warnings"
            and _safe_warning_token(item) == "invalid_warning_token"
        ):
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "route-stage handoff-bundle template index-entry verification "
            "summary contains forbidden markers"
        )
