...
loadFeeds();
    ...
    try:
        _register_hologram_routes(app, hivemind)
    except Exception as e:
        log.warning(f"Hologram routes not loaded: {e}")
...
def _register_hologram_routes(app, hivemind):
    """Register hologram brain visualization endpoints."""
    from pathlib import Path as _Path
    from fastapi.responses import HTMLResponse as _HR

    _html_path = _Path(__file__).resolve().parent / "hologram-brain-v5.html"
    _html_cache: dict = {}  # mutable container for lazy cache

    def _get_html() -> str:
        if "v" not in _html_cache:
            _html_cache["v"] = _html_path.read_text(encoding="utf-8")
        return _html_cache["v"]

    @app.get("/hologram", response_class=_HR)
    async def hologram_view():
        return _HR(_get_html())

    @app.get("/api/hologram/state")
    async def hologram_state():
        """Synthesize hologram brain state from runtime stats."""
        rt = getattr(hivemind, 'autonomy_runtime', None)
        if not rt:
            # Try alternative attribute names
            rt = getattr(hivemind, '_autonomy_runtime', None)

        if not rt or not getattr(rt, 'is_running', False):
            return _hologram_zero_state()

        return _hologram_from_runtime(rt, hivemind)


def _hologram_zero_state():
    """Return zero-activation hologram state."""
    return {
...
def _hologram_from_runtime(rt, hivemind):
    """Build hologram payload from a live AutonomyRuntime."""

    def _clamp(v, lo=0.0, hi=1.0):
        return max(lo, min(hi, v))

    def _safe_stats(attr):
    ...
    rk = _safe_stats("resource_kernel")
    wm_stats = _safe_stats("world_model")
    cb_stats = _safe_stats("case_builder")
    wmem_stats = _safe_stats("working_memory")
    sr_stats = _safe_stats("solver_router")
    vf_stats = _safe_stats("verifier")

    cc = getattr(rt, "capability_confidence", None)
    ...
    events = []
    for edge_key, _ in sorted_edges[:4]:
