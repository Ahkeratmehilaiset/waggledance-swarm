...
# ---------------------------------------------------------------------------
# Axis V2 x E3 integration regression (2026-09-09)
# ---------------------------------------------------------------------------


def test_explicit_pip_selection_leaves_axis_attestation_unchanged(
    tmp_path,
) -> None:
    """The E3 explicit selection must not disturb the Axis attestation gate.

    Both fail-closed gates read the SAME rebuilt ``expected`` object, so their
    independence is a real invariant and not a coincidence: the axis fields are
    derived from the evidence root alone, while an explicit selection only pins
    which security artifact is audited. This asserts that empirically through
    the REAL collector rebuild -- never a stubbed ``expected`` -- and proves the
    explicit recovery actually executed rather than failing closed, which would
    have made the comparison vacuous.
    """
    root, head, evidence_root = _axis_env(tmp_path, axis_a=False)
    release_notes = _rt_release_notes(tmp_path)
    # Two candidates so pinning one is meaningful. Both stay vulnerable so the
    # security attestation is blocked on either choice and cannot itself become
    # the reason the axis blockers move.
    _rt_pip_report(evidence_root, _RT_PIP_OLD, vulns=1)
    _rt_pip_report(evidence_root, _RT_PIP_NEW, vulns=1)

    def _run(tag, **explicit):
        rebuilt = build_soak_evidence(
            "docs/release/RELEASE_READINESS.md",
            commit=head,
            ended_at_utc=dt.datetime(2026, 5, 22, 12, 0, tzinfo=dt.UTC),
            use_local_artifacts=True,
        ...
        actual = dict(rebuilt)
        actual["axis_a_regression"] = "pass"
        soak_evidence = tmp_path / f"v3.12.0_{tag}.json"
        soak_evidence.write_text(json.dumps(actual), encoding="utf-8")
        report = build_report(
            soak_evidence=soak_evidence,
            release_readiness="docs/release/RELEASE_READINESS.md",
    ...
    plain_rebuilt, plain = _run("plain")
    explicit_rebuilt, explicit = _run("explicit", pip_audit_report=_RT_PIP_OLD)

    # The explicit recovery really ran: the collector recorded the pinned
    # basis, and the verifier neither rejected the recorded path nor collapsed
    # the whole rebuild into a single unbuildable blocker.
    selection = explicit_rebuilt["artifact_selection"]["pip_audit_report"]
    assert selection["basis"] == "explicit"
    assert selection["path"].endswith(_RT_PIP_OLD)
    for report in (plain, explicit):
        assert not any(
            item.startswith("explicit_selection_invalid:")
            for item in report["blockers"]
        )
        assert not any(
            item.startswith("expected_evidence_unbuildable")
            for item in report["blockers"]
        )

    # Axis A is claimed pass with its canonical artifact deliberately absent
    # (an ordinary missing-evidence fixture), so the named blocker must be
    # present -- by MEMBERSHIP, since unrelated blockers legitimately coexist.
    for report in (plain, explicit):
        assert "axis_a_report_unreadable" in report["blockers"]

    # The intended envelope S is the fixture commit, not the live checkout:
    # Axis B carries a valid artifact built against that same fixture commit,
    # so a wrong envelope would surface a named axis_b blocker.
    assert head != COMMIT
    assert plain_rebuilt["axis_b_gate"] == "pass"
    for report in (plain, explicit):
        assert not any(
            item.startswith("axis_b_") for item in report["blockers"]
        )

    # The invariant itself, on both sides of the boundary: identical axis
    # blockers, and identical rebuilt axis fields from the two real collector
    # builds (the report carries no ``expected``, so compare the builds).
    def _axis_blockers(report):
        return sorted(
            item for item in report["blockers"] if item.startswith("axis_")
        )

    assert _axis_blockers(plain) == _axis_blockers(explicit)
    for field in ("axis_a_regression", "axis_b_gate"):
        assert plain_rebuilt[field] == explicit_rebuilt[field]
