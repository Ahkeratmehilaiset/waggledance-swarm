...
<#
  Integrity-loaded library: one managed Lead, one owned app-server thread.
...
function Get-WdConversationThreadParameters {
    ...
    return @{ model=$Model; cwd=$Worktree; approvalPolicy='never'; sandbox='workspace-write'
        config=@{model_reasoning_effort=$Effort; sandbox_workspace_write=@{writable_roots=@($RuntimeRoot)}} }
}
...
function Remove-WdConversationArtifacts {
    ...
    $finished=@(Get-ChildItem -LiteralPath $Journal -File | Where-Object { $_.Name -match '^turn-[0-9a-f]{32}\.(checkpointed|interrupted|transport-ready|deferred)\.json$' } | Sort-Object LastWriteTimeUtc -Descending)
...
function Start-WdConversationTurn {
    param($Context, [string] $Text, [object[]] $Attachments = @(), [string] $ActionId = '', [bool] $Automatic = $false, [string] $WakePath = '')
    if ($Context.active -or $Context.requests.Count -gt 0 -or $Context.hold) { throw 'conversation is not ready for a new turn' }
    ...
    if ($Context.turnCount -eq 0 -and $Context.image) { $inputItems+=@{type='localImage';path=$Context.image} }
    $base=if ($Context.turnCount -eq 0) { $Context.startup } else { $Context.continuation }
    ...
    $Context.spec=$spec; $Context.prefix=$prefix; $Context.started=$started
    ...
    Send-WdConversationRpc $Context 'turn/start' @{threadId=$Context.thread;input=$inputItems;model=$Context.model;effort=$Context.effort;approvalPolicy='never'
        sandboxPolicy=@{type='workspaceWrite';writableRoots=@($Context.worktree,$Context.runtime);networkAccess=$false;excludeTmpdirEnvVar=$false;excludeSlashTmp=$false}} $ActionId
...
function Complete-WdConversationTurn {
    ...
    $Context.active=''; $Context.lastTurn=[DateTimeOffset]::UtcNow
    if ($status -ceq 'interrupted') {
        ...
        $Context.owner.pending_path=$null; $Context.owner.status='waiting'; $Context.owner.last_disposition='interrupted_reconciliation_required'
        Write-WdTurnOwner $Context.pointer $Context.ownerPath $Context.owner
    ...
    if ($status -cne 'completed') { Set-WdConversationHold $Context 'blocked_native_turn_failed' $false; return }
    try {
        ...
        Write-WdTurnOwner $Context.pointer $Context.ownerPath $Context.owner
        Remove-WdConversationArtifacts $Context.journal
    } catch { Set-WdConversationHold $Context ('blocked_checkpoint: '+$_.Exception.Message) $false }
}
    ...
    if (-not $method -and $null -ne $id) {
        ...
        switch ($request.method) {
            'initialize' {
                ...
                $p=Get-WdConversationThreadParameters $Context.worktree $Context.runtime $Context.model $Context.effort
...
function Invoke-WdCodexConversationLoop {
    ...
    if ($Agent -cne 'codex-lead-1' -or $Backend -cne 'codex' -or $Model -cne 'gpt-5.6-sol' -or $Effort -cne 'ultra') { throw 'conversation lane pins mismatch' }
    ...
    try {
        ...
        $owner=[ordered]@{schema='wd.lane-turn-owner.v1';agent=$Agent;session_id=$SessionId;generation=$Generation;pid=$PID
            ...
            worktree=$worktreeFull;journal_root=$journal;pending_path=$boot;cli_permission_posture='workspace-write_never'}
        ...
        $thread=''; $automatic=$true; $identity=Join-Path $journal 'conversation.json'
        ...
        $view=New-WdOperatorConversationView -Headless:$Headless
        ...
        $c=@{native=$native;view=$view;journal=$journal;pointer=$pointer;ownerPath=$ownerPath;owner=$owner;epoch=$epoch;sequence=0
            ...
            image=if($thread){''}else{$ImagePath};lastTurn=[DateTimeOffset]::MinValue;turnBytes=[long]0;spec=$null;prefix='';started=[DateTimeOffset]::UtcNow
            stopping=$false;deadline=[DateTimeOffset]::MaxValue;turnTimeout=$TurnTimeoutSeconds;actions=@{};nextWakeAttempt=[DateTimeOffset]::MinValue;heartbeat=[DateTimeOffset]::UtcNow}
        if ($thread) { $c.turnCount=1 }
        ...
        while (-not $close -and ($MaxIterations -eq 0 -or $iteration -lt $MaxIterations)) {
            ...
            foreach ($action in @(Get-WdOperatorConversationActions -View $view)) {
                ...
                if ($kind -ceq 'interrupt' -and $c.active) {
                    ...
                    if ($c.active) {
                        ...
                        try { Start-WdConversationTurn $c $text $attachments $actionId }
                        catch {
                    ...
                    Add-WdOperatorConversationMessage -View $view -Role user -Text $text
                }
            }
            if (-not $close -and -not $c.hold -and $c.ready -and $c.automatic -and -not $c.active -and $c.requests.Count -eq 0 -and [DateTimeOffset]::UtcNow -ge $c.nextWakeAttempt) {
            ...
            $status=if ($c.hold) { $c.hold+'; automatic dispatch disabled; pending evidence retained. Reconcile journal before restart.' } elseif ($c.active) { 'Lead working; Send steers this turn. Interrupt pauses automation.' } elseif (-not $c.ready) { 'Connecting owned Lead conversation...' } else { 'Lead ready. Native conversation, not a workflow-completion verdict.' }
            Set-WdOperatorConversationStatus -View $view -Text $status -CanSend:($c.ready -and -not $c.hold -and $c.requests.Count -eq 0) -CanInterrupt:([bool]$c.active -and -not $c.hold -and $c.requests.Count -eq 0) -CanToggleAutomation:($c.ready -and -not $c.hold) -AutomationEnabled:([bool]$c.automatic) -TurnActive:([bool]$c.active) -OwnerEpoch $epoch -ObservedTurnId $c.active
        ...
        if (-not $c.hold) { $owner.status='stopped'; Write-WdTurnOwner $pointer $ownerPath $owner }
