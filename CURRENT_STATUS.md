...
**In-flight prerelease candidate (Phase 18B):** **v3.10.1-gap-miner-feedback-alpha** — to be tagged from the Phase 18B PR squash-merge SHA after PR-level CI green and `--match-head-commit`-protected merge. Phase 18B closes the runtime-gap-mining feedback half of the autonomous learning loop on main: structured runtime gap signals → six-element verdict enum (`ALLOWLISTED_SOLVER_SPEC`, `INSUFFICIENT_EVIDENCE`, `OUT_OF_FAMILY_REJECTED`, `HIGH_RISK_REJECTED`, `BUILDER_HANDOFF_QUARANTINED`, `DUPLICATE_SUPPRESSED`). New mainline modules at `waggledance/core/autonomy_growth/{gap_mining,gap_candidate}.py`, proof harness at `tools/run_phase18b_gap_miner_feedback_proof.py`, 19 unit tests, `docs/benchmarks/GAP_MINER_FEEDBACK_LOOP_2026.md`. Host run: 30 synthetic signals → 14 candidates (6 allowlisted + 3 insufficient + 2 out-of-family + 1 high-risk + 1 builder-handoff + 1 duplicate); 6 deterministic solver specs; `release_gate_pass = true`; Docker `--network none` PASS; Phase 18A bundle continues to validate (carry-forward gate green); `provider_jobs_delta = builder_jobs_delta = 0`. Builder handoff is a quarantined contract with `no_auto_promotion = true`; no live builder execution. Axis M on the competitive evidence matrix upgrades to "PROVEN with measured runtime-gap feedback loop within six-family allowlist". Also fixes a Phase 18A bundle EOL portability defect surfaced in P0: the exporter now writes LF-only bytes via `_write_text_lf`/`_copy_text_lf`, plus a new `.gitattributes` declares the bundle subtree `text eol=lf`, so SHA-256 checksums verify on a fresh Windows checkout. v3.8.0 remains GitHub Latest; v3.9.0 + v3.9.1 + v3.9.2 + v3.9.3 + v3.10.0 + v3.10.1 alphas all Pre-release.
**Shipped branch:** `main` at `2d32b9b2...` (Phase 18A post-release docs PR #80 squash-merge); previous head `4554b24a` was the Phase 18A core merge. Earlier history: Phase 17D post-release docs PR #78 at `b9dd6b7d`; Phase 17D core PR #77 at `d0704efe`; Phase 17C post-release docs PR #76 at `4f8a9ea7`; Phase 17C core PR #75 at `db5d7db1`; Phase 17B post-release docs PR #74 at `27b8175`; Phase 17B core PR #73 at `f4d0a4a4`; Phase 17A squash-merge of [PR #71](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/71) at `c726995c`; Phase 16G post-stable CI fix at `48ac0b3` (PR #70) and `86cde94` (PR #67); Phase 16F squash-merge of [PR #68](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/68) at `824176eb` (= v3.8.0 tag target); Phase 16D squash-merge of [PR #66](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/66) at `7210a7e`; Phase 16C–A squash-merges; Phase 15 PR #62 at `2b9978d`; Phase 10 PR #54 at `08b7e8c` on top of `8bf1869` (post-v3.6.0 truthfulness commit) on top of `a1c4152` (PR #51 Phase 9 squash).
**All tags (most-recent first):** [`v3.10.0-benchmark-schema-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.10.0-benchmark-schema-alpha) Pre-release · [`v3.9.3-local-model-sweep-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.3-local-model-sweep-alpha) Pre-release · [`v3.9.2-local-ollama-baseline-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.2-local-ollama-baseline-alpha) Pre-release · [`v3.9.1-local-efficiency-benchmark-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.1-local-efficiency-benchmark-alpha) Pre-release · [`v3.9.0-producer-fabric-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.0-producer-fabric-alpha) Pre-release · [`v3.8.0`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.8.0) **Latest** · [`v3.7.8-docker-gate-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.7.8-docker-gate-alpha) Pre-release. v3.8.0 not moved or retagged by Phase 17A / 17B / 17C / 17D / 18A / 18B.
**CI status:** 🟢 main push-event CI green from PR #67 merge `86cde948` onward; PR #71 + PR #73 + PR #74 + PR #75 + PR #76 + PR #77 + PR #78 + PR #79 + PR #80 PR-level CI green; Phase 18B PR will add an entry once opened. PR #67 added `fetch-depth: 0` to `actions/checkout` in both workflows; that fix carries forward.

### Phase 18B — Runtime gap miner + solver feedback loop (in flight, candidate prerelease)
