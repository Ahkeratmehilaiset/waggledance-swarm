...
"""tool-argument-abuse tail-floor expansion provenance fixture tests."""
...
def test_expansion_is_exactly_one_tool_argument_abuse_case():
...
def test_every_expansion_case_has_paired_refuse_expectation():
    ...
    for exp in exps:
        ...
        assert "tool:argument_constraint" in exp["expected_reason_codes"]
        assert "path:scope_escape" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    family_canaries = [
        c["privacy_canary"]
        for c in all_cases
        if c["defect_type"] == "tool_argument_abuse"
    ]
