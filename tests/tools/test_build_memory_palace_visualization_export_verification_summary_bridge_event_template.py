...
SCRIPT = (
    ROOT
    / "tools"
    / "build_memory_palace_visualization_export_verification_summary_bridge_event_template.py"
)
...
def test_visualization_summary_bridge_event_template_validates_schema() -> None:
    report = build_memory_palace_visualization_export_verification_summary_bridge_event_template(
        ...
        run_id="codex-tools-1-20260608T203000Z",
        session_id="codex-tools-1-20260608T203000Z",
        now_utc=datetime(2026, 6, 8, 20, 30, tzinfo=timezone.utc),
    )
    ...
    summary = payload["memory_palace_visualization_export_verification_summary"]
...
def test_visualization_summary_bridge_event_template_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
    ...
    result = _run(
        ...
        "--run-id",
        "codex-tools-1-20260608T203000Z",
        "--session-id",
        "codex-tools-1-20260608T203000Z",
        "--now",
        "2026-06-08T20:30:00Z",
        "--json",
    )
...
def test_visualization_summary_bridge_event_template_missing_input_is_path_free() -> None:
    result = _run(
        "--summary-json",
...
def test_visualization_summary_bridge_event_template_rejects_unsafe_bridge_fields() -> None:
    report = build_memory_palace_visualization_export_verification_summary_bridge_event_template(
        summary=_valid_summary(),
        agent_id="Codex",
        task_id=(
...
def test_visualization_summary_bridge_event_template_blocks_unsafe_summary_contract() -> None:
    cases: tuple[tuple[str, Callable[[dict], None], str], ...] = (
        ...
        (
            "source_verification_not_ok",
            lambda summary: summary.__setitem__("source_verification_ok", False),
            "verification_summary_source_verification_not_ok",
        ),
        (
            "blocker_present",
        ...
        (
            "count_invalid",
            lambda summary: summary["counts"].__setitem__(
                "node_count_checked",
                -1,
            ),
            "verification_summary_node_count_checked_invalid",
        ),
    )
...
def test_visualization_summary_bridge_event_template_rejects_duplicate_json_keys_path_free(
    tmp_path: Path,
) -> None:
...
def test_visualization_summary_bridge_event_template_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    summary_path = tmp_path / "verification-summary.json"
...
def _valid_summary() -> dict[str, object]:
    ...
    return build_memory_palace_visualization_export_verification_summary(
        verification,
    )
