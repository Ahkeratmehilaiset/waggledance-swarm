...
The measurement-only hex-upgrade cross-consistency digest then checks that the
reviewer summary, shadow-only invariant, and chain final summary agree, and a
local template index entry summary binds that digest bridge-event template by
derived booleans and digest refs only. It does not include raw payloads, record
local paths, append bridge events, transport artifacts, upgrade claims, or
activate runtime subdivision authority.
