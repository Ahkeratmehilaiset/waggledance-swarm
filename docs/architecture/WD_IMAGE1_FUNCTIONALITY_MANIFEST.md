...
| Deterministic solver-first routing | Partial, with opt-in receipt binding proof plus default-off runtime receipt coverage counters and Prometheus metrics | `waggledance/core/reasoning/solver_router.py`, `waggledance/core/autonomy/runtime.py`, `waggledance/adapters/http/routes/metrics.py`, `docs/architecture/HONEYCOMB_SOLVER_SCALING.md` | Wire a default-off operator-configured runtime summary receipt sink from settings into AutonomyRuntime without changing default receipt emission. |
