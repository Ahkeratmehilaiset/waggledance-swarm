...
6. **Head-exact binding** — all required, non-waived approvals bind to the exact
   ...
   implemented in the current gate code**: `verify_bridge_consensus` and
   `check_rco_pass_present` still bind strictly to the exact head SHA. Until an
   reviewed gate-code implementation lands, every re-push requires approval
   re-posts at the new head, even for a content-identical base rebase.
