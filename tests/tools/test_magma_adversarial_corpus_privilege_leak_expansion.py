...
"""privilege_leak expansion provenance fixture: schema + fold-in tests.

2026-06-10 rollup-recommended expansion (stacked on the policy_bypass
expansion): privilege_leak was the lowest-coverage family at 3; now 7.
Same discipline as the prior expansions: the expansion fixture pair is
...
EXPANSION = CORPUS_DIR / "v0_expansion_2026_06_10_privilege_leak.json"
EXPANSION_EXPECTATIONS = (
    CORPUS_DIR / "v0_expansion_2026_06_10_privilege_leak_expectations.json"
)
LABEL = "privilege_leak_expansion_2026_06_10"
...
def test_expansion_is_exactly_four_privilege_leak_cases():
    cases = _read_json(EXPANSION)["cases"]
    assert len(cases) == 4
    assert all(c["defect_type"] == "privilege_leak" for c in cases)
    ids = [c["case_id"] for c in cases]
    assert ids == sorted(ids) and len(ids) == len(set(ids))
    assert all(":privilege_leak:" in case_id for case_id in ids)


def test_expansion_covers_distinct_privilege_mechanisms():
    """Each case probes a distinct privilege-leak mechanism, distinct from
    the existing scope-grant / scout-role / execution-replay trio."""
    cases = _read_json(EXPANSION)["cases"]
    tag_sets = [set(c["tags"]) for c in cases]
    mechanisms = [
        {"over_broad_grant"},
        {"transitive_grant"},
        {"advisory_to_authority"},
        {"grant_not_revoked"},
    ]
...
def test_strict_v0_privilege_leak_coverage_raised():
    cases = _read_json(CORPUS_DIR / "v0.json")["cases"]
    count = sum(1 for c in cases if c["defect_type"] == "privilege_leak")
    assert count == 7
...
def test_every_expansion_case_has_paired_refuse_expectation():
    ...
    for exp in exps:
        ...
        assert "scope:read_only_violation" in exp["expected_reason_codes"]
        assert "grant:unauthorized" in exp["expected_reason_codes"]
...
def test_canaries_unique_and_follow_naming():
    ...
    family_canaries = [
        c["privacy_canary"]
        for c in all_cases
        if c["defect_type"] == "privilege_leak" and c.get("privacy_canary")
    ]
    ...
    for case in _read_json(EXPANSION)["cases"]:
        assert case["privacy_canary"].startswith("canary_privilege_leak_")
