...
2. If `feedback_kind="broken_route"`: triggers an L40 negative-tunnel mining pass for the (from_cell, to_solver) pair.
...
1. **Event schema**: required fields event_type, feedback_id, feedback_kind, query_class_hash, operator_id, priority, submitted_at_utc.
