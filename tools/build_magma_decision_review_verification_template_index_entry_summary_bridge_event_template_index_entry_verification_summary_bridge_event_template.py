...
"""Build a MAGMA verifier-summary bridge-event template."""
...
TEMPLATE_VERSION = "magma_alert_feed_reviewer_handoff_bundle_operator_decision_reference_review_bundle_verification_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template.v1"
...
class ContractError(ValueError):
    """Raised when the local verifier summary is not template-safe."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        except (SafeInputError, ContractError) as exc:
            report = _failure_report(exc.code)
        except ValueError:
            report = _failure_report(
                "operator_decision_reference_review_verifier_summary_bridge_event_template_invalid"
            )
    ...
    if args.json:
        ...
        print(
            ...
            "bridge-event template FAILED: " + ", ".join(report["blockers"]),
            file=sys.stderr,
        )
    return 0 if report["ok"] else 1


def build_magma_decision_review_verification_template_index_entry_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template(
    *,
    ...
    if not isinstance(summary, Mapping):
        raise ContractError("verifier_summary_not_mapping")
    _assert_no_forbidden_input("verifier_summary", summary)
    ...
    event = {
        ...
        "message": (
            ...
            "recording, runtime controls, merge, promotion, or release decision."
        ),
        "paths": [],
        "write_scope": [],
        "run_id": run_id,
...
def _load_json_report(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise SafeInputError("verifier_summary_unreadable") from exc
    ...
    try:
        ...
        raise SafeInputError("verifier_summary_decode_error") from exc
    except json.JSONDecodeError as exc:
        raise SafeInputError("verifier_summary_json_error") from exc
    except ValueError as exc:
        raise SafeInputError("verifier_summary_json_error") from exc
    if not isinstance(parsed, Mapping):
        raise SafeInputError("verifier_summary_not_mapping")
    return parsed
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "sanitized operator decision-reference review verifier-summary "
