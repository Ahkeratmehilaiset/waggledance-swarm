...
def test_restore_to_temp(zip_path: Path, r: CheckResult) -> bool:
    ...
    try:
        ...
        # Check key files
        for f in ["hivemind.py", "main.py", "start_waggledance.py", "requirements.txt"]:
            if (tmp / f).is_file():
        ...
        if agents_dir.is_dir():
            ...
            if agent_count >= 74:
                ...
                r.warn("Agents", f"Only {agent_count} agents (expected 76)")
...
def main():
    ...
    if fail > 0:
        ...
        print(f"\n  {G}Environment fully ready. Start WaggleDance with: python start_waggledance.py{W}")
        sys.exit(0)
