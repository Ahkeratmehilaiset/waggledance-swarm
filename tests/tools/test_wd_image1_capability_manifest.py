...
def test_manifest_embeds_hexagonal_upgrade_proof_without_upgrading_claim() -> None:
    ...
    # next_smallest_pr advanced beyond the now-wired template-only bridge-event
    # renderer to the next honest step: a local index entry for that template.
    ...
    assert "index-entry verifier-result summary" in capability["next_smallest_pr"]
    assert "local index entry" in capability["next_smallest_pr"]
    assert "verifier-summary bridge-event template" in capability["next_smallest_pr"]
    assert "template-only bridge-event renderer" not in capability["next_smallest_pr"]
    assert "reviewer summary renderer" not in capability["next_smallest_pr"]
    assert "local verifier" not in capability["next_smallest_pr"]
...
def test_manifest_stores_hex_xcons_template_index_entry_verification_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
    ...
    for key, value in summary.items():
        if key == "report_version":
            ...
            assert isinstance(value, bool), key
    assert summary["verification_available"] is True
    assert summary["template_only"] is True
    assert summary["source_contract_match"] is True
    assert summary["rebuilt_index_entry_match"] is True
    assert summary["bridge_event_schema_match"] is True
    assert summary["digest_ref_match"] is True
    assert summary["all_digest_checks_match"] is True
    assert summary["all_size_checks_match"] is True
    assert summary["all_schema_version_checks_match"] is True
    assert summary["no_runtime_authority_granted"] is True
    assert summary["no_runtime_subdivision_authority_granted"] is True
    assert summary["no_direct_bridge_write"] is True
    assert summary["no_bridge_event_written"] is True
...
def test_manifest_hex_xcons_template_index_entry_verification_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    src = inspect.getsource(mod._capabilities)
    ok_assign = src.split('hex_upgrade_proof["ok"] = bool(', 1)[1].split(")", 1)[0]
    assert "cross_consistency_digest_bridge_event_template_index_entry_verification" not in (
        ok_assign
    )
    index_idx = src.index(
        'hex_upgrade_proof[\n'
    ...
    verification_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification"'
    )
    assert verification_idx > index_idx


def test_manifest_hex_xcons_template_index_entry_verification_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
    ...
    forged = {
        ...
        "template_only": False,
        "source_contract_match": False,
        "rebuilt_index_entry_match": False,
        "bridge_event_schema_match": False,
        "digest_ref_match": False,
        "all_digest_checks_match": False,
        "all_size_checks_match": False,
        "all_schema_version_checks_match": False,
        "no_runtime_authority_granted": False,
        "no_runtime_subdivision_authority_granted": False,
        "no_direct_bridge_write": False,
        "no_bridge_event_written": False,
...
def test_manifest_stores_hex_xcons_template_index_entry_verification_summary_bridge_event_template_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
    ...
    assert summary["template_only"] is True
    assert summary["bridge_event_schema_validated"] is True
    assert summary["summary_version_match"] is True
    assert summary["verification_summary_available"] is True
    assert summary["source_contract_match"] is True
    assert summary["rebuilt_index_entry_match"] is True
    assert summary["bridge_event_schema_match"] is True
    assert summary["digest_ref_match"] is True
...
def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    summary_key = (
        "cross_consistency_digest_bridge_event_template_index_entry_"
        "verification_summary_bridge_event_template"
    )
    assert isinstance(proof.get(summary_key), dict)
    assert proof.get("ok") is True
    import inspect
    from tools import wd_image1_capability_manifest as mod
    ...
    src = inspect.getsource(mod._capabilities)
    ok_assign = src.split('hex_upgrade_proof["ok"] = bool(', 1)[1].split(")", 1)[0]
    assert summary_key not in ok_assign
    verification_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification"'
    )
    summary_template_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template"'
    )
    assert summary_template_idx > verification_idx


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
    ...
    assert proof.get("ok") == baseline_ok
    assert proof.get(summary_key) == forged


def test_manifest_stores_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
    ...
    assert summary["rebuilt_template_match"] is True
    assert summary["verification_summary_available"] is True
    assert summary["source_index_entry_version_match"] is True
    assert summary["all_artifact_digests_recorded"] is True
    assert summary["no_runtime_authority_granted"] is True
    assert summary["no_runtime_subdivision_authority_granted"] is True
    assert summary["no_direct_bridge_write"] is True
    assert summary["no_bridge_event_written"] is True
    ...
    blob = json.dumps(summary)
    assert str(ROOT) not in blob
    assert "created_at_utc" not in blob
    assert "sha256" not in blob


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    summary_key = (
        "cross_consistency_digest_bridge_event_template_index_entry_"
        "verification_summary_bridge_event_template_index_entry"
    )
    assert isinstance(proof.get(summary_key), dict)
    assert proof.get("ok") is True
    import inspect
    ...
    summary_template_index_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry"'
    )
    assert summary_template_index_idx > summary_template_idx


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
    ...
    forged = {
        ...
        "rebuilt_template_match": False,
        "verification_summary_available": False,
        "source_index_entry_version_match": False,
        "all_artifact_digests_recorded": False,
        "no_runtime_authority_granted": False,
        "no_runtime_subdivision_authority_granted": False,
        "no_direct_bridge_write": False,
        "no_bridge_event_written": False,
...
def test_manifest_stores_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
...
def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    summary_template_index_verification_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification"'
    )
    assert summary_template_index_verification_idx > summary_template_index_idx


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
...
def test_manifest_stores_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
    ...
    assert summary["template_only"] is True
    assert summary["verification_report_boundary_ok"] is True
...
def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    summary_key = (
        "cross_consistency_digest_bridge_event_template_index_entry_"
        "verification_summary_bridge_event_template_index_entry_verification_summary"
    )
    assert isinstance(proof.get(summary_key), dict)
    assert proof.get("ok") is True
    import inspect
    ...
    summary_template_index_verification_summary_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary"'
    )
    assert (
        summary_template_index_verification_summary_idx
        > summary_template_index_verification_idx
    )


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
...
def test_manifest_stores_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_content_safe() -> None:
    manifest = build_manifest()
    hex_cap = next(
        capability
        for capability in manifest["capabilities"]
...
def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_measurement_only() -> None:
    proof = next(
        capability["proof"]
        for capability in build_manifest()["capabilities"]
        if capability["capability_id"] == "hexagonal_upgrades"
    ...
    ok_assign = src.split('hex_upgrade_proof["ok"] = bool(', 1)[1].split(")", 1)[0]
    assert summary_key not in ok_assign
    verification_summary_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary"'
    )
    verification_summary_template_idx = src.index(
        'hex_upgrade_proof[\n'
        '        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template"'
    )
    assert verification_summary_template_idx > verification_summary_idx


def test_manifest_hex_xcons_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary_bridge_event_template_cannot_flip_ok(
    monkeypatch,
) -> None:
    baseline_ok = next(
        capability["proof"].get("ok")
