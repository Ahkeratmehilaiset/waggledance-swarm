...
"""Build a hex xcons verifier-summary bridge-event template.

The template is reviewer context for an already-local verification summary. It
never appends bridge events, transports payloads, records local paths, upgrades
...
_ARTIFACT_IDS = (TEMPLATE_ARTIFACT_ID,)
...
_NESTED_FALSE_FIELDS = frozenset(_SUMMARY_FALSE_FIELDS)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T09:20:00Z.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "hex-upgrade xcons verifier-summary bridge-event template FAILED: "
    ...
    *,
    ...
    payload = {
        ...
        HEX_XCONS_VERIFICATION_KEY: {
            ...
            "source_template_version": _safe_ref_or_invalid(
                verification.get("source_template_version")
            ),
        ...
        "reviewer_next_actions": [
            "review_hex_xcons_template_index_entry_verification_summary_bridge_event_template",
    ...
    event = {
        ...
        "message": (
            ...
            "automatic_release_decision=false; claim_safe=false; "
            "template_only=true; no bridge write, transport, external fetch, "
            "payload inclusion, local path recording, runtime controls, claim "
            "upgrade, gate skip, merge, promotion, release decision, or "
...
def _assert_no_forbidden_output(text: str) -> None:
    if _contains_path_marker(text) or _forbidden_output_markers(text):
        raise ValueError(
            "hex-upgrade verifier-summary bridge-event template contains "
            "forbidden markers"
        )
