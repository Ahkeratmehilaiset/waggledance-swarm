...
"""Render a path-free hex cross-consistency index-entry verifier summary."""
...
_SUMMARY_EXTRA_FALSE_FIELDS = (
    "automatic_release_decision",
    "controls_present",
    "external_writes_applied",
    "network_access_performed",
)
_SUMMARY_FALSE_FIELDS = (*_SUMMARY_EXTRA_FALSE_FIELDS, *AUTHORITY_FALSE_FIELDS)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-20T09:00:00Z.",
    )
    ...
    try:
        ...
        try:
            ...
            summary = _failure_summary(
                ...
                "index_entry_verification_summary_invalid"
            )
    ...
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification": {
            ...
            "source_template_version": _safe_ref_or_invalid(
                verification_report.get("source_template_version")
            ),
            ...
            "literal_future_claim_safe": verification_report.get(
            ...
            is True,
            "direct_bridge_write_performed": verification_report.get(
                "direct_bridge_write_performed"
            )
            is True,
            "transport_added": verification_report.get("transport_added") is True,
            "bridge_event_written": verification_report.get("bridge_event_written")
            is True,
            "artifact_payloads_included": verification_report.get(
                "artifact_payloads_included"
            )
            is True,
            "local_paths_recorded": verification_report.get("local_paths_recorded")
            is True,
        ...
        "reviewer_next_actions": [
            "review_hex_upgrade_cross_consistency_template_index_entry_verification_summary",
...
def render_hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# Hex Cross-Consistency Index-Entry Verification Summary",
        "",
        ...
        f"- Source template version: `{verification.get('source_template_version')}`",
    ...
    lines.extend(
        [
            ...
            "include payloads, record local paths, upgrade claims, skip gates, "
            "or grant runtime subdivision authority.",
            "",
        ]
    )
    markdown = "\n".join(lines)
...
def _boundary_blockers(report: Mapping[str, Any]) -> list[str]:
    ...
    if report.get("artifact_payloads_included") is not False:
        blockers.append("verification_report_artifact_payloads_included_not_false")
    if report.get("local_paths_recorded") is not False:
        blockers.append("verification_report_local_paths_recorded_not_false")
    if report.get("path_free_verified") is not True:
        blockers.append("verification_report_path_free_verified_not_true")
...
def _assert_no_forbidden_output(text: str) -> None:
    if _contains_path_marker(text) or _forbidden_output_markers(text):
        raise ValueError(
            "hex-upgrade cross-consistency index-entry verification summary "
            "contains forbidden markers"
        )
