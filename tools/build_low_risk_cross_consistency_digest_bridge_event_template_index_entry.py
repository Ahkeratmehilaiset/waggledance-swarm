...
"""Build a local index entry for the low-risk cross-consistency digest bridge-event template.

Advances the WD Image #1 *low-risk autonomy loop* pillar. PR #1291 added the
``build_low_risk_cross_consistency_digest_bridge_event_template`` renderer and #1294
wired its summary into the manifest. This tool records that template-only bridge-event
report in a path-free LOCAL INDEX ENTRY binding it to stable digests + selected
safe-scalar verdicts - the next step in the established bridge-event-template chain
(template -> index entry -> verifier), mirroring the route-stage / operator-feedback
index-entry pattern.

It is READ-ONLY and grants no authority. It never appends bridge events, transports,
enqueues scheduler work, writes files, calls providers, mutates runtime, grants
runtime authority, or skips gates. Content-safe by construction: it emits ONLY derived
booleans / strict ints / fixed version+schema strings / sha256 digest refs - never the
raw event message/ts_utc/payload. It validates the source template against its contract
(including ``validate_event``), refuses a template that self-declares ``claim_safe`` /
``approval_granted``, requires every authority axis strictly False, and is fail-closed
on malformed / path-tainted / contract-violating input. ``claim_safe`` is hardcoded
False.

Exact validation commands::

    python tools/build_low_risk_cross_consistency_digest_bridge_event_template_index_entry.py \\
        --bridge-event-template-json <template.json> --json
    python -m pytest \\
        tests/test_build_low_risk_cross_consistency_digest_bridge_event_template_index_entry.py -q
...
INDEX_ENTRY_VERSION = (
    "wd.low_risk_cross_consistency_digest_bridge_event_template_index_entry.v1"
)
PROOF_ID = "low_risk_cross_consistency_digest_bridge_event_template_index_entry_v1"
TEMPLATE_ARTIFACT_ID = "low_risk_cross_consistency_digest_bridge_event_template"

# every authority/side-effect axis the index entry asserts strictly False.
...
# the source template report's top-level authority axes that must each be strictly False.
...
# the template payload's authority_boundary axes that must each be strictly False.
...
def build_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
    *,
    ...
    # surface the cross-consistency verdicts the template carries (strict bool).
    ...
    entry: dict[str, Any] = {
        ...
        "artifacts": [
            {
                "artifact_id": TEMPLATE_ARTIFACT_ID,
                "role": "verified_no_authority_low_risk_cross_consistency_template",
        ...
        "reviewer_next_actions": [
            "review_low_risk_cross_consistency_digest_bridge_event_template_index_entry",
    ...
    errors = validate_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
        entry
    )
    if errors:
...
def validate_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
...
def _assert_template_contract(template_report: Mapping[str, Any]) -> Mapping[str, Any]:
    """Validate the source template report contract; return its cross_consistency section."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = build_low_risk_cross_consistency_digest_bridge_event_template_index_entry(
    ...
    if args.json or report["ok"]:
        ...
        print(
            "low-risk cross-consistency digest bridge-template index entry FAILED: "
...
def _failure_report(reason: str) -> dict[str, Any]:
    report: dict[str, Any] = {
        ...
        "manual_review_required": True,
        "blockers": [
            "low_risk_cross_consistency_digest_bridge_event_template_index_entry_failed:"
            + _safe_reason(reason)
        ],
        "warnings": [],
    }
