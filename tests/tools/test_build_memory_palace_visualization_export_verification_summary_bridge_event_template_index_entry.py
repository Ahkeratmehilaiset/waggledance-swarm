...
SCRIPT = (
    ROOT
    / "tools"
    / "build_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry.py"
)
FIXED_NOW = datetime(2026, 6, 8, 21, 45, tzinfo=timezone.utc)
...
def test_visualization_summary_bridge_event_template_index_entry_ties_digests_without_authority() -> None:
    artifacts = _artifact_set()
    raw = _artifact_bytes(artifacts)

    entry = build_memory_palace_visualization_export_verification_summary_bridge_event_template_index_entry(
    ...
    assert entry["created_at_utc"] == "2026-06-08T21:45:00Z"
    ...
    assert template_entry["node_count_checked"] == 6
    assert template_entry["edge_count_checked"] == 5
    assert template_entry["shortcut_edge_count_checked"] == 2
...
def test_visualization_summary_bridge_event_template_index_entry_cli_json_is_path_free(
    tmp_path: Path,
) -> None:
...
def test_visualization_summary_bridge_event_template_index_entry_rejects_template_drift() -> None:
    artifacts = _artifact_set()
    ...
    summary_payload["counts"]["node_count_checked"] = 999
    raw = _artifact_bytes(artifacts)
...
def test_visualization_summary_bridge_event_template_index_entry_rejects_authority_escalation() -> None:
    artifacts = _artifact_set()
...
def test_visualization_summary_bridge_event_template_index_entry_rejects_unexpected_summary_fields() -> None:
    artifacts = _artifact_set()
    artifacts["summary"] = deepcopy(artifacts["summary"])
    artifacts["summary"]["source_export_ok"] = False
    raw = _artifact_bytes(artifacts)
    ...
    try:
        ...
        assert getattr(exc, "code", "") == "verification_summary_unexpected_field_present"
    else:  # pragma: no cover
        raise AssertionError("expected unexpected summary field rejection")


def test_visualization_summary_bridge_event_template_index_entry_missing_input_is_path_free(
    tmp_path: Path,
) -> None:
    paths = _write_artifacts(tmp_path, _artifact_set())
...
def test_visualization_summary_bridge_event_template_index_entry_non_finite_json_is_path_free(
    tmp_path: Path,
) -> None:
    paths = _write_artifacts(tmp_path, _artifact_set())
...
def test_visualization_summary_bridge_event_template_index_entry_duplicate_json_key_is_path_free(
    tmp_path: Path,
) -> None:
    paths = _write_artifacts(tmp_path, _artifact_set())
...
def _artifact_set() -> dict[str, dict]:
    ...
    template = build_memory_palace_visualization_export_verification_summary_bridge_event_template(
        ...
        task_id=(
            "codex-tools-1/"
            "memory-palace-visualization-summary-template-index-entry-20260608"
        ),
        to="operator,codex-lead-1,claude-rco-1,claude-rco-2",
        run_id="codex-tools-1-20260608T214500Z",
        session_id="codex-tools-1-20260608T214500Z",
        now_utc=FIXED_NOW,
    )
