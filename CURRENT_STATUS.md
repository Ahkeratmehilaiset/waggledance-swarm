...
**Latest prerelease (Phase 17A):** **v3.9.0-producer-fabric-alpha** (released 2026-05-04T18:32:47Z from PR #71 squash-merge `c726995c`). Pre-release entry on GitHub; v3.8.0 remains GitHub Latest.
**Released prerelease (Phase 17B):** **v3.9.1-local-efficiency-benchmark-alpha** (released 2026-05-04T20:59:09Z from PR #73 squash-merge `f4d0a4a4`). Pre-release entry on GitHub. Adds a reproducible benchmark harness that aggregates the existing Phase 11–17A canonical proofs into a single artifact with the master-prompt-mandated metric set; documents NOT_RUN slots for six external competitors; introduces an optional Ollama latency probe (default skipped for `--network none` safety). On the host run: `release_gate_pass = true`, capability-lookup p50/p95/p99 = 4.33 / 10.98 / 14.39 ms, 1000/1000 hits, 0/0 deltas. Raw intelligence vs frontier MoE is **NOT CLAIMED**.
**In-flight prerelease candidate (Phase 17C):** **v3.9.2-local-ollama-baseline-alpha** — to be tagged from the Phase 17C PR squash-merge SHA after PR-level CI green and `--match-head-commit`-protected merge. Upgrades the Phase 17B optional Ollama track from `SKIPPED_OPTIONAL` to a `MEASURED` 30-prompt deterministic baseline against one already-installed local model (`gemma4:e4b`, ollama 0.22.1). Host run: `release_gate_pass = true`, `prompts_succeeded = 30 / 30`, `median_latency_seconds = 0.7866`, `p95_latency_seconds = 17.5538`, `mean_latency_seconds = 2.5539`. No model pull or download; no cloud API calls; no allowlist widening; no autonomy code change; no Stage-2 flip. v3.8.0 remains GitHub Latest; v3.9.0 + v3.9.1 alphas remain Pre-release.
**Shipped branch:** `main` at `27b8175...` (Phase 17B post-release docs PR #74 squash-merge); previous head `f4d0a4a4` was the Phase 17B core merge. Earlier history: Phase 17A squash-merge of [PR #71](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/71) at `c726995c`; Phase 16G post-stable CI fix at `48ac0b3` (PR #70) and `86cde94` (PR #67); Phase 16F squash-merge of [PR #68](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/68) at `824176eb` (= v3.8.0 tag target); Phase 16D squash-merge of [PR #66](https://github.com/Ahkeratmehilaiset/waggledance-swarm/pull/66) at `7210a7e`; Phase 16C–A squash-merges; Phase 15 PR #62 at `2b9978d`; Phase 10 PR #54 at `08b7e8c` on top of `8bf1869` (post-v3.6.0 truthfulness commit) on top of `a1c4152` (PR #51 Phase 9 squash).
**All tags (most-recent first):** [`v3.9.1-local-efficiency-benchmark-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.1-local-efficiency-benchmark-alpha) Pre-release · [`v3.9.0-producer-fabric-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.9.0-producer-fabric-alpha) Pre-release · [`v3.8.0`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.8.0) **Latest** · [`v3.7.8-docker-gate-alpha`](https://github.com/Ahkeratmehilaiset/waggledance-swarm/releases/tag/v3.7.8-docker-gate-alpha) Pre-release. v3.8.0 not moved or retagged by Phase 17A / 17B / 17C.
**CI status:** 🟢 main push-event CI green from PR #67 merge `86cde948` onward; PR #71 + PR #73 + PR #74 PR-level CI green; Phase 17C PR will add an entry once opened. PR #67 added `fetch-depth: 0` to `actions/checkout` in both workflows; that fix carries forward.

### Phase 17C — Local Ollama baseline (in flight, candidate prerelease)

Phase 17C upgrades the Phase 17B optional Ollama track from `SKIPPED_OPTIONAL` to `MEASURED` for one already-installed local model. Candidate tag: **`v3.9.2-local-ollama-baseline-alpha`** PRERELEASE only.
