...
def test_corpus_is_versioned_and_complete(corpus: dict):
    ...
    assert isinstance(corpus.get("refuse_cases"), list) and len(corpus["refuse_cases"]) >= 5
...
def test_refuse_case_is_refused_by_gate(case: dict, tmp_path: Path):
    ...
    head = case["head"]
    rco_agent = case.get("rco_agent", "claude-rco-1")
    expected = case["expected"]

    # Library path (direct events list, no FS)
    result = check_rco_pass_present(
        events=events, task_id=task_id, head=head, rco_agent=rco_agent
    )
    assert result["ok"] is expected["ok"]
    assert result["decision"] == expected["decision"]
    assert result["has_qualifying_rco_pass_at_head"] is expected["has_qualifying_rco_pass_at_head"]
    ...
    for key in CLAIM_GATES:
        assert result[key] is False, f"gate {key} must be false in result for refuse case {case['name']}"
    ...
    assert res.returncode != 0, f"refuse case {case['name']} must not exit 0; stdout={res.stdout} stderr={res.stderr}"
...
def test_allow_case_is_allowed_by_gate(case: dict, tmp_path: Path):
    ...
    if "latest_rco_is_veto" in expected:
        assert result["latest_rco_is_veto"] is expected["latest_rco_is_veto"]
    for key in CLAIM_GATES:
        assert result[key] is False, f"gate {key} must be false in result for allow case {case['name']}"
    ...
    assert res.returncode == 0, f"allow case {case['name']} must exit 0; stderr={res.stderr} stdout={res.stdout}"
    payload = json.loads(res.stdout)
...
def test_corpus_events_exercise_head_binding_and_veto_logic(corpus: dict):
    ...
    assert len(heads) >= 2  # at least the main HEAD and the other_head used in stale case
