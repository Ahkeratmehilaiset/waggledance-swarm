...
def _hex_cross_consistency_bridge_event_template_index_entry_summary(
    ...
    template = _build_xcons_tpl(
        ...
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-digest-template-index-entry",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    ...
    template_entry = index_entry.get("template_index_entry")
    template_entry = template_entry if isinstance(template_entry, Mapping) else {}
    ...
    summary: dict[str, Any] = {
        ...
        "bridge_event_schema_validated": (
            template_entry.get("bridge_event_schema_validated") is True
        ),
        "source_contract_match": consistency.get("source_contract_check") == "match",
        "digest_ref_match": consistency.get("digest_ref_check") == "match",
        "rebuilt_template_match": (
            consistency.get("rebuilt_template_check") == "match"
        ),
        ...
        "no_bridge_event_written": index_entry.get("bridge_event_written") is False,
        "no_payloads_included": (
            index_entry.get("digest_payloads_included") is False
            and index_entry.get("artifact_payloads_included") is False
        ),
        "no_local_paths_recorded": index_entry.get("local_paths_recorded") is False,
        "cross_consistent": template_entry.get("cross_consistent") is True,
        "all_views_present": template_entry.get("all_views_present") is True,
        "reviewer_clean": template_entry.get("reviewer_clean") is True,
        "shadow_only_clean": template_entry.get("shadow_only_clean") is True,
        "chain_summary_clean": template_entry.get("chain_summary_clean") is True,
        "claim_safe": False,
    }
    summary["path_free_verified"] = (
...
def _hex_cross_consistency_bridge_event_template_index_entry_verification_summary(
    digest: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Content-safe summary of the hex cross-consistency index verifier."""
    ...
    template = _build_xcons_tpl(
        digest=source_digest,
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-template-index-entry-verification",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    template = template if isinstance(template, Mapping) else {}
    ...
    schema_checks = verification.get("schema_version_checks")
    schema_checks = schema_checks if isinstance(schema_checks, Mapping) else {}
    summary: dict[str, Any] = {
        "report_version": (
        ...
        and all(item == "match" for item in size_checks.values()),
...
def _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_summary(
    digest: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Content-safe summary of the hex cross-consistency summary template."""
    ...
    template = _build_xcons_tpl(
        digest=source_digest,
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-template-index-entry-verification-summary-template",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    template = template if isinstance(template, Mapping) else {}
    ...
    summary_template = summary_template if isinstance(summary_template, Mapping) else {}
    event = summary_template.get("bridge_event_template")
    event = event if isinstance(event, Mapping) else {}
    payload = event.get("payload")
    payload = payload if isinstance(payload, Mapping) else {}
    summary_verification = payload.get(_hex_verification_key)
    summary_verification = (
        summary_verification if isinstance(summary_verification, Mapping) else {}
    )
    digest_checks = summary_verification.get("digest_checks")
    digest_checks = digest_checks if isinstance(digest_checks, Mapping) else {}
    size_checks = summary_verification.get("size_checks")
    size_checks = size_checks if isinstance(size_checks, Mapping) else {}
    schema_checks = summary_verification.get("schema_version_checks")
    schema_checks = schema_checks if isinstance(schema_checks, Mapping) else {}
    summary: dict[str, Any] = {
        "report_version": (
            "wd.hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
            "verification_summary_bridge_event_template_summary.v1"
        ),
        "template_available": summary_template.get("ok") is True,
        "template_only": summary_template.get("template_only") is True,
        "bridge_event_schema_validated": (
            summary_template.get("ok") is True
        ...
        "verification_summary_available": verification_summary.get("ok") is True,
        "source_contract_match": (
            summary_verification.get("source_contract_check") == "match"
        ),
        "rebuilt_index_entry_match": (
            summary_verification.get("rebuilt_index_entry_check") == "match"
        ),
        ...
        "digest_ref_match": summary_verification.get("digest_ref_check") == "match",
        "all_digest_checks_match": bool(digest_checks)
        and all(item == "match" for item in digest_checks.values()),
        "all_size_checks_match": bool(size_checks)
        and all(item == "match" for item in size_checks.values()),
        ...
        and all(item == "match" for item in schema_checks.values()),
        "no_runtime_authority_granted": (
            summary_template.get("runtime_authority_granted") is False
        ),
        "no_runtime_subdivision_authority_granted": (
            summary_template.get("runtime_subdivision_authority_granted") is False
        ),
        "no_direct_bridge_write": (
            summary_template.get("direct_bridge_write_performed") is False
        ),
        "no_bridge_event_written": summary_template.get("bridge_event_written")
        is False,
        "no_payloads_included": (
            summary_template.get("digest_payloads_included") is False
            and summary_template.get("artifact_payloads_included") is False
        ),
        "no_local_paths_recorded": summary_template.get("local_paths_recorded")
        is False,
        "cross_consistent": source_digest.get("cross_consistent") is True,
        "all_views_present": source_digest.get("all_views_present") is True,
        "reviewer_clean": source_digest.get("reviewer_clean") is True,
    ...
    summary["path_free_verified"] = (
        summary_template.get("ok") is True and not _contains_path_marker(summary)
    )
    return summary


def _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_summary(
    digest: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Content-safe summary of the hex cross-consistency summary-template index."""
    ...
    template = _build_xcons_tpl(
        digest=source_digest,
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-template-index-entry-verification-summary-template-index",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    template = template if isinstance(template, Mapping) else {}
    try:
        ...
        summary_template_index_entry = {}

    summary_template_index_entry = (
        summary_template_index_entry
        if isinstance(summary_template_index_entry, Mapping)
        else {}
    )
    template_entry = summary_template_index_entry.get("template_index_entry")
    template_entry = template_entry if isinstance(template_entry, Mapping) else {}
    consistency = summary_template_index_entry.get("consistency")
    consistency = consistency if isinstance(consistency, Mapping) else {}
    verification_summary_block = summary_template_index_entry.get(
        "verification_summary"
    )
    verification_summary_block = (
        verification_summary_block
        if isinstance(verification_summary_block, Mapping)
        else {}
    )
    summary: dict[str, Any] = {
        "report_version": (
            "wd.hex_upgrade_cross_consistency_digest_bridge_event_template_index_entry_"
            "verification_summary_bridge_event_template_index_entry_summary.v1"
        ),
        "index_entry_available": summary_template_index_entry.get("ok") is True,
        "template_only": summary_template_index_entry.get("template_only") is True,
        "bridge_event_schema_validated": (
            template_entry.get("bridge_event_schema_validated") is True
        ),
        "source_contract_match": template_entry.get("source_contract_check")
        == "match",
        "rebuilt_template_match": template_entry.get("rebuilt_template_check")
        == "match",
        "verification_summary_available": (
            verification_summary_block.get("verification_ok") is True
        ),
        "source_index_entry_version_match": (
            verification_summary_block.get("source_index_entry_version_match")
            is True
        ),
        "all_artifact_digests_recorded": (
        ...
        "no_runtime_authority_granted": (
            summary_template_index_entry.get("runtime_authority_granted") is False
        ),
        "no_runtime_subdivision_authority_granted": (
            summary_template_index_entry.get("runtime_subdivision_authority_granted")
            is False
        ),
        "no_direct_bridge_write": (
            summary_template_index_entry.get("direct_bridge_write_performed") is False
        ),
        "no_bridge_event_written": (
            summary_template_index_entry.get("bridge_event_written") is False
        ),
        "no_payloads_included": (
            summary_template_index_entry.get("digest_payloads_included") is False
            and summary_template_index_entry.get("artifact_payloads_included") is False
        ),
        "no_local_paths_recorded": (
            summary_template_index_entry.get("local_paths_recorded") is False
        ),
        "cross_consistent": source_digest.get("cross_consistent") is True,
        "all_views_present": source_digest.get("all_views_present") is True,
        "reviewer_clean": source_digest.get("reviewer_clean") is True,
    ...
    summary["path_free_verified"] = (
        summary_template_index_entry.get("ok") is True
        and not _contains_path_marker(summary)
    )
    return summary


def _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary(
    digest: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Content-safe summary of the hex cross-consistency summary-template index verifier."""
    ...
    template = _build_xcons_tpl(
        digest=source_digest,
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-template-index-entry-verification-summary-template-index-verifier",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    template = template if isinstance(template, Mapping) else {}
    ...
    verification = (
        summary_template_index_entry_verification
        if isinstance(summary_template_index_entry_verification, Mapping)
        else {}
    )
    digest_checks = verification.get("digest_checks")
    digest_checks = digest_checks if isinstance(digest_checks, Mapping) else {}
    size_checks = verification.get("size_checks")
    size_checks = size_checks if isinstance(size_checks, Mapping) else {}
    schema_version_checks = verification.get("schema_version_checks")
    schema_version_checks = (
    ...
    summary: dict[str, Any] = {
        ...
        == "match",
        "rebuilt_index_entry_match": verification.get("rebuilt_index_entry_check")
        == "match",
        "bridge_event_schema_match": verification.get("bridge_event_schema_check")
        == "match",
        "all_digest_checks_match": bool(digest_checks)
        ...
        "no_bridge_event_written": (
            verification.get("bridge_event_written") is False
        ),
        "no_payloads_included": (
...
def _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_reviewer_summary(
    digest: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Content-safe summary of the reviewer summary for the summary-template index verifier."""
    ...
    template = _build_xcons_tpl(
        digest=source_digest,
        agent_id="codex-lead-1",
        task_id="wd-image1-hex-xcons-summary-template-index-entry-verification-summary",
        to="operator,claude-rco-1,codex-tools-1",
        role="lead-impl",
    )
    template = template if isinstance(template, Mapping) else {}
    try:
        ...
        reviewer_summary = _build_summary_template_index_entry_verification_summary(
            ...
            reviewer_agent_id="codex-tools-1",
            handoff_ref=(
                "wd-image1-hex-xcons-summary-template-index-entry-verification-summary"
            ),
        )
    except Exception:
        reviewer_summary = {}
    ...
    verification_block = reviewer_summary.get(
    ...
    boundary = reviewer_summary.get("operator_boundary")
    ...
    summary: dict[str, Any] = {
        "report_version": (
            ...
            "reviewer_summary_summary.v1"
        ),
        "summary_available": reviewer_summary.get("ok") is True,
        "template_only": reviewer_summary.get("template_only") is True,
        "verification_report_boundary_ok": (
        ...
        and all(value == "match" for value in schema_version_checks.values()),
        "no_runtime_authority_granted": (
            reviewer_summary.get("runtime_authority_granted") is False
        ),
        "no_runtime_subdivision_authority_granted": (
            reviewer_summary.get("runtime_subdivision_authority_granted") is False
        ),
        "no_direct_bridge_write": (
            reviewer_summary.get("direct_bridge_write_performed") is False
        ),
        "no_bridge_event_written": (
            reviewer_summary.get("bridge_event_written") is False
        ),
        "no_payloads_included": (
            reviewer_summary.get("digest_payloads_included") is False
            and reviewer_summary.get("artifact_payloads_included") is False
        ),
        "no_local_paths_recorded": reviewer_summary.get("local_paths_recorded")
        is False,
        "cross_consistent": source_digest.get("cross_consistent") is True,
        "all_views_present": source_digest.get("all_views_present") is True,
        "reviewer_clean": source_digest.get("reviewer_clean") is True,
    ...
    summary["path_free_verified"] = (
        reviewer_summary.get("ok") is True and not _contains_path_marker(summary)
    )
    return summary
...
def _capabilities(root: Path) -> tuple[Capability, ...]:
    ...
    hex_upgrade_evidence = _evidence(
        ...
        (
            ...
            (
                ...
                "Local index entry for the hex-upgrade cross-consistency digest bridge-event template; digest binding only, no payload/path inclusion, bridge append, claim upgrade, or runtime subdivision authority.",
            ),
            (
            ...
            (
                ...
                "Template-only bridge-event renderer for the hex-upgrade cross-consistency digest bridge-event template index-entry verifier summary; no payload/path inclusion, bridge append, claim upgrade, or runtime subdivision authority.",
            ),
            (
                ...
                "Local index entry for the hex-upgrade cross-consistency digest bridge-event template index-entry verifier-summary bridge-event template; digest binding only, no payload/path inclusion, bridge append, claim upgrade, or runtime subdivision authority.",
            ),
            (
                ...
                "Local verifier for the hex-upgrade cross-consistency digest bridge-event template index-entry verifier-summary bridge-event template index entry; recomputes digest, size, schema, source-contract, rebuilt-entry, and no-authority checks.",
            ),
            (
            ...
            (
                ...
                "Verifier-summary bridge-event template tests prove schema-valid template-only handoff JSON, path-free CLI output, contract rejection, and no runtime subdivision authority.",
            ),
            (
                ...
                "Verifier-summary bridge-event template index-entry tests prove digest binding, path-free CLI output, drift rejection, and no runtime subdivision authority.",
            ),
            (
                ...
                "Verifier-summary bridge-event template index-entry verifier tests prove digest recomputation, path-free CLI output, drift rejection, and no runtime subdivision authority.",
            ),
            (
    ...
    # Local template index-entry summary for the same digest bridge-event template.
    # Stored as content-safe derived booleans only, never raw event/template payloads
    # or local paths. Measurement-only: not folded into hex_upgrade_proof["ok"] and
    # never upgrades a claim or grants runtime subdivision authority.
    hex_upgrade_proof[
    ...
    # Local verifier summary for that digest bridge-event template index entry.
    # Stored as derived booleans only. Measurement-only: not folded into
    # hex_upgrade_proof["ok"] and never upgrades claims or grants runtime
    # subdivision authority.
    hex_upgrade_proof[
        "cross_consistency_digest_bridge_event_template_index_entry_verification"
    ] = _hex_cross_consistency_bridge_event_template_index_entry_verification_summary(
        hex_upgrade_proof["cross_consistency_digest"]
    )
    # Template-only bridge-event renderer summary for that cross-consistency
    # verifier-summary result. Stored as derived booleans only. Measurement-only:
    # not folded into hex_upgrade_proof["ok"] and never appends bridge events,
    # upgrades claims, transports payloads, or grants runtime subdivision authority.
    hex_upgrade_proof[
        "cross_consistency_digest_bridge_event_template_index_entry_verification_summary_bridge_event_template"
    ] = _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_summary(
        hex_upgrade_proof["cross_consistency_digest"]
    )
    # Local index-entry summary for the verifier-summary bridge-event template.
    # Stored as derived booleans only. Measurement-only: not folded into
    ...
    ] = _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_summary(
        hex_upgrade_proof["cross_consistency_digest"]
    )
    # Local verifier summary for that verifier-summary bridge-event template index
    # entry. Stored as derived booleans only. Measurement-only: not folded into
    # hex_upgrade_proof["ok"] and never includes payloads/paths, appends bridge
    # events, upgrades claims, or grants runtime subdivision authority.
    hex_upgrade_proof[
    ...
    ] = _hex_cross_consistency_bridge_event_template_index_entry_verification_summary_bridge_event_template_index_entry_verification_summary(
        hex_upgrade_proof["cross_consistency_digest"]
    )
    # Path-free reviewer-summary renderer output for that verifier result.
    # Stored as derived booleans only. Measurement-only: not folded into
    # hex_upgrade_proof["ok"] and never includes payloads/paths, appends bridge
    # events, upgrades claims, or grants runtime subdivision authority.
    hex_upgrade_proof[
    ...
    return (
        ...
        Capability(
            ...
            safe_statement=(
                ...
                "entry can bind that template and the source digest by digest, "
                "size, schema, source-contract, rebuilt-template, and "
                "bridge-event-schema checks without including payloads, "
                "recording paths, appending bridge events, upgrading claims, "
                "or granting runtime subdivision authority. A local verifier "
                "for that cross-consistency digest bridge-event template "
                "index entry recomputes digest, size, schema, "
                "source-contract, rebuilt-entry, digest-ref, and "
                ...
                "authority boundary. A template-only bridge-event renderer "
                "turns that verifier summary into schema-valid handoff JSON "
                "while preserving the same no-payload, no-path, no-append, "
                "no-transport, no-claim-upgrade, and no-runtime-subdivision "
                "authority boundary. A local index entry binds that "
                "verifier-summary bridge-event template to its source summary "
                "by digest, size, schema, source-contract, rebuilt-template, "
                "and bridge-event-schema checks without payload inclusion, "
                "local path recording, bridge writes, transport, claim "
                "upgrades, or runtime subdivision authority. A local verifier "
