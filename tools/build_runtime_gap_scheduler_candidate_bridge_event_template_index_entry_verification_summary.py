...
"""Render a path-free runtime-gap scheduler index-entry verifier summary."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--now",
        default=None,
        help="Optional UTC timestamp override such as 2026-06-07T14:35:00Z.",
    )
...
def build_runtime_gap_scheduler_candidate_bridge_event_template_index_entry_verification_summary(
    *,
    ...
    summary = {
        ...
        "runtime_gap_scheduler_candidate_bridge_event_template_index_entry_verification": {
            ...
            "scheduler_enqueue_allowed": verification_report.get(
                "scheduler_enqueue_allowed"
            )
            is True,
            "scheduler_tick_allowed": verification_report.get("scheduler_tick_allowed")
            is True,
        ...
        "reviewer_next_actions": [
            "review_runtime_gap_scheduler_candidate_index_entry_verification_summary",
...
def render_runtime_gap_scheduler_candidate_bridge_event_template_index_entry_verification_summary_markdown(
    ...
    lines = [
        "# Runtime-Gap Scheduler Index-Entry Verification Summary",
        "",
        ...
        "- Queue writes applied: `false`",
        "- Control-plane writes applied: `false`",
    ...
    lines.extend(
        [
            ...
            "include payloads, record local paths, enqueue scheduler work, run a "
            "scheduler tick, write queues/control-plane state, fast-track priority, "
            "skip gates, or grant runtime authority.",
            "",
        ]
    )
    markdown = "\n".join(lines)
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "runtime-gap scheduler index-entry verification summary contains "
            "forbidden markers"
        )
