...
"""Verify a local route-stage reviewer handoff bundle summary template index."""
...
SUMMARY_VERSION = (
    "waggledance.route_stage_feed_health_drill_evidence_reviewer_handoff_"
    "bundle_verification_summary.v1"
)
TEMPLATE_VERSION = (
    "waggledance.route_stage_feed_health_drill_evidence_reviewer_handoff_"
    "bundle_verification_summary_bridge_event_template.v1"
)
INDEX_ENTRY_VERSION = (
    "waggledance.route_stage_feed_health_drill_evidence_reviewer_handoff_"
    "bundle_verification_summary_bridge_event_template_index_entry.v1"
)
...
_TEMPLATE_REPORT_KEYS = frozenset(
    {
        "proof_id",
        "ok",
        "template_version",
...
class TemplateIndexEntryContractError(ValueError):
    """Raised when source artifacts cannot rebuild the local index entry."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        summary_bytes, summary = _load_json_artifact(
            args.bundle_verification_summary_json,
            SUMMARY_ARTIFACT_ID,
        )
        ...
        report = verify_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry(
            ...
            bundle_verification_summary=summary,
            bridge_event_template_report=template_report,
            bundle_verification_summary_bytes=summary_bytes,
...
def verify_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry(
    *,
    index_entry: Mapping[str, Any],
    bundle_verification_summary: Mapping[str, Any],
    bridge_event_template_report: Mapping[str, Any],
    bundle_verification_summary_bytes: bytes,
    bridge_event_template_bytes: bytes,
) -> dict[str, Any]:
    """Recompute local artifact checks without emitting bridge events."""

    for artifact_id, artifact in (
        (INDEX_ENTRY_ARTIFACT_ID, index_entry),
        (SUMMARY_ARTIFACT_ID, bundle_verification_summary),
    ...
    rebuilt_entry = _rebuilt_source_index_entry(
        bundle_verification_summary=bundle_verification_summary,
        bridge_event_template_report=bridge_event_template_report,
        bundle_verification_summary_bytes=bundle_verification_summary_bytes,
...
def build_route_stage_feed_health_drill_evidence_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry(
    *,
    bundle_verification_summary: Mapping[str, Any],
    bridge_event_template_report: Mapping[str, Any],
    bundle_verification_summary_bytes: bytes,
    ...
    """Return the expected path-free local index entry for verifier use."""

    _assert_source_contract(
        bundle_verification_summary=bundle_verification_summary,
        bridge_event_template_report=bridge_event_template_report,
    )
    summary_digest = _sha256_hex(bundle_verification_summary_bytes)
    ...
    artifacts = [
        _artifact_record(
            ...
            role="route_stage_reviewer_handoff_bundle_verification_summary",
            artifact=bundle_verification_summary,
            raw=bundle_verification_summary_bytes,
        ),
        _artifact_record(
    ...
    entry = {
        ...
        "consistency": {
            "required_artifacts_present": list(_REQUIRED_ARTIFACTS),
        ...
        "reviewer_next_actions": [
            "review_route_stage_reviewer_handoff_bundle_verification_summary_bridge_event_template_index_entry",
            "compare_index_entry_to_local_summary_and_template",
...
def _rebuilt_source_index_entry(
    *,
    ...
    except TemplateIndexEntryContractError as exc:
...
def _assert_source_contract(
    *,
    ...
    blockers: list[str] = []
    blockers.extend(_summary_contract_blockers(bundle_verification_summary))
    blockers.extend(_template_contract_blockers(bridge_event_template_report))
    if blockers:
        raise TemplateIndexEntryContractError(sorted(set(blockers))[0])


def _summary_contract_blockers(summary: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if summary.get("ok") is not True:
        blockers.append("summary_not_ok")
    ...
    if summary.get("template_only") is not True:
        blockers.append("summary_template_only_not_true")
    if summary.get("manual_review_required") is not True:
        blockers.append("summary_manual_review_required_not_true")
    for field in AUTHORITY_FALSE_FIELDS:
        if summary.get(field) is not False:
            blockers.append(f"summary_{field}_not_false")
    if summary.get("blockers") != []:
        blockers.append("summary_blockers_present")
    boundary = _mapping(summary.get("operator_boundary"))
    if boundary.get("manual_review_required") is not True:
        blockers.append("summary_operator_boundary_manual_review_required_not_true")
    for field in AUTHORITY_FALSE_FIELDS:
        if boundary.get(field) is not False:
            blockers.append(f"summary_operator_boundary_{field}_not_false")
    ...
    if verification.get("verification_ok") is not True:
        blockers.append("summary_verification_not_ok")
    for field in (
        ...
        if verification.get(field) != "match":
            blockers.append(f"summary_verification_{field}_not_match")
    if verification.get("blocker_count") != 0:
        blockers.append("summary_verification_blocker_count_nonzero")
    if verification.get("template_only") is not True:
        blockers.append("summary_verification_template_only_not_true")
    return sorted(set(blockers + _recursive_authority_blockers(summary, "summary")))


def _template_contract_blockers(template_report: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    unexpected = sorted(set(template_report) - _TEMPLATE_REPORT_KEYS)
    blockers.extend(f"bridge_event_template_unexpected_field:{key}" for key in unexpected)
    ...
    if template_report.get("template_only") is not True:
        blockers.append("bridge_event_template_template_only_not_true")
    if template_report.get("manual_review_required") is not True:
        blockers.append("bridge_event_template_manual_review_required_not_true")
    for field in AUTHORITY_FALSE_FIELDS:
        if template_report.get(field) is not False:
            blockers.append(f"bridge_event_template_{field}_not_false")
    if template_report.get("blockers") != []:
    ...
    try:
        ...
        blockers.append("bridge_event_template_schema_invalid")
    if event.get("type") != "handoff":
        blockers.append("bridge_event_template_type_mismatch")
    if event.get("status") != EVENT_STATUS:
        blockers.append("bridge_event_template_status_mismatch")
    if event.get("paths") != []:
        blockers.append("bridge_event_template_paths_not_empty")
    if event.get("write_scope") != []:
        blockers.append("bridge_event_template_write_scope_not_empty")
    if event.get("pid") != 0:
        blockers.append("bridge_event_template_pid_not_zero")
    if event.get("cwd") != "template_not_emitted":
        blockers.append("bridge_event_template_cwd_mismatch")

    payload = _mapping(event.get("payload"))
    if payload.get("schema_version") != TEMPLATE_VERSION:
        blockers.append("bridge_event_template_payload_schema_mismatch")
    if payload.get("summary_version") != SUMMARY_VERSION:
        blockers.append("bridge_event_template_payload_summary_version_mismatch")
    if payload.get("template_only") is not True:
        blockers.append("bridge_event_template_payload_template_only_not_true")
    if payload.get("manual_review_required") is not True:
        blockers.append("bridge_event_template_payload_manual_review_required_not_true")
    for field in AUTHORITY_FALSE_FIELDS:
        if payload.get(field) is not False:
            blockers.append(f"bridge_event_template_payload_{field}_not_false")
    boundary = _mapping(payload.get("operator_boundary"))
    if boundary.get("manual_review_required") is not True:
        blockers.append("bridge_event_template_boundary_manual_review_required_not_true")
    for field in AUTHORITY_FALSE_FIELDS:
        if boundary.get(field) is not False:
            blockers.append(f"bridge_event_template_boundary_{field}_not_false")
    return sorted(set(blockers + _recursive_authority_blockers(template_report, "bridge_event_template")))


def _recursive_authority_blockers(value: Any, label: str) -> list[str]:
    blockers: list[str] = []
    if isinstance(value, Mapping):
        for raw_key, child in value.items():
            ...
            blockers.extend(_recursive_authority_blockers(child, label))
...
def _collect_template_entry_blockers(
    ...
    *,
    ...
    if template_entry.get("template_sha256") != _sha256_hex(bridge_event_template_bytes):
...
def _assert_no_forbidden_output(text: str) -> None:
    if _forbidden_output_markers(text):
        raise ValueError(
            "route-stage reviewer handoff bundle verification-summary "
            "bridge-event template index entry verification contains "
            "forbidden markers"
        )
